...
export const digitalCourse = {
  ...
  sourceNote: "主线按本地阎石《数字电子技术基础》第六版目录与正文核对；第 1–6 章另有分章笔记，第 3、4、6 章可对照 SystemVerilog/ModelSim 工程。第 7、8 章以教材为主，现有 Web 工作台只承担可验证的逻辑级降级实验。",
  ...
  chapters: [
    {
      ...
      experiments: [{ id: "digital-01-exp", title: "四位开关数值观察", workbench: "digital", goal: "验证四位二进制从 0000 到 1111 与十六进制 0 到 F 的一一对应。", steps: ["放置四个开关和四个 LED", "依次切换 0000、0101、1010、1111", "记录探针位序和对应数值"], expected: "LED 与探针稳定显示输入位型，四位组合分别对应 0、5、A、F。", limitation: "资料关联：教材第 1 章与分章笔记 1-Number system；本地没有本章独立 HDL 工程，本实验只验证位型与码值。" }],
      check: [
    ...
    },
    {
      ...
      experiments: [{ id: "digital-02-exp", title: "等价逻辑式验证", workbench: "digital", goal: "用真值表验证德摩根关系 ¬(A·B)=¬A+¬B。", steps: ["搭建与非门支路", "搭建两个非门接或门支路", "遍历 A、B 四种组合并比较输出"], expected: "两条支路在四种输入下输出完全一致。", limitation: "资料关联：教材第 2 章与分章笔记 2-Boolean algebra；本地无独立化简实验工程，Web 真值表用于逐行核对等价性。" }],
      check: [
    ...
    },
    {
      ...
      experiments: [{ id: "digital-03-exp", title: "通用门搭建", workbench: "digital", goal: "验证仅用与非门实现非、与、或三种功能。", steps: ["用并接输入实现非门", "在与非输出后再接与非反相得到与门", "按德摩根关系搭建或门", "运行真值表"], expected: "三种搭建分别与标准非、与、或门真值表一致。", limitation: "资料关联：本地 Appendix_code 提供 inv.sv、gates.sv、and8.sv 与 tristate.sv；Web 工作台不执行这些 HDL 文件，也不模拟 CMOS/TTL 电气参数。" }],
      check: [
    ...
    },
    {
      ...
      experiments: [{ id: "digital-04-exp", title: "一位全加器", workbench: "digital", goal: "验证全加器八行真值表和进位关系。", steps: ["放置三个开关作为 A、B、Cin", "用异或、与、或门搭建 S 与 Cout", "连接两个 LED", "运行全部输入组合"], expected: "S=A⊕B⊕Cin；任意两个或三个输入为高时 Cout=1。", presetId: "digital-full-adder", limitation: "资料关联：Appendix_code 含 fulladder、mux、decoder 及对应测试文件；Web 实验验证同一逻辑关系，但不冒充 ModelSim 编译与测试平台结果。" }],
      check: [
    ...
    },
    {
      ...
      experiments: [{ id: "digital-05-exp", title: "D 触发器采样", workbench: "digital", goal: "验证 D 触发器只在有效时钟边沿更新输出。", steps: ["放置开关、时钟、D 触发器和 LED", "在两个时钟边沿之间改变 D", "用逻辑分析仪同时观察 D、CLK、Q"], expected: "Q 不跟随边沿之间的 D 变化，只在有效边沿复制 D。", presetId: "digital-d-flipflop", limitation: "资料关联：本地工程提供 latch.sv、flop.sv 与测试文件，Modelsim.md 含 RAM/ROM 示例；没有独立存储器工程，Web 实验只验证 D 触发器边沿行为。" }],
      check: [
    ...
    },
    {
      ...
      examples: [{ title: "由周期求频率与占空比（自编）", prompt: "矩形脉冲周期 T=2 ms，高电平宽度 t_w=0.5 ms，求频率与占空比。", steps: ["把周期换算为 0.002 s", "由 f=1/T 得到 500 Hz", "由 q=t_w/T 得到 0.25"], answer: "频率为 500 Hz，占空比为 25%。" }],
      experiments: [{ id: "digital-07-exp", title: "时钟参数观察（降级实验）", workbench: "digital", goal: "验证工作台时钟频率设置与逻辑分析仪显示周期的一致性，不冒充整形电路仿真。", steps: ["放置时钟和逻辑探针", "设置 1 Hz 后观察两个周期", "改为 2 Hz 并再次测量"], expected: "1 Hz 周期约 1 s，2 Hz 周期约 0.5 s；仅验证内置信号源。", limitation: "教材覆盖施密特、单稳态、多谐振荡与 555；本地无对应独立 HDL 工程，Web 工作台也没有 RC、阈值或 555 器件模型，只能验证理想时钟参数。" }],
      check: [
      ...
      summary: ["先用参数定量描述脉冲，再区分整形、延时与自激振荡。", "施密特有回差，单稳态有一个稳态，多谐振荡没有稳态。", "555 是通用实现平台；Web 降级实验只验证理想时钟，不替代器件级计算。"],
      tags: ["脉冲", "施密特", "单稳态", "555"],
    },
    {
      ...
      experiments: [{ id: "digital-08-exp", title: "三位代码阶梯观察", workbench: "digital", goal: "观察数字码递增形成的离散阶梯，验证代码顺序，不冒充模拟精密模型。", steps: ["放置三位计数器和代码探针", "将时钟设为 1 Hz", "记录 000 到 111 的码值序列", "跳转模拟工作台比较连续信号概念"], expected: "代码按八个离散状态循环；当前数字工作台只验证码序。", limitation: "教材覆盖真实 D/A、取样保持和多类 A/D；本地无对应独立实验工程。Web 数字台不求解模拟电压、量化误差、建立时间或采样过程。" }],
      check: [
      ...
      summary: ["D/A 由数字权重叠加得到模拟量，A/D 经过取样、保持、量化和编码得到数字码。", "结构选择要同时比较分辨率、误差、转换速度和电路规模。", "工作台的码序观察不是实际 ADC/DAC 仿真；模拟接口仍需真实电路模型与器件参数。"],
