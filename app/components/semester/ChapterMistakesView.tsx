...
export function ChapterMistakesView({ courses, state, onSave, onReviewed, onOpenChapter }: Props) {
  ...
  return <div className="mistakes-page page-enter">
    <header className="page-heading"><div><h1>练习与错题</h1><p>错题只记录原因和正确思路，不参与章节完成度。</p></div><button className="new-mistake-button" type="button" onClick={() => setEditing(null)}><Icon name="plus" size={18} />新增错题</button></header>
    ...
    <div className="mistake-table-wrap"><table className="mistake-table"><thead><tr><th>题目 / 章节</th><th>错误原因</th><th>正确思路</th><th>下次复习</th><th>状态</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{filtered.map((mistake) => { const course = courses.find((item) => item.id === mistake.courseId)!; const chapter = course.chapters.find((item) => item.id === mistake.chapterId)!; return <tr key={mistake.id} className={mistake.reviewed ? "is-mastered" : ""}><td><button type="button" className="mistake-title" onClick={() => onOpenChapter(course, chapter.id)}>{mistake.title}</button><small>{course.title} · {chapter.number} {chapter.title}</small></td><td>{mistake.reason}</td><td>{mistake.correctApproach}</td><td>{mistake.nextReviewDate ? <time dateTime={mistake.nextReviewDate}>{mistake.nextReviewDate}</time> : "未安排"}</td><td><span className={`status-label ${mistake.reviewed ? "mastered" : "pending"}`}>{mistake.reviewed ? "已复盘" : "待复盘"}</span></td><td className="mistake-actions"><button type="button" aria-label={`编辑：${mistake.title}`} onClick={() => setEditing(mistake)}><Icon name="edit" size={18} /></button><button type="button" disabled={mistake.reviewed} aria-label={`标记已复盘：${mistake.title}`} onClick={() => onReviewed(mistake.id)}><Icon name="check" size={18} /></button></td></tr>; })}</tbody></table></div>
