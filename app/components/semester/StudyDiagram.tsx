...
function axes(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, label: string) {
  context.strokeStyle = diagramColors.line;
  context.lineWidth = 1.5;
  arrow(context, x, y + height, x + width, y + height);
  arrow(context, x, y + height, x, y);
  context.fillStyle = diagramColors.muted;
...
export function StudyDiagram({ kind, title }: StudyDiagramProps) {
  ...
  useEffect(() => {
    ...
    context.fillStyle = diagramColors.background;
    context.fillRect(0, 0, width, height);
    context.lineCap = "round";
    context.lineJoin = "round";
    context.strokeStyle = diagramColors.ink;
    context.fillStyle = diagramColors.ink;
    context.lineWidth = 2;
    ...
    if (kind === "logic") {
      ...
      if (title.includes("卡诺")) {
        ...
        context.strokeStyle = diagramColors.teal; context.lineWidth = 4; context.strokeRect(256, 36, 388, 28);
        context.fillStyle = diagramColors.muted; context.fillText("格雷码相邻圈组 → 保留不变量", 310, 215);
      } else {
      ...
      context.strokeStyle = diagramColors.teal; context.beginPath(); context.moveTo(405, 110); context.lineTo(405, 160); context.lineTo(455, 135); context.closePath(); context.stroke();
      context.strokeStyle = diagramColors.ink; arrow(context, 455, 135, 710, 135);
      ...
      context.strokeStyle = diagramColors.teal; context.lineWidth = 3;
      ...
      context.strokeStyle = diagramColors.orange;
      ...
      context.fillStyle = diagramColors.muted; context.fillText(title.includes("卷积") ? "翻转 → 平移 → 找重叠区间 → 积分" : "只在有效时钟边沿更新状态", 300, 215);
    } else if (kind === "spectrum") {
      axes(context, 80, 25, 730, 165, "|X(jω)|");
      context.strokeStyle = diagramColors.teal; context.lineWidth = 4;
      ...
      context.fillStyle = diagramColors.muted; context.fillText("−2ω₀   −ω₀      0      ω₀      2ω₀", 260, 215);
    } else if (kind === "sampling") {
      axes(context, 55, 25, 770, 165, "x(t), x[n]");
      context.strokeStyle = diagramColors.muted; context.lineWidth = 2; context.beginPath();
      for (let x = 55; x <= 820; x += 4) {
        const y = 125 - 55 * Math.sin((x - 55) / 62);
        if (x === 55) context.moveTo(x, y); else context.lineTo(x, y);
      }
      context.stroke(); context.fillStyle = diagramColors.teal;
      for (let x = 80; x <= 800; x += 58) {
        const y = 125 - 55 * Math.sin((x - 55) / 62);
        context.beginPath(); context.arc(x, y, 5, 0, Math.PI * 2); context.fill();
        context.beginPath(); context.moveTo(x, 190); context.lineTo(x, y); context.strokeStyle = diagramColors.teal; context.stroke();
      }
      context.fillStyle = diagramColors.muted; context.fillText("采样点必须足够密，频谱副本才不会重叠", 300, 215);
    } else {
      const boxes = ["输入 x", "LTI 系统 h", "输出 y"];
      boxes.forEach((label, index) => {
        const x = 90 + index * 290;
        context.strokeStyle = index === 1 ? diagramColors.teal : diagramColors.line;
        context.lineWidth = index === 1 ? 3 : 2;
        context.strokeRect(x, 65, 190, 90);
        context.fillStyle = diagramColors.ink; context.fillText(label, x + 55, 116);
        if (index < 2) { context.strokeStyle = diagramColors.ink; arrow(context, x + 190, 110, x + 275, 110); }
      });
      context.fillStyle = diagramColors.muted; context.fillText("输入分解 → 系统响应 → 线性叠加", 320, 205);
    }
  }, [kind, title]);
