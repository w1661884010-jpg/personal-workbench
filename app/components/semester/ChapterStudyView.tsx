...
import { CHECK_PASS_SCORE, getChapterStatus } from "../../lib/course-model";
...
export function ChapterStudyView({ course, chapter, state, onBack, onSelectChapter, onSubmitCheck, onComplete, onOpenExperiment }: ChapterStudyViewProps) {
  ...
  const [answers, setAnswers] = useState<number[]>(() => Array.from({ length: chapter.check.length }, (_, index) => saved?.answers[index] ?? -1));
  const submitted = state.checkSubmissions[chapter.id];
  const passed = (submitted?.score ?? 0) >= CHECK_PASS_SCORE;
  ...
  return <div className="chapter-study page-enter">
    ...
    <article className="chapter-article">
      <header className="chapter-hero" style={{ borderColor: course.accent }}>
        <div><span className="eyebrow">{course.textbook} · {chapter.number}</span><h1>{chapter.title}</h1><p>{sectionCounts.core} 项主线必学 · {sectionCounts.optional} 项选择学习</p></div>
...
function Formula({ expression }: { expression: string }) {
  const html = useMemo(() => katex.renderToString(expression, { displayMode: true, throwOnError: false, strict: false }), [expression]);
