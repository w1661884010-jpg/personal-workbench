...
- [x] 阶段 B：横向紧凑切换器（176×44、两项等宽 84px）+ 说明图标同行右侧 + matchMedia 断点 transform/aria 归位（before 标签 restore-2026-09-05-before-narrow-layout）。
- [x] 阶段 B：运行按钮可换行且 cw-primary order:-1 优先；实验目标选择框独占一行、操作紧随其后；存区分组（名称+保存+另存 / 选择+载入+删除 / 清空与全删低强调）；输入 minmax(0,1fr) 可收缩；旧 760 块并入 ≤820 分组块、无空列空行。
- [x] 阶段 B：验证 + 回归测试先失败（15/18）后通过（19/19）；完整套件 69/69；桌面 1440 截图字节级一致（SHA256 A6A15A49…）。
- [x] 完整测试套件 69/69 + git diff --check + node --check app.js（未改打包入口，未重建 bundle）+ 窄屏前后截图与 1440 桌面对照截图。
- [x] 停在本地验收；未经确认不推送、不部署。
## 2026-09-05：演练页窄窗口布局优化（阶段 A：参数/结果/间距；阶段 B：图表）
- [x] 计划已记录（docs/superpowers/plans/2026-09-05-narrow-practice-layout.md）；before 标签 `restore-2026-09-05-before-narrow-practice-layout`（ec041b9 工作区干净）。
- [x] 阶段 A：参数区 ≤760 两列网格（repeat(2,minmax(0,1fr)) 间距 12、输入 min-height 42px）、≤340 极窄单列——按数量自适应不写死五字段。
- [x] 阶段 A：结果区去固定最小宽（2 列网格）、长结果 rows 第三元素 "wide"→语义类 is-wide 满行、极窄单列满宽；演练演示参数按实验 id 内存持久化（tab 切换/缩放不回滚）。
- [x] 阶段 A：≤620 压缩外层留白、返回按钮紧凑次要层级；桌面（1440 五参数单行）不变。
- [x] 阶段 A：回归测试先失败（5/6）后通过（6/6）；六演练全视口核验；提交 + after 标签。
- [x] 阶段 B：X 刻度数按绘图区宽度自适应（plotW<480 隔格标注，末端让位单位）、末端刻度与 “t / s / 样本 n” 不重叠；只改显示不改采样/计算。
...
# 2026-09-07 移动端画布有效边界
- [x] 残影按用户复测反馈标记已修复，原因不作确定断言。
- [x] 确認 SVG 最小高度造成上下空带，完成最小修改规划。
- [x] 补充失败回归，再调整移动端高度（375/393px 先失败，修复后 5/5）。
- [x] 完整测试 95/95、Pages 构建及 393px 截图核验；未提交、未推送。
# 2026-09-07 自适应画布与元件查找
- [x] 确定动态 viewBox、搜索网格与紧凑工具栏方案。
- [x] 完成回归测试并验证修改：先复现小画布失败；数字/模拟多比例、搜索、边缘放置与拖动通过。
- [x] 完整测试 98/98、bundle/Pages 构建、截图；未提交或推送。
# 3000 依赖隔离
- [x] 迁入源码和本地 npm 依赖，补齐 3020 构建输入。
- [x] 两份构建与完整测试各 135/135 通过，无旧目录有效引用。
- [ ] 3000 目录已消失，两份构建复验成功；SendToRecycleBin 返回成功，但未找到回收站条目，不可声明回收站恢复已验证。
# README 实拍说明
- [x] 拍摄并检查课程、演练、工作台截图。
- [x] 补充图解与准确的操作文案，修正单击选中说明。
- [x] 三个图片链接存在，完整测试 135/135、零跳过，发布构建通过。
