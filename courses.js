        ...
        experiments: [
          {
            ...
            title: "新时刻取到了哪个旧时刻的值",
            workbench: "notebook",
            goal: "把 x(at+b) 读成「新时刻 t 去旧波形上取 x(at+b) 这个值」，分清压缩拉伸（看 |a|）、反折（看 a 的符号）与平移（看 t₀=−b/a）三件事。",
            steps: [
              ...
              "反例对照二：取 a=−1、b=0，读出支撑 [−3, 0] 与反折状态；再把 b 调回 4，观察 t₀=−b/a 的变化，说明反折由 a 的符号决定、平移由 b 与 a 共同决定。",
              "用自己的话解释：为什么 x(2t−4) 是延迟 2 秒而不是 4 秒？"
            ],
            expected: "a=2,b=−4 时支撑为 [2, 3.5]、3/|a|=1.5、t₀=2 s；a=−1,b=0 时支撑为 [−3, 0] 且状态显示反折；a=1,b=0 时上下波形坐标值逐点一致；四个特征点 (0,0)(1,2)(2,2)(3,0) 按 t=(τ−b)/a 映射后代回误差 ≤1e-10。",
          ...
          },
          {
            ...
            steps: [
              "先预测：把观察时刻从 t=−1 移到 t=w₁，重叠面积会怎样变化？写下预测再动手。",
              "保持 w₁=w₂=1，把 t 从 −1 拖到 2，对照上图翻转平移的 h(t−τ) 与下图 y(t) 的游标位置。",
              "反例对照一：切到预设 (1,2)，分别记录峰值（=min(w₁,w₂)）与总面积（=w₁·w₂），说明二者为何不等。",
              "反例对照二：点击上图一个历史位置，核对它已历经的时间 t−τ 与 h(t−τ) 的取值，再解释“翻转”并不是为了让响应衰减。",
              "用自己的话解释：固定 t 观察历史变量 τ 时，为什么会出现翻转？"
            ],
            expected: "t=−1 时重叠面积为 0；(1,1) 在 t=1 取峰值 1，(1,2) 在 t∈[1,2] 走过平台 1；数值积分（dτ=0.005 复合梯形）与解析分段式绝对误差 < 0.01。",
          ...
          },
          {
            ...
            steps: [
              ...
              "反例对照二：只把 T 从 2 增到 8，谱点从 17 个变成 65 个、明显变密，但 T·c_k 仍落在同一条包络 sinc(f) 上——谱线变密没有改变各次谐波的相对权重。",
              "用自己的话解释：为什么 c_k 随 T 增大而减小，而 T·c_k 却始终落在同一条固定包络上？"
            ],
            expected: "T=2 时 |k/T|≤4 有 17 个谱点、T=8 时有 65 个（含零系数）；所有谱点落在包络 τ·sinc(τf) 上误差 ≤1e-10；T=4,k=2 时 c₂=1/(2π)；T=4,N=101 时 |x_N(0)−1|≤0.02；T=2,N=101 跳变内侧过冲落在 8.8%–9.2%；MSE 按一个周期 4096 个中点等权计算。",
            limitation: "只取单位高度、中心对称、脉宽 τ=1 的周期矩形列；渐近参考线 1.08949 只是极限值，不声称任意有限 N 的峰值都不超过它。"
          }
        ],
        check: [
          {
      ...
      },
      {
        ...
        sections: [
          ...
          },
          {
            ...
            pitfalls: [
              "系数向量顺序写错：freqz/zplane/residuez 的 b、a 按 [b0,b1,…] 对应 z^0、z^{−1}、… 排列，而 tf2zp 用的是 z 的降幂——同一个 H(z)=1−2z^{−1} 在 zplane 里写 [1 -2]、在 tf2zp 里要写成 ([1 -2],[1 0])。顺序写错时 tf2zp 会直接报“传递函数不当”",
        ...
        experiments: [
          {
            ...
            steps: [
              ...
              "取 f_in=7 Hz、f_s=20 Hz，观察 sinc 重建与原曲线的贴合程度，并记录 RMSE 读数。",
          ...
          },
          {
            ...
            steps: [
              ...
              "取默认 L=32、矩形窗、补零 M=0，记录指标卡的 L/f_s、f_s/L 与补零后网格 f_s/N_fft 三个量，并数一数 [9,13] Hz 内有几个局部峰。",
              "反例对照一：把 L 降到 16，两音合并成一个约 11.08 Hz 的峰；再把 L 增到 64，分成约 9.89 Hz 与 12.14 Hz 两个峰。说明「增大观察时间 ⇒ 主瓣变窄」而不是「补零让谱变清晰」。",
              "反例对照二：固定 L=64，只把补零 M 从 0 增到 224。谱线变密、网格 f_s/N_fft 变细，但同一频率上的谱值完全不变——补零增加的是读数密度，不是信息。",
          ...
          },
          {
            ...
            steps: [
              ...
              "取 N₁=N₂=4、N=4，对照左图线性结果 [1,2,3,4,3,2,1] 与右图循环结果 [4,4,4,4]，指出被折回的是哪三个样本。",
              "反例对照一：把 N 增到 7（= N₁+N₂−1），循环结果与线性完全一致、首 N 点误差 E=0；再看 N=10，尾部出现 3 个零。说明 N 够大就没有折回，而不是「循环卷积总是不准」。",
              "反例对照二：取 N₁=N₂=8、N=2，13 个折回样本被压进 2 个桶（桶数 = min(N, L−N)），每桶累加成 32；再对照 DFT 核对路线与直接路线。",
              "用自己的话解释：为什么 N ≥ N₁+N₂−1 就不会发生尾部折回？频域采样与时域周期求和是怎么对应起来的？"
            ],
        ...
        examples: [
          ...
          },
          {
            ...
            steps: [
              ...
              "用 freqs(b,a,[wp ws]) 读两点衰减（w 必须是向量）。验收标准是 wp 处 ≤1 dB、ws 处 ≥40 dB；实测：buttord 返回的 wc=1.1247 让阻带恰好取到 40.000 dB、通带留 0.617 dB 余量，而正文手算的 Ωc=1.088119（让通带恰为 1 dB）在 ws 处给出 42.297 dB——两者都合格，差别只在把余量留在哪一端。"
            ],
