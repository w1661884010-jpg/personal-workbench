    ...
    chapters: [
      {
        ...
        examples: [
          {
            title: "判断正弦信号的类别",
            prompt: "判断 \\(x(t)=2\\cos (4\\pi t)\\) 是连续还是离散、是否周期，并求基波周期和平均功率。",
            steps: [
              "自变量 t 连续，因此它是连续时间信号。",
              "角频率 \\(\\omega _0=4\\pi\\) rad/s，所以 \\(T_0=2\\pi /\\omega _0=0.5\\) s。",
        ...
        experiments: [
          {
            ...
            steps: [
              "在连续信号演示中生成 \\(x(t)=2\\cos (4\\pi t)\\)，显示 0—1.5 s。",
        ...
        sections: [
          {
            ...
            detail: [
              "复指数信号 \\(x(t)=e^{j\\omega_0t}\\) 是这一节的核心。由欧拉公式 \\(e^{j\\theta}=\\cos\\theta+j\\sin\\theta\\)，它同时装着余弦与正弦：\\(e^{j\\omega_0t}=\\cos\\omega_0t+j\\sin\\omega_0t\\)。",
              "把 \\(e^{j\\omega_0t}\\) 看成复平面上长度不变、以角频率 \\(\\omega_0\\) 匀速旋转的单位向量，它的实部是余弦、虚部是正弦——“相位”因此有了几何含义：相位就是那个转角。一般复指数再乘上 \\(e^{\\sigma t}\\) 的幅度包络，就能同时描述“振荡”与“增长、衰减”两种行为。",
              "取样信号 \\(\\operatorname{sinc}(x)=\\frac{\\sin(\\pi x)}{\\pi x}\\) 在 \\(x=0\\) 处取极限值 1，在非零整数点处为零，两侧按 1/x 衰减。它不是硬凑出来的函数：宽度为 τ 的矩形脉冲，其频谱正是一个 sinc，主瓣第一个零点落在 \\(f=1/\\tau\\)。脉宽越窄，零点越远、频谱越宽——这条“时宽与带宽成反比”的结论在第二节会反复用到。"
            ],
            points: [
              "正弦由幅度、角频率、初相三者唯一确定；相位是更本质的量，幅度只决定“多大”",
              "欧拉公式是复指数与正弦之间的双向字典：\\(e^{j\\theta}=\\cos\\theta+j\\sin\\theta\\)",
              "复指数 \\(e^{j\\omega_0t}\\) 的几何图像是匀速旋转的单位向量，实部、虚部分别是余弦与正弦",
              "\\(\\operatorname{sinc}(x)=\\frac{\\sin(\\pi x)}{\\pi x}\\)：\\(x=0\\) 处为 1，非零整数点为零，正是矩形脉冲的频谱形状"
            ],
            pitfalls: [
              "角频率 ω 与频率 f 混用：\\(\\omega=2\\pi f\\)，相差一个 \\(2\\pi\\)；把“50 Hz”写成 \\(\\omega =50\\) 是常见错误",
              "记错 sinc 的尺度约定：\\(\\sin(\\pi x)/(\\pi x)\\) 的零点是所有非零整数，而 \\(\\sin x/x\\) 的零点在 π 的整数倍"
            ],
            links: [
            ...
            variables: [
              "θ：相位角，弧度制；复指数里取 θ=\\(\\omega_0t\\)",
              "j：虚数单位，\\(j^2=-1\\)",
              "\\(\\omega_0\\)：角频率（rad/s），与频率的关系是 \\(\\omega_0=2\\pi f\\)",
              "\\(\\operatorname{sinc}(x)=\\frac{\\sin(\\pi x)}{\\pi x}\\)：x=0 处取极限值 1，非零整数点为零（\\(\\sin x/x\\) 的写法零点在 π 的整数倍）"
            ]
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "从零跳到一的阶跃，用来描述开关接通；持续时间趋近于零、但总作用量保持不变的冲激，用来描述瞬时激励。阶跃仍可用普通函数表示，冲激则要在广义函数意义下理解。卷积、采样与系统冲激响应，都要用到这些基本信号。",
            detail: [
              "先看单位冲激的极限图像：矩形脉冲的宽度为 \\(\\tau\\)、高度为 \\(1/\\tau\\)，面积始终等于 1。当 \\(\\tau\\to0\\) 时，脉冲越来越窄、越来越高，在广义函数意义下趋于 \\(\\delta(t)\\)。不能把它当作一个在原点有普通函数值的信号。",
              "筛选性质把这幅图像变成可计算的规则：\\[\\int_{-\\infty}^{\\infty}x(t)\\delta(t-t_0)\\,dt=x(t_0).\\]冲激位于 \\(t_0\\)，积分便“取出”该处的函数值；通常要求 \\(x\\) 在该点连续。冲激是偶的，即 \\(\\delta(-t)=\\delta(t)\\)；尺度变换满足 \\(\\delta(at)=\\delta(t)/|a|\\)，其中 \\(a\\ne0\\)。注意分母的绝对值，不能写成 \\(a\\)。",
              "阶跃、冲激、冲激偶形成一条微分链：\\[\\frac{d}{dt}u(t)=\\delta(t),\\qquad \\frac{d}{dt}\\delta(t)=\\delta'(t).\\]冲激偶的筛选性质多一个负号：\\[\\int_{-\\infty}^{\\infty}x(t)\\delta'(t-t_0)\\,dt=-x'(t_0).\\]反向积分时写明积分限：\\(\\int_{-\\infty}^{t}\\delta(\\tau)\\,d\\tau=u(t)\\)；再积一次阶跃，得到斜升 \\(t\\,u(t)\\)。",
              "含跳变的信号求导，不能只看各个平滑区间。例如 \\(x(t)=u(t)-u(t-1)\\)，在两个跳变点之间的导数为零，但完整的广义导数是\\[x'(t)=\\delta(t)-\\delta(t-1).\\]向上跳 1，对应强度为 1 的正冲激；向下跳 1，对应负冲激。一般地，有符号跳变量为 \\(A\\)，冲激强度就是 \\(A\\)。"
            ],
            points: [
              "奇异信号由普通信号取极限得到；冲激的面积（强度）恒为 1",
              "筛选性质 \\[\\int_{-\\infty}^{\\infty}x(t)\\delta(t-t_0)\\,dt=x(t_0)\\] 是冲激最有用的性质",
              "冲激是偶函数；\\(\\delta(at)=\\frac{\\delta(t)}{|a|}\\)，尺度因子必须取绝对值",
              "微积分链：\\(\\frac{du}{dt}=\\delta,\\quad\\frac{d\\delta}{dt}=\\delta'\\)；反向 \\[\\begin{aligned}\\int_{-\\infty}^t\\delta(\\tau)\\,d\\tau&=u(t)\\\\[4pt]\\int_{-\\infty}^t u(\\tau)\\,d\\tau&=t\\,u(t)\\end{aligned}\\]"
            ],
            pitfalls: [
              "把 \\(\\delta(t)\\) 想成“在 0 点取无穷大、别处为 0 的函数”：它不是一个普通函数，只有放进积分里才有意义",
            ...
            variables: [
              "\\(\\delta(t)\\)：单位冲激，\\(\\int_{-\\infty}^{\\infty}\\delta(t)\\,dt=1\\)（面积为 1，称强度）",
              "\\(t_0\\)：冲激所在时刻",
              "a：时间尺度因子，\\(a\\ne0\\)；尺度后必须除以 |a| 才能保持面积不变",
              "\\(u(t)\\)：单位阶跃，t<0 时为 0、t>0 时为 1",
              "\\(\\delta'(t)\\)：冲激偶，抽样性质带负号，作用在 x′(\\(t_0\\)) 上"
            ]
          },
          {
            ...
            detail: [
              "三种运算都作用在自变量上，作用对象是 t 而不是 x：\\(x(t-t_0)\\) 是右移 \\(t_0\\)（延迟），\\(x(t+t_0)\\) 是左移；\\(x(-t)\\) 关于纵轴翻转；\\(x(at)\\) 在 a>1 时把时间轴压缩 a 倍，波形变窄。组合时顺序会影响结果，记法上要分清 \\(x(at-b)\\) 与 \\(x(a(t-b))\\)：前者是先把 \\(x(t)\\) 右移 b 再做 a 倍压缩，后者是先压缩 a 倍再右移 b，两者相差 a 倍时移。",
              "判断准则很简单——用具体数值验算：若 \\(x(t)\\) 的某个特征点在 \\(t=1\\)，那么 \\(x(2t-2)\\) 中该特征点出现在 \\(2t-2=1\\) 即 \\(t=1.5\\) 处。",
              "以 \\(x(t)=t\\,[u(t)-u(t-1)]\\)（\\(0\\le t\\le 1\\) 上的斜升）为例：\\(x(2t)\\) 把区间压到 \\(0\\le t\\le 0.5\\)；\\(x(t-1)\\) 把区间挪到 \\(1\\le t\\le 2\\)；\\(x(2-t)\\) 先关于纵轴翻转，再右移 2，落到 \\(1\\le t\\le 2\\) 且方向反转。起点、终点、峰值三个特征点各验算一次，基本可以排除顺序错误。"
            ],
            points: [
              "平移改的是“什么时候开始”，翻转改的是“时间方向”，尺度改的是“时间快慢”",
              "\\(x(t-t_0)\\) 右移（延迟），\\(x(t+t_0)\\) 左移（超前）",
            ...
            variables: [
              "\\(x(t-t_0)\\)：右移 \\(t_0\\)（延迟）；\\(x(t+t_0)\\)：左移 \\(t_0\\)（超前）",
              "\\(x(-t)\\)：关于纵轴翻转，只改时间方向",
          ...
          },
          {
            ...
            detail: [
              ...
              "设 \\(x(t)\\) 在 \\(0\\le t\\le 1\\) 上为 1、其余时刻为 0，求 \\(x(2-t)\\)。目标式写成 \\(x(-(t-2))\\)，即“先翻转、再右移 2”：先翻转得 \\(x(-t)\\)，非零区间由 \\([0,1]\\) 变为 \\([-1,0]\\)；再把 \\(x(-t)\\) 整体右移 2（把 t 换成 t−2）得 \\(x(-(t-2))=x(2-t)\\)，区间随之移到 \\([1,2]\\)。",
              "交叉验证：直接解不等式 \\(0\\le 2-t\\le 1\\)，同样得到 \\(1\\le t\\le 2\\)。于是 \\(y(t)=x(2-t)\\) 在 \\(1\\le t\\le 2\\) 上为 1，其余为 0——原来的 \\([0,1]\\) 被镜像到 \\([1,2]\\)。注意顺序不能颠倒：若先右移 2 得 \\(x(t-2)\\)（支撑 \\([2,3]\\)），再关于纵轴翻转，得到的是 \\(x(-t-2)\\)（支撑 \\([-3,-2]\\)），并不是 \\(x(2-t)\\)。"
            ],
            points: [
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "前面几节都在处理单个信号，卷积第一次把两个信号合起来，定义是一个积分：\\[y(t)=\\int_{-\\infty}^{\\infty}x(\\tau)h(t-\\tau)\\,d\\tau.\\]式子本身不难，难在两件事——怎么算，以及算出来代表什么。课件用十页讲它：定义与交换律、求解步骤、两个矩形脉冲、方波与衰减指数，最后落到物理意义。",
            detail: [
              "图解五步（课件原文的顺序）：改变自变量 → 翻转 → 平移 → 相乘 → 积分，并注意积分限。具体做法是把 \\(x(t)\\)、\\(h(t)\\) 都改写成以 τ 为自变量的函数；把其中一个（例如 h）关于纵轴翻转成 \\(h(-\\tau)\\)；整体右移 t 得 \\(h(t-\\tau)\\)；与 \\(x(\\tau)\\) 相乘得到被积函数；最后对 τ 积分。",
              "积分上下限由两个信号“重叠”的 τ 区间决定，所以必须按 t 的取值分段讨论——t 落在不同区间时重叠区间端点不同，这正是分段结果的来源。卷积满足交换律 \\(x*h=h*x\\)，也满足结合律与分配律。",
              "两个宽度都是 1、高度都是 1 的矩形脉冲卷积：t<0 时无重叠，\\(y=0\\)；\\(0\\le t\\le 1\\) 时重叠长度为 t，\\(y=t\\)；\\(1<t\\le 2\\) 时重叠长度为 2−t，\\(y=2-t\\)；t>2 时为 0。结果是底宽 2、峰值 1 的三角形。峰值的一般规则是 \\(\\min(w_1,w_2)\\)——它由较窄的那个脉冲决定；而卷积曲线的总面积等于两个脉冲面积之积 \\(w_1w_2\\)（单位高度时就是 \\(w_1w_2\\)）。",
              "本例外形上两条数值都等于 1，纯属等宽等高的巧合，不能把“峰值 = 面积之积”当成规则：取 \\(w_1=1\\)、\\(w_2=3\\)，峰值仍是 1，而总面积是 3。再看冲激：\\(x(t)*\\delta(t-t_0)=x(t-t_0)\\)，与冲激卷积只是把信号平移到冲激所在的位置——这条恒等式是下一节“信号分解”的直接结果，也是冲激响应 \\(h(t)\\) 能描述整个系统的原因。"
            ],
            points: [
              "卷积定义 \\[y(t)=\\int_{-\\infty}^{\\infty}x(\\tau)h(t-\\tau)\\,d\\tau,\\]满足交换律、结合律、分配律",
              "图解五步：改变自变量 → 翻转 → 平移 → 相乘 → 积分，积分限由重叠区间决定",
              "结果必须按 t 分段讨论，分段点由两个信号定义域端点的组合决定",
              "\\(x(t)*\\delta(t-t_0)=x(t-t_0)\\)：与移位冲激卷积只是平移"
            ],
            pitfalls: [
              "把卷积当成“两个函数逐点相乘”：卷积要对所有 τ 加权叠加再积分，逐点相乘是频域那一侧的操作",
              "漏掉“翻转”这一步，把 \\(h(t-\\tau)\\) 直接写成 \\(h(\\tau-t)\\)，导致积分限取反、结果的支撑区间整体错位"
            ],
            links: [
          ...
          },
          {
            ...
            detail: [
              "第一步是冲激分解，它就是筛选性质的另一种写法：\\[x(t)=\\int_{-\\infty}^{\\infty}x(\\tau)\\delta(t-\\tau)\\,d\\tau.\\]把 t 看作观察时刻、τ 看作扫描变量，任意信号都被写成无穷多个移位冲激的加权和，权重恰好是 \\(x(\\tau)\\)。",
              "第二步用系统性质：设系统对 \\(\\delta(t)\\) 的响应是 \\(h(t)\\)，由时不变性，对 \\(\\delta(t-\\tau)\\) 的响应是 \\(h(t-\\tau)\\)；由线性（齐次性加可加性），权重 \\(x(\\tau)\\) 原样传递、各项直接叠加，于是 \\[y(t)=\\int_{-\\infty}^{\\infty}x(\\tau)h(t-\\tau)\\,d\\tau.\\]这就是卷积积分的来历。",
              "它同时说明了两件事：\\(h(t)\\) 能完整描述一个 LTI 系统；卷积不是人为约定的运算，而是“分解—求响应—叠加”三步的缩写。",
              "用这套逻辑复查两条已知结论：取 \\(x(t)=\\delta(t-t_0)\\)，分解式中只有 \\(\\tau =\\)\\(t_0\\) 处权重非零，输出就是 \\(h(t-t_0)\\)，与“冲激卷积等于平移”一致；取输入为阶跃 \\(u(t)\\)（即冲激的积分和），由线性得输出是 \\(h(t)\\) 的积分，这就是“阶跃响应” \\(g(t)=\\int_{-\\infty}^{t}h(\\tau)\\,d\\tau\\)（对因果系统积分从 0⁻ 起算）。",
              "注意区分两个方向：\\(g(t)\\) 是把“输入取成 \\(u(t)\\)”时系统的输出；而“冲激响应本身就是 \\(u(t)\\) 的系统”，它对任意输入 x 的输出才是 x 的滑动积分。所以只要知道 \\(h(t)\\)，任意输入下的输出都能算——这就是“冲激响应完全描述 LTI 系统”的含义。",
              "另一条分解思路是正交分解：把信号拆成互不干扰的分量，后面频域分析用的复指数就是一组正交基。"
            ],
            points: [
              "冲激分解 \\[x(t)=\\int_{-\\infty}^{\\infty}x(\\tau)\\delta(t-\\tau)\\,d\\tau\\] 是筛选性质的另一种写法",
              "卷积的来历：冲激分解 + 时不变（负责移位）+ 线性（负责加权叠加）",
              "\\(h(t)\\)、阶跃响应 \\(g(t)=\\int_{-\\infty}^{t}h(\\tau)\\,d\\tau\\) 与系统函数是同一件事的三种说法",
            ...
            formula: "\\begin{aligned}x(t)&=\\int_{-\\infty}^{\\infty}x(\\tau)\\,\\delta(t-\\tau)\\,d\\tau\\\\[6pt]y(t)&=\\int_{-\\infty}^{\\infty}x(\\tau)\\,h(t-\\tau)\\,d\\tau\\\\[6pt]g(t)&=\\int_{-\\infty}^{t}h(\\tau)\\,d\\tau\\end{aligned}",
            variables: [
              "\\(x(\\tau)\\)：分解系数，等于输入在 τ 时刻的取值",
              "\\(\\delta(t-\\tau)\\)：移位单位冲激（基函数）",
              "\\(h(t)\\)：系统对 \\(\\delta(t)\\) 的响应，即冲激响应",
              "\\(y(t)\\)：零状态响应",
              "\\(g(t)=\\int_{-\\infty}^{t}h(\\tau)\\,d\\tau\\)：阶跃响应（输入取 \\(u(t)\\) 时的输出）；因果系统积分从 0⁻ 起算"
            ]
          },
          {
            ...
            detail: [
              "性质表不是口诀，每条都对应一个可核对的量纲或符号：时移只改相位、不改幅值，因为延迟并不改变各频率成分的大小；频移把整个频谱搬运 \\(\\omega_0\\)，是调制与混频的数学形式；尺度变换 \\(x(at)\\) 同时压缩时宽、展宽带宽并乘 \\(1/|a|\\)，体现时宽与带宽的反比关系；时域微分乘 jω，说明高频成分对快变化更敏感。动手前先核对三件事：横轴用 f 还是 ω、指数符号约定是不是 \\(e^{-j\\omega t}\\)、以及 \\(1/(2\\pi)\\) 落在正变换还是反变换。",
            ...
            points: [
              ...
              "时域卷积↔频域相乘，时域相乘↔频域卷积（带 \\(1/(2\\pi)\\) 系数）",
              "Parseval：时域总能量等于频域总能量，是检查变换结果最省事的工具"
            ],
            pitfalls: [
              "把 f 与 ω 混用：两者相差 \\(2\\pi\\)，谱图横轴必须说清是哪一个",
              "漏掉 \\(1/(2\\pi)\\)：卷积定理与 Parseval 的常数取决于正变换的定义，混用两套约定必然出错",
            ...
            variables: ["\\(X(j\\omega)\\)：连续频谱", "\\(t_0\\)：时移量", "\\(\\omega_0\\)：频移量", "a：时间尺度因子", "帕塞瓦尔关系：时域能量与频域能量相等"]
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "绝对可积是傅里叶积分收敛的一个充分条件，不是必要条件：像 \\(e^{at}u(t)\\)（a>0）这类随 t 增长的信号确实没有普通意义下的频谱，但 \\(\\delta(t)\\)、\\(u(t)\\)、\\(\\cos\\omega_0t\\) 这些信号虽然不是绝对可积，仍可用广义函数（冲激谱）写出它们的傅里叶变换。含初始条件的电路方程则连广义函数也不便处理，于是引入拉普拉斯变换：在傅里叶核 \\(e^{-j\\omega t}\\) 上再乘一个实指数因子 \\(e^{-\\sigma t}\\)，把 \\(s=\\sigma+j\\omega\\) 当作复频率，只要 σ 取得足够大，增长信号也能被压住而收敛。代价是变换结果必须连同收敛域一起给出。",
            detail: [
              "收敛域 ROC 是使积分绝对收敛的 σ 范围，它是变换的一部分而不是附带说明：同一个代数式配上不同收敛域，对应完全不同的时间信号。例如 \\(\\frac{1}{s-a}\\) 在 ROC 为 \\(\\operatorname{Re}s>a\\)（极点右侧）时对应右边信号 \\(e^{at}u(t)\\)，在 \\(\\operatorname{Re}s<a\\)（极点左侧）时对应左边信号 \\(-e^{at}u(-t)\\)。",
              "能出现“左边信号”与两种 ROC，前提是这里用的是双边变换；单边变换（积分下限 0⁻）只覆盖 \\(t\\ge 0\\) 的部分，不能用来区分左边信号，它的主场是含初始条件的微分方程。极点给出 ROC 的边界，零极点图能读出因果性、稳定性与振荡频率。傅里叶变换只是 \\(s=j\\omega\\) 这条轴上的一刀切片——当且仅当 ROC 包含虚轴时这一刀才落在收敛域内。",
              "\\(x(t)=e^{-2t}u(t)\\) 的拉普拉斯变换是 \\(\\frac{1}{s+2}\\)，极点在 \\(s=-2\\)，ROC 为 \\(\\operatorname{Re}s>-2\\)。由于 ROC 包含虚轴（\\(\\sigma=0\\) 落在 −2 右侧），它的傅里叶变换存在，就是 \\(X(j\\omega)=\\frac{1}{j\\omega+2}\\)。",
              "把指数换成 \\(e^{2t}u(t)\\)，极点移到 \\(s=2\\)，ROC 变成 \\(\\operatorname{Re}s>2\\) 不再包含虚轴，傅里叶变换不存在——这正是“增长信号没有频谱”的复频域说法。反过来，左边信号也能有傅里叶变换：\\(x(t)=e^{3t}u(-t)\\) 的极点在 \\(s=3\\)，ROC 为 \\(\\operatorname{Re}s<3\\)，虽然极点在右半平面，但 \\(\\sigma=0\\)<3，虚轴仍在收敛域内，傅里叶变换存在且等于 \\(\\frac{1}{3-j\\omega}\\)。",
              "所以判据是“ROC 是否含虚轴”，不是“极点在不在左半平面”。"
            ],
            points: [
              "拉普拉斯变换＝傅里叶变换＋指数加权，\\(s=\\sigma+j\\omega\\) 把频率推广为复频率",
              "变换式与收敛域必须成对出现，缺一不可",
            ...
            pitfalls: [
              ...
              "把 \\(s=j\\omega\\) 无条件代入当作傅里叶变换：只有 ROC 包含虚轴时这一步才成立"
            ],
            links: [
            ...
            formula: "\\begin{aligned}X(s)&=\\int_{-\\infty}^{\\infty}x(t)e^{-st}\\,dt\\\\[6pt]s&=\\sigma+j\\omega,\\quad s\\in\\mathrm{ROC}\\end{aligned}",
            variables: [
              "s：复频率，\\(s=\\sigma+j\\omega\\)",
              "σ：指数加权因子（实部），决定 ROC",
              "ω：角频率（虚部）",
          ...
          },
          {
            ...
            detail: [
              "相关的定义是 \\[R_{xy}(\\tau)=\\int_{-\\infty}^{\\infty}x(t)y^*(t-\\tau)\\,dt:\\]把 y 平移 τ 后与 x 逐点相乘再积分，τ 取遍所有值就得到整条相关函数。它与卷积只差一步——卷积要把其中一个信号反折，相关不反折，所以 \\(R_{xy}(\\tau)\\) 就是 \\(x(\\tau)\\) 与 \\(y^*(-\\tau)\\) 的卷积，在频域对应 \\(R_{xy}\\leftrightarrow X(j\\omega)Y^*(j\\omega)\\)。",
              "相关定理把“找最像的时移”变成一次乘法加一次反变换，雷达测距、声呐定向、通信同步都在用它。换下标时要连共轭和符号一起换：\\(R_{yx}(\\tau)=R_{xy}^*(-\\tau)\\)；实信号才退化成 \\(R_{yx}(\\tau)=R_{xy}(-\\tau)\\)。归一化相关系数 \\(\\rho_{xy}(\\tau)\\) 用两个信号在零时移处的自相关值作分母，剔除幅值缩放的影响，由柯西—施瓦茨不等式保证 \\(|\\rho_{xy}(\\tau)|\\le1\\)，能量信号的自相关还满足 \\(|R_{xx}(\\tau)|\\le R_{xx}(0)\\)：等于 1 表示完全线性相关，等于 0 表示正交。",
              "\\(x(t)=\\sin(2\\pi t)\\) 与 \\(y(t)=\\sin(2\\pi(t-0.25))\\) 都是周期信号，能量无限，上面那个从 −∞ 积到 +∞ 的定义会发散，必须改用“一个周期内的时间平均”（功率相关）\\[R_{xy}(\\tau)=\\frac1T\\int_0^T x(t)y^*(t-\\tau)\\,dt.\\]代入本定义直接积分得 \\(R_{xy}(\\tau)=0.5\\cos[2\\pi(\\tau+0.25)]\\)：峰值出现在 \\(\\tau =-0.25+kT\\)（\\(T=1\\) s，k 为整数），即 \\(R_{xy}(-0.25)=+0.5\\)、\\(R_{xy}(+0.25)=-0.5\\)。",
              "这里要特别留意符号——按本定义，峰值所在的时移与 y 相对 x 的延迟符号相反：y 比 x 滞后 0.25 s，峰值却落在 \\(\\tau =-0.25\\) s；若改用 \\(R_{yx}(\\tau)\\) 定义，峰值才出现在 +0.25 s。所以报告时延之前，必须先说清用的是哪一个定义。峰值也不唯一：相关函数以 T 为周期重复，\\(\\tau =-0.25+kT\\) 全取同一个峰值，用相关测时延必须把搜索范围限制在一个周期以内。",
              "相关之所以能从噪声里捞出已知波形，靠的是“期望为零”而不是“数值处处为零”：设 \\(n(t)\\) 是零均值且与 \\(x(t)\\) 不相关的噪声，则它与 x 的互相关期望 \\(\\mathbb{E}[R_{xn}(\\tau)]\\) 在所有时延处都等于零——不只是 \\(\\tau \\ne 0\\) 处。用有限长数据估计时，样本互相关会留下残差；残差随记录长度下降，但不保证在任何时延上都接近零，也不随 τ 增大而系统变小。",
              "实际做法是把“接收信号＋噪声”与已知波形做互相关：信号分量在正确时延处形成一个高出残差的峰，记录越长峰越突出。"
            ],
            points: [
              "相关＝平移后相乘再积分，衡量两个信号在不同时移下的相似度",
              "相关与卷积只差“是否反折”：\\(R_{xy}(\\tau)\\) 是 x 与 \\(y^*(-\\tau)\\) 的卷积，频域上是 X 乘 Y 的共轭",
              "按 \\(R_{xy}(\\tau)\\)=平均[\\(x(t)\\)\\(y^*(t-\\tau)\\)] 定义，峰值时移与“y 相对 x 的延迟”符号相反（滞后 0.25 s ⇒ 峰值在 \\(\\tau =-0.25\\) s）",
              "换下标要连共轭和符号一起换：\\(R_{yx}(\\tau)=R_{xy}^*(-\\tau)\\)（实信号才写成 \\(R_{xy}(-\\tau)\\)）",
              "归一化相关系数满足 \\(|\\rho |\\le 1\\)：1 为完全相关，0 为正交",
              "周期信号能量无限，要用时间平均的功率相关；峰值按周期重复，测时延须先限定在一个周期内"
            ],
            pitfalls: [
              "把相关的 τ 符号记反：\\(R_{xy}(\\tau)\\)=平均[\\(x(t)\\)\\(y^*(t-\\tau)\\)] 的峰值出现在“延迟量”的相反数处（延迟 \\(+0.25 s \\to\\) 峰值在 \\(\\tau =-0.25\\) s）；换下标时复信号是 \\(R_{yx}(\\tau)=R_{xy}^*(-\\tau)\\)，不能写成 \\(R_{xy}(-\\tau)\\)",
          ...
          },
          {
            ...
            detail: [
              "吉布斯现象是一种“不一致收敛”现象：部分和在跳变点附近的最大偏差不随项数消失，而这并不否定满足狄利克雷条件时级数在每一点的逐点收敛——逐点收敛仍然成立，只是收敛在跳变附近不一致。对 ±1 方波（跳变量 2），以最高奇次谐波 N 的部分和测量，过冲随 N 增大趋于跳变量的约 8.95%；有限项的过冲必须具体计算，不能一概断言大于或小于该极限（本例 \\(N=1\\)、3、5 分别约为 13.66%、10.02%、9.42%，都高于渐近值）。",
              "这也解释了理想低通滤波器的阶跃响应为什么会有振铃，以及实际滤波器为什么要在过渡带陡峭程度与过冲之间做取舍。",
              "复算路径：用符号计算核对变换对与性质，用数值卷积核对分段结果。核对前先固定尺度约定（横轴用 f 还是 ω、\\(1/(2\\pi)\\) 放在哪一侧），再逐项比对。工具给出的曲线只能作旁证，解析分段式仍是判据——数值误差会掩盖符号与常数错误。"
            ],
            points: [
              "狄利克雷条件是傅里叶级数收敛的充分条件，不满足时逐点收敛可能失效",
              "吉布斯过冲的渐近极限约为跳变幅度的 8.95%（\\(N\\to \\infty\\)，N 为最高奇次谐波）；有限项的值需具体计算，不能一概断言小于该极限",
        ...
        examples: [
          {
            ...
            prompt: "\\(x(t)\\)\\(=\\cos (\\)\\(2\\pi\\)\\(t)+\\cos (3t)\\)（t 以秒计），判断它是否为周期信号；若是，求基波周期。",
            steps: [
              "\\(\\cos (\\)\\(2\\pi t\\)) 的角频率 \\(\\omega _1=\\)\\(2\\pi\\)，周期 \\(T_1=\\)\\(2\\pi\\)\\(/\\omega _1=1\\) s。",
              "\\(\\cos (3t)\\) 的角频率 \\(\\omega _2=3\\)，周期 \\(T_2=\\)\\(2\\pi /3\\) s。",
              "两周期之比 \\(T_1/T_2=3/(\\)\\(2\\pi\\)) 是无理数，不存在能同时容纳整数个 T₁ 与 T₂ 的公共周期。"
            ],
            answer: "不是周期信号。两个分量的周期之比为无理数时，叠加结果不再周期；若换成 \\(\\cos (4\\pi t)\\)（\\(T=0.5\\) s），比值 2:1 为有理数，合成周期就是 1 s。"
          },
          {
            title: "含跳变信号的导数",
            prompt: "\\(x(t)\\)\\(=2\\)\\(u(t)\\)\\(-3u(t-2)\\)，求 dx/dt。",
            steps: [
              "先看波形：t<0 时 \\(x=0\\)；\\(0\\le t<2\\) 时 \\(x=2\\)；\\(t\\ge 2\\) 时 \\(x=2-3=-1\\)。",
              "\\(t=0\\) 处向上跳变 2，对应强度 +2 的冲激。",
              "\\(t=2\\) 处从 2 跳到 −1，向下跳变 3，对应强度 −3 的冲激。"
            ],
            answer: "\\(dx/dt=2\\)\\(\\delta(t)-3\\delta\\)(t−2)。平坦段导数为 0，只有跳变点留下冲激，强度等于跳变幅度、符号由跳变方向决定。"
          },
          {
          ...
          {
            title: "矩形脉冲的镜像变换",
            prompt: "\\(x(t)\\)\\(=1\\)（\\(0\\le t\\le 1\\)），其他时刻为 0。求 \\(y(t)=x(2-t)\\) 的非零区间。",
            steps: [
              "把目标式写成标准形：\\(2-t=-(t-2)\\)，即“先翻转、再右移 2”。",
              "先翻转：\\(x(-t)\\) 的非零区间由 \\([0,1]\\) 变到 \\([-1,0]\\)。",
              "再右移 2（t 换成 t−2）：区间移到 \\([1,2]\\)，即 \\(x(-(t-2))=x(2-t)\\)。",
              "交叉验证：直接解 \\(0\\le 2-t\\le 1\\)，同样得到 \\(1\\le t\\le 2\\)。"
            ],
            answer: "\\(y(t)\\)\\(=1\\) 当 \\(1\\le t\\le 2\\)，其他时刻为 0——原来的 \\([0,1]\\) 被镜像到 \\([1,2]\\)，区间长度不变而位置对称。反过来先右移再翻转只会得到 \\(x(-t-2)\\)（支撑 \\([-3,-2]\\)），是错的。"
          },
          {
            title: "与冲激、阶跃卷积分别得到什么",
            prompt: "设 \\(y(t)=x(t)*h(t)\\)。当 \\(h(t)\\)\\(=\\delta (t-1)\\) 与 \\(h(t)=u(t)\\) 时，\\(y(t)\\) 分别是什么？",
            steps: [
              "与移位冲激卷积：\\(y(t)=x(t)\\)\\(*\\delta (t-1)=\\)\\(x(t-1)\\)，只是把输入整体右移 1。",
              "与阶跃卷积：\\[y(t)=\\int_{-\\infty}^{\\infty}x(\\tau)u(t-\\tau)\\,d\\tau,\\]而 \\(u(t-\\tau )\\) 在 \\(\\tau \\le t\\) 时为 1、τ>t 时为 0，于是积分上限收缩到 t。",
              "所以 \\(y(t)=\\int_{-\\infty}^{t}x(\\tau)\\,d\\tau\\)，即输入到当前时刻为止的累积（滑动积分）。"
            ],
            answer: "\\(h=\\delta (t-1)\\) 时 \\(y(t)=x(t-1)\\)；\\(h=\\)\\(u(t)\\) 时 \\(y(t)=\\int_{-\\infty}^{t}x(\\tau)\\,d\\tau\\)，是输入 x 的滑动积分。注意这只是“冲激响应为 \\(u(t)\\) 的系统对任意输入 x 的输出”，不要把 ∫x 叫做阶跃响应——该系统的阶跃响应是输入取 \\(u(t)\\) 时的输出 \\((u*u)(t)=t\\,u(t)\\)。"
          },
          {
            title: "由冲激响应求阶跃响应",
            prompt: "某 LTI 系统的冲激响应 \\(h(t)=e^{-2t}u(t)\\)。用分解与线性求它的阶跃响应 \\(g(t)\\)。",
            steps: [
              "把阶跃写成冲激的积分：\\[u(t)=\\int_{-\\infty}^{t}\\delta(\\tau)\\,d\\tau.\\]",
              "积分是线性运算，由线性与时不变性，输出等于 h 的同样积分。",
              "\\[\\begin{aligned}g(t)&=\\int_0^t e^{-2\\tau}\\,d\\tau\\\\[4pt]&=\\frac{1-e^{-2t}}2\\end{aligned},\\]\\(t\\ge 0\\)（t<0 时积分为 0）。"
            ],
            answer: "\\(g(t)=\\frac{1-e^{-2t}}2\\,u(t)\\)。\\(t\\to \\infty\\) 时终值 1/2，正好等于 \\(h(t)\\) 曲线下的总面积。"
          },
          {
            title: "两个单位矩形脉冲的卷积",
            prompt: "令 \\(x(t)=h(t)\\)\\(=1\\)（\\(0\\le t\\le 1\\)），其他时刻为0，求 \\(y(t)=x(t)*h(t)\\)。",
            steps: [
              "卷积值等于区间 \\([0,1]\\) 与 \\([t-1,t]\\) 的重叠长度。",
              "\\(0\\le t\\le 1\\) 时重叠长度随 t 增加，\\(y(t)\\)\\(=t\\)。",
              "\\(1<t\\le 2\\) 时重叠长度随 t 减少，\\(y(t)\\)\\(=2-t\\)；其他时刻无重叠。"
            ],
            answer: "\\(y(t)\\)\\(=0\\)（t<0）；t（\\(0\\le t\\le 1\\)）；2−t（\\(1<t\\le 2\\)）；0（t>2），波形为峰值1的三角脉冲。"
          },
          {
            title: "时移性质写出频谱",
            prompt: "已知 \\(x(t)\\)↔\\(X(j\\omega)\\)，不用重新积分，写出 \\(x(t-2)\\) 的傅里叶变换。",
            steps: [
              "识别这是时域右移 \\(t_0\\)\\(=2\\)，不改变频谱幅值包络。",
              "套用时移性质 \\(x(t-t_0)\\longleftrightarrow e^{-j\\omega t_0}X(j\\omega)\\)。",
              "检查指数符号：右移对应负相位斜率。"
            ],
            answer: "\\(x(t-2)\\longleftrightarrow e^{-j2\\omega}X(j\\omega)\\)。"
          },
          {
            ...
            steps: [
              ...
              "积分得到 \\(E=1/(2a)\\)。",
              "Parseval 要求 \\(\\frac1{2\\pi}\\int_{-\\infty}^{\\infty}|X(j\\omega)|^2\\,d\\omega\\) 与该结果相同。"
            ],
          ...
          },
          {
            ...
            steps: [
              "只在 t<0 上积分：\\(X(s)=\\int_{-\\infty}^0 e^{3t}e^{-st}\\,dt\\)，把两个指数合并得 \\(\\int_{-\\infty}^0 e^{(3-s)t}\\,dt\\)。",
              "积分收敛要求 \\(\\operatorname{Re} (3-s)>0\\)，即 \\(\\operatorname{Re}s<3\\)；在此收敛域下 \\(X(s)\\)=\\(\\frac{1}{3-s}=-\\frac{1}{s-3}\\)，极点在 \\(s=3\\)，收敛域位于极点左侧。",
              "收敛域 \\(\\operatorname{Re}s<3\\) 包含虚轴（\\(\\sigma=0\\)<3），所以 \\(s=j\\omega\\) 可以代入：傅里叶变换存在。"
            ],
            answer: "\\(X(s)=\\frac{1}{3-s}\\)，收敛域 \\(\\operatorname{Re}s<3\\)，包含虚轴，因此傅里叶变换存在：\\(X(j\\omega)=\\frac{1}{3-j\\omega}\\)。这里 \\(X(s)=-\\frac{1}{s-3}\\) 与左边信号 \\(e^{3t}u(-t)\\) 对应；信号在 \\(t\\to -\\infty\\) 时按 \\(e^{3t}\\) 衰减到 0，是有界的。"
          }
        ],
        experiments: [
          {
            id: "signals-ch1-waveform-transform",
            title: "新时刻取到了哪个旧时刻的值",
            workbench: "notebook",
            goal: "把 \\(x(at+b)\\) 读成「新时刻 t 去旧波形上取 \\(x(at+b)\\) 这个值」，分清压缩拉伸（看 |a|）、反折（看 a 的符号）与平移（看 \\(t_0=-b/a\\)）三件事。",
            steps: [
              "先预测：\\(x(2t-4)\\) 相对原波形是延迟 2 秒还是延迟 4 秒？写下来再动手。",
              "保持 \\(a=1\\)、\\(b=0\\)，确认上下两个坐标系里的波形逐点相同，建立「上轴看旧时刻 τ、下轴看新时刻 t」的对应。",
              "反例对照一：取 \\(a=2\\)、\\(b=-4\\)，读出支撑区间 \\([2, 3.5]\\) 与压缩后的宽度 \\(3/|a|=1.5\\)，说明「压缩」是横向变窄而不是幅度变小。",
              "反例对照二：取 \\(a=-1\\)、\\(b=0\\)，读出支撑 \\([-3, 0]\\) 与反折状态；再把 b 调回 4，观察 \\(t_0=-b/a\\) 的变化，说明反折由 a 的符号决定、平移由 b 与 a 共同决定。",
              "用自己的话解释：为什么 \\(x(2t-4)\\) 是延迟 2 秒而不是 4 秒？"
            ],
            expected: "\\(a=2,b=-4\\) 时支撑为 \\([2, 3.5]\\)、\\(3/|a|=1.5\\)、\\(t_0=2\\) s；\\(a=-1,b=0\\) 时支撑为 \\([-3, 0]\\) 且状态显示反折；\\(a=1,b=0\\) 时上下波形坐标值逐点一致；四个特征点 (0,0)(1,2)(2,2)(3,0) 按 \\(t=(\\tau -b)/a\\) 映射后代回误差 \\(\\le 1e-10\\)。",
            limitation: "首版只用这一个梯形波形，不含指数与门函数预设；\\(a=0\\) 会使映射退化，因此被拒绝并保留上一个有效值。"
          },
          {
            id: "signals-ch1-convolution",
            title: "连续卷积与图解五步动态演示",
            workbench: "notebook",
            goal: "把卷积的图解五步（改元、翻转、平移、相乘、积分）变成可操作的动态过程：先预测再动手，用重叠面积解释输出，并说明峰值与总面积为什么是两个不同的量。",
            steps: [
              "先预测：把观察时刻从 \\(t=-1\\) 移到 \\(t=w_1\\)，重叠面积会怎样变化？写下预测再动手。",
              "保持 \\(w_1=w_2=1\\)，把 t 从 −1 拖到 2，对照上图翻转平移的 \\(h(t-\\tau )\\) 与下图 \\(y(t)\\) 的游标位置。",
              "反例对照一：切到预设 (1,2)，分别记录峰值（\\(=\\mathrm{min} (w_1,w_2)\\)）与总面积（\\(=w_1\\cdot w_2\\)），说明二者为何不等。",
              "反例对照二：点击上图一个历史位置，核对它已历经的时间 t−τ 与 \\(h(t-\\tau )\\) 的取值，再解释“翻转”并不是为了让响应衰减。",
              "用自己的话解释：固定 t 观察历史变量 τ 时，为什么会出现翻转？"
            ],
            expected: "\\(t=-1\\) 时重叠面积为 0；(1,1) 在 \\(t=1\\) 取峰值 1，(1,2) 在 \\(t∈[1,2]\\) 走过平台 1；数值积分（\\(d\\tau =0.005\\) 复合梯形）与解析分段式绝对误差 < 0.01。",
            limitation: "以因果 LTI 零状态响应为前提；脉冲响应不必然衰减，“翻转”只是卷积定义的结果，不是为匹配衰减。"
          },
          {
            id: "signals-ch1-fourier-synthesis",
            title: "谐波逼近和谱线变密是两回事",
            workbench: "notebook",
            goal: "把「增加谐波阶数 N」与「加长周期 T」分开看：前者让部分和逼近波形，后者让谱线变密但保持同一条连续包络。",
            steps: [
              "先预测：把 T 从 2 增到 8（周期变长），\\(x_N(t)\\) 会更快接近方波吗？谱线会变多还是变密？写下来再动手。",
              "取默认 \\(T=2\\)、\\(N=5\\)，观察部分和与目标方波的差距，并记录指标卡的谱点数与 RMSE。",
              "反例对照一：只把 N 从 5 加到 101（T 不变），谱点数仍是 17 个，但 RMSE 明显下降——逼近变好靠的是增加谐波，不是增加谱线。",
              "反例对照二：只把 T 从 2 增到 8，谱点从 17 个变成 65 个、明显变密，但 \\(T\\cdot c_k\\) 仍落在同一条包络 \\(\\operatorname{sinc} (f)\\) 上——谱线变密没有改变各次谐波的相对权重。",
              "用自己的话解释：为什么 \\(c_k\\) 随 T 增大而减小，而 \\(T\\cdot c_k\\) 却始终落在同一条固定包络上？"
            ],
            expected: "\\(T=2\\) 时 \\(|k/T|\\le 4\\) 有 17 个谱点、\\(T=8\\) 时有 65 个（含零系数）；所有谱点落在包络 \\(\\tau \\cdot \\operatorname{sinc} (\\tau f)\\) 上误差 \\(\\le 1e-10\\)；\\(T=4,k=2\\) 时 \\(c_2=1/(2\\pi )\\)；\\(T=4,N=101\\) 时 \\(|x_N(0)-1|\\le 0.02\\)；\\(T=2,N=101\\) 跳变内侧过冲落在 8.8%–9.2%；MSE 按一个周期 4096 个中点等权计算。",
            limitation: "只取单位高度、中心对称、脉宽 \\(\\tau =1\\) 的周期矩形列；渐近参考线 1.08949 只是极限值，不声称任意有限 N 的峰值都不超过它。"
          }
        ],
        check: [
          {
          ...
          },
          {
            ...
            prompt: "\\(x(t)\\)乘以 \\(e^(j\\omega _0t)\\) 后，频谱怎样变化？",
            options: ["整体向右平移 ω₀", "整体向左平移 ω₀", "幅值全部加倍", "变成时域卷积"],
            answer: 0,
            explanation: "频移性质为 \\(x(t)e^(j\\omega _0t)\\)↔\\(X(j(\\omega -\\omega _0))\\)。"
          },
          {
            ...
            answer: 1,
            explanation: "由柯西—施瓦茨不等式，\\(|\\rho xy(\\tau )|\\le 1\\)。"
          },
          {
          ...
          },
          {
            ...
            answer: 0,
            explanation: "卷积含反折步骤，相关不含；这正是 \\(R_{xy}(\\tau )=x(\\tau )*y*(-\\tau )\\) 中多出一个共轭反折的原因。"
          },
          {
            id: "signals-ch1-check-8",
            prompt: "正弦信号 \\(\\cos (2\\pi \\cdot 50t\\)) 的频率与角频率分别是：",
            options: ["50 Hz 与 100π rad/s", "100π Hz 与 50 rad/s", "50 Hz 与 50 rad/s", "两者数值相同"],
            answer: 0,
            explanation: "一般式 \\(\\cos (\\omega t)\\) 中 \\(\\omega =2\\pi\\)\\(\\cdot 50=100\\pi\\) rad/s，对应频率 \\(f=\\omega /2\\pi =50\\) Hz。"
          },
          {
            ...
            answer: 2,
            explanation: "\\(\\delta (at)=\\delta (t)/|a|\\)：时间轴压缩 2 倍后，为保持面积为 1，冲激强度必须减半。"
          },
          {
          ...
          },
          {
            ...
            prompt: "若 \\(x(t)\\) 只在 \\(0\\le t\\le 1\\) 上非零，则 \\(x(2-t)\\) 的非零区间是：",
            options: ["\\(0\\le t\\le 1\\)", "\\(1\\le t\\le 2\\)", "\\(-1\\le t\\le 0\\)", "\\(2\\le t\\le 3\\)"],
            answer: 1,
            explanation: "令 \\(0\\le 2-t\\le 1\\) 直接解得 \\(1\\le t\\le 2\\)。等价的操作顺序是先翻转得 \\([-1,0]\\)，再右移 2；若先右移 2 得 \\([2,3]\\) 再翻转，得到的是 \\(x(-t-2)\\)，支撑 \\([-3,-2]\\)。"
          },
          {
            id: "signals-ch1-check-12",
            prompt: "\\(x(t)*\\delta (t-t_0)\\) 的结果是：",
            options: ["\\(x(t)\\)", "\\(x(t-t_0)\\)", "\\(x(t_0)\\)", "\\(x(t)\\cdot \\delta\\)(t−t₀)"],
            answer: 1,
            explanation: "与移位冲激卷积只把信号平移到 t₀ 处，这也是“冲激响应能描述整个系统”的起点。"
          },
          {
            id: "signals-ch1-check-13",
            prompt: "卷积积分 \\(y(t)=\\int x(\\tau )h(t-\\tau )d\\tau\\) 成立，依赖系统的哪两条性质？",
            options: ["线性与时不变", "因果与稳定", "无记忆与可逆", "周期性与对称性"],
            answer: 0,
            explanation: "移位靠时不变（\\(\\delta (t-\\tau )\\to h(t-\\tau )\\)），加权叠加靠线性；缺任何一条都不能写成卷积。"
          }
        ],
        summary: [
      ...
      },
      {
        ...
        sections: [
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "上一章把连续信号放在时域、频域和复频域里看了一遍，这一节开始把它送进数字世界。采样的动作很简单：以等间隔 T 在时刻 nT 上取 \\(x_c(t)\\) 的值，得到序列 \\(x[n]=x_c(nT)\\)。但它对频谱做了什么并不直观——采样不是近似取点，而是在频域把原频谱以采样角频率 \\(\\Omega_s=2\\pi/T\\) 为周期整份复制。理解这一步，后面采样定理、混叠与重建才有落点。",
            detail: [
              "把采样写成连续信号与冲激串相乘最容易看清频域后果：\\[p(t)=\\sum_{n=-\\infty}^{\\infty}\\delta(t-nT),\\]\\(x_s(t)=x_c(t)\\,p(t)\\)。时域相乘对应频域卷积，而冲激串的频谱仍是间隔 \\(\\Omega_s\\) 的冲激串，于是 \\[X_s(j\\Omega)=\\frac1T\\sum_{k=-\\infty}^{\\infty}X_c\\!\\left(j(\\Omega-k\\Omega_s)\\right)\\]——原频谱被搬移到每个 \\(k\\Omega_s\\) 上叠加，形成周期延拓。注意每一份副本的幅度被乘了 1/T，量纲从谱密度变成离散样点的谱。",
              "恢复是采样的逆过程：只要各份副本互不重叠且都在折叠频率 \\(\\Omega_s/2\\) 之内，用一个理想低通（截止 \\(\\Omega_s/2\\)、增益 T）就能截出原来那一份。理想低通对应的时域冲激响应是 sinc，所以重建就是给每个样点配一个以它为中心的 sinc 并叠加。工程上用零阶保持（DAC 的实际行为）代替理想插值：它简单可实现，但等效于给频谱乘上 sinc 包络，高频被压低，因此实际电路后面还要补一级重建滤波。"
            ],
            points: [
              "采样 = 连续信号与冲激串相乘，样点值就是 \\(x[n]=x_c(nT)\\)",
              "频域后果是频谱以 \\(\\Omega_s=2\\pi/T\\) 为周期整份复制，每份幅度乘 1/T",
              "恢复 = 用理想低通截出 \\(\\Omega_s/2\\) 以内的一份；时域等价于 sinc 插值",
            ...
            variables: [
              "T：采样间隔（s），\\(\\Omega_s=2\\pi/T\\) 为采样角频率",
              "p(t)：冲激串，间距为 T",
              "\\(x_c(t)\\)：被采样的连续信号；\\(x_s(t)\\) 为采样后的冲激串信号",
              "\\(X_c(j\\Omega)\\)：原连续频谱；\\(X_s(j\\Omega)\\) 为采样后频谱（周期延拓）"
            ]
          },
          {
            ...
            detail: [
              "设 \\(x_c(t)\\) 带限于最高频率 \\(f_{\\max}\\)（即 |f|>\\(f_{\\max}\\) 时频谱为零）。周期延拓后相邻副本的间隔是 \\(f_s=1/T\\)，不重叠要求 \\(f_s>2f_{\\max}\\)；\\(f_s/2\\) 称为折叠频率。唯一恢复的条件要写成严格不等式：等号 \\(f_s=2f_{\\max}\\) 并不够——取 \\(x_c(t)\\)=\\(\\sin(2\\pi f_{\\max}t)\\)，在 \\(t=nT=n/(2\\)\\(f_{\\max}\\)) 处样点为 \\(\\sin (\\pi n)\\)≡0，与“全零信号”给出完全相同的样点，仅凭样点无法判断原信号是哪一个。",
              "要恢复，还要排除这类恰好落在折叠频率上的端点分量（等价于要求信号能量集中在 |f|<\\(f_s/2\\) 之内）。",
              "一旦 \\(f_s<2f_{\\max}\\)，超过折叠频率的分量不会消失，而是折叠到低频：\\(f_s\\)\\(=100\\) Hz 采样时，99 Hz、101 Hz 与 1 Hz 的余弦都给出同一组样点；换成正弦则 99 Hz 与 101 Hz 的样点符号相反（分别等价于 −1 Hz 与 +1 Hz）——折叠保留的是频率的绝对值，相位会随落在第几条折叠带而改变。",
              "混叠是确定性折叠而不是随机噪声，换一个采样率，折叠后的位置会跟着变——这既是判别混叠的方法，也说明它一旦发生就无法由该组样点挽回。工程做法固定为三步：采样之前用模拟抗混叠低通把 \\(f_{\\max}\\) 压到 \\(f_s/2\\) 以内、采样率给滤波器留过渡带余量、采样之后不再指望数字滤波补救。"
            ],
            points: [
              "唯一恢复的条件是严格不等式 \\(f_s>2f_{\\max}\\)；等号处存在 \\(\\sin(2\\pi f_{\\max}t)\\) 这类反例（样点全零）",
              "\\(f_s/2\\) 是折叠频率；超过它的分量会折叠到低频，保留的是频率绝对值",
              "折叠改变相位：\\(f_s\\)\\(=100\\) Hz 时 99 Hz 与 101 Hz 的余弦样点相同，正弦样点符号相反",
              "抗混叠滤波器必须在采样器之前，采样率要给它的过渡带留余量"
            ],
            pitfalls: [
              "把 \\(f_s\\ge2f_{\\max}\\) 当成唯一恢复的充分条件：等号处端点频谱信号会失效，必须排除该情形",
              "只按 \\(2f_{\\max}\\) 机械取采样率，不给抗混叠滤波器的过渡带留余量"
            ],
            links: [
            ...
            variables: [
              "\\(f_s\\)：采样频率（Hz），等于 1/T",
              "\\(f_{\\max}\\)：信号的最高有效频率（带宽上限）",
              "\\(f_s/2\\)：折叠频率（奈奎斯特频率）",
              "\\(2f_{\\max}\\)：奈奎斯特率；等号不保证唯一恢复，须排除折叠频率上的端点分量"
            ]
          },
          {
            ...
            detail: [
              "对长度为 N 的有限长序列 \\(x[n]\\)（\\(n=0\\)…,N−1），把它的 DTFT \\(X(e^{j\\omega})\\) 在 \\(\\omega=2\\pi k/N\\) 上等间隔取样，得到 N 个值 \\(X[k]\\)。频域采样定理指出：这 N 个样点可以唯一恢复原序列，方式是 \\[x[n]=\\frac1N\\sum_{k=0}^{N-1}X[k]e^{j2\\pi kn/N}.\\]道理与上一节对偶：频域取样使时域周期延拓，\\(x_N[n]=\\sum_{r=-\\infty}^{\\infty}x[n+rN]\\)，原序列如果正好落在一个周期以内，延拓时就不会相互重叠。",
              "注意唯一恢复是有前提的——必须已知序列长度 N 与它的支撑位置，否则同一组频域样点对应的是那条周期求和序列，而不是任意一个长度 N 的片段。",
              "序列长度超过 N 却硬用 N 个频点表示时，得到的是周期求和 \\(x_N[n]\\) 的样点：时域上首尾相互叠加，超出周期的部分折回到开头。这与“截断”不是一回事——\\(fft(x,N)\\) 在 N 小于数据长度时直接丢掉尾部，结果只保留前 N 个样点；而频域取样对应的是循环叠加。两者数值一般不同（例如 \\([1,2,3,4]\\) 取 2 点频域样本对应 \\([1+3, 2+4]=[4,6]\\)，截断则得到 \\([1,2]\\)）。",
              "这组“时域长度 N ↔ 频域样点数 N”的对应关系，正是下一节离散傅里叶变换的定义来源。"
            ],
            points: [
              "频域采样定理是时域采样定理的对偶：频域取样 ⇒ 时域周期求和 \\(x_N[n]=\\sum_{r=-\\infty}^{\\infty}x[n+rN]\\)",
            ...
            variables: [
              "N：序列长度，也是频域取样点数",
              "\\(X(e^{j\\omega})\\)：序列的 DTFT（连续、以 \\(2\\pi\\) 为周期）",
              "X[k]：第 k 个频域样点，取样间隔 \\(2\\pi /N\\)",
              "n：时域序号，取 0,…,N−1"
            ]
          },
          {
            ...
            detail: [
              "基本序列先备齐：单位样值 \\(\\delta[n]\\)（\\(n=0\\) 时为 1，其余为 0）、单位阶跃 \\(u[n]\\)、实指数 \\(a^n\\)、复指数 \\(e^{j\\omega_0 n}\\)、正弦与矩形序列。与连续情形最大的不同在于 \\(\\delta[n]\\) 是普通函数：它有确定的值 1，不是极限意义上的冲激，但同样具备筛选性质 \\(\\sum_n x[n]\\delta[n-k]=x[k]\\)，而这条性质正是卷积和的出发点。",
              "周期性要单独判断。连续正弦 \\(\\cos (\\omega t)\\) 对任何 ω 都周期，而离散复指数 \\(e^{j\\omega_0 n}\\) 只有满足 \\(\\omega_0\\)\\(N=\\)\\(2\\pi k\\)（N、k 为整数）时才周期，等价说法是 \\(\\omega_0/(2\\pi)\\)\\(=k/N\\) 必须是有理数。例如 \\(\\cos (2n)\\) 的 \\(\\omega_0=2\\)，2/(\\(2\\pi\\)\\()=1/\\pi\\) 是无理数，它不是周期序列。",
              "另两条常用指标：能量 \\(E=\\sum_n |x[n]|^2\\) 有限则为能量序列；功率用长时间平均定义，\\[P=\\lim_{K\\to\\infty}\\frac1{2K+1}\\sum_{n=-K}^{K}|x[n]|^2,\\]当该极限存在且有限（非零）时称功率序列。对周期序列，这个长时间平均退化为一个周期内的平均 \\(\\frac1N\\sum_{n=0}^{N-1}|x[n]|^2\\)，所以周期平均只是周期序列的简化算法，不是功率的一般定义。"
            ],
            points: [
              "离散序列只在整数序号上有定义，记为 \\(x[n]\\)",
              "单位样值 \\(\\delta[n]\\) 是普通函数（\\(\\delta [0]=1\\)），并有筛选性质 \\(\\sum_n x[n]\\delta[n-k]=x[k]\\)",
              "\\(e^{j\\omega_0 n}\\) 周期 ⟺ \\(\\omega_0/(2\\pi)\\) 为有理数；\\(\\cos (2n)\\) 不是周期序列",
              "周期序列属功率序列；能量有限的序列属能量序列"
            ],
            pitfalls: [
              "照搬连续结论，认为任何离散正弦都是周期的：必须检验 \\(\\omega_0/(2\\pi)\\) 是否为有理数",
              "把 \\(\\delta[n]\\) 当成连续冲激 \\(\\delta(t)\\)：它没有“面积为 1”的说法，就是取值为 1 的一个样点"
            ],
            links: [
            ...
            variables: [
              ...
              "\\(\\delta[n]\\)：单位样值序列",
              "\\(u[n]\\)：单位阶跃序列（n≥0 时为 1）",
              "\\(e^{j\\omega_0 n}\\)：复指数序列，\\(\\omega_0\\) 为数字角频率（rad）"
            ]
          },
          {
            ...
            detail: [
              "逐点运算与连续一致：移位 \\(x[n-n_0]\\) 右移 n₀、反折 \\(x[-n]\\)、相加与相乘按序号逐点做。反折只是把序号取负，样点一个不少、信息不丢，是一一对应的可逆运算——它不属于抽取。真正的抽取是整数倍 M>1 的下采样 \\(y[n]\\)\\(=x[Mn]\\)：只保留每 M 个样点中的一个，对任意序列一般不可逆。",
              "它可逆的例外情形是序列事先已被限带到 |ω|<π/M：此时抽取后的频谱副本不重叠，可以用插值重建原序列。抽取之前必须先抗混叠（把带宽压到新的折叠频率以内），否则照样混叠。",
              "上采样（插零）是另一件事：\\(x_u[n]=x[n/L]\\)（n 为 L 的整数倍）而其余位置取 0——它只是把样点拉开、中间补零，频谱被压缩到 |ω|<π/L 并产生镜像，因此插零之后还要接一个低通插值滤波器，才得到真正意义上的“插值”。差分与累加是微积分的离散对应：一阶后向差分 \\(x[n]\\)\\(-x[n-1]\\) 对应微分，累加 \\(\\sum_{k=-\\infty}^n x[k]\\) 对应积分，两者互为逆运算（差去常数）。",
              "卷积和把两个序列合起来：\\(y[n]=\\sum_k x[k]h[n-k]\\)，步骤与连续卷积相同（改元、翻转、平移、相乘、求和），区别主要是求和代替积分；结果同样可以按 n 的区间分段写出，只是分段点是整数而不是连续区间。它满足交换律、结合律与分配律，并满足 \\(x[n]*\\delta[n-n_0]=x[n-n_0]\\)。"
            ],
            points: [
          ...
          },
          {
            ...
            detail: [
              "设 \\(x[n]\\) 以 N 为周期，DFS 把它写成 N 个复指数之和：\\(x[n]=\\sum_{k=0}^{N-1}a_k e^{j2\\pi kn/N}\\)，系数 \\[a_k=\\frac1N\\sum_{n=0}^{N-1}x[n]e^{-j2\\pi kn/N}.\\]两式的求和都只有 N 项，因为 \\(e^{j2\\pi kn/N}\\) 对 k 以 N 为周期——k 和 k+N 给出同一个基函数。于是“频谱”只在 N 个频点上取值，而且这 N 个值自身也是以 N 为周期的。",
              "与连续 FS 对照记忆：连续的谐波频率 kω₀ 无限延伸、系数随 k 衰减；离散的谐波只有 N 个、系数以 N 为周期重复。原因还是采样——频域周期化之后，一个周期以外的频谱只是重复，没有新信息。实序列还有共轭对称 \\(a_k=a_{-k}^*\\)，因此幅度谱关于 \\(k=N/2\\) 对称，实际只需一半数据。"
            ],
            points: [
              "以 N 为周期的序列，DFS 只用 N 个谐波即可完全表示",
              "系数 \\(a_k\\) 也以 N 为周期：k 与 k+N 是同一个基函数",
              "与连续 FS 的差别：连续有无穷多条谐波且系数衰减，离散只有 N 条且循环重复",
              "实序列满足 \\(a_k=a_{-k}^*\\)，幅度谱关于 N/2 对称，只需一半数据"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "周期序列用 N 条谱线表示，非周期序列则需要连续谱——这就是离散时间傅里叶变换（DTFT）。它与第1章的连续时间傅里叶变换形式相似，但频谱有两处结构性差别：以 \\(2\\pi\\) 为周期、而且是数字角频率 ω 的函数。这两点都直接来自“时域是离散的”这一前提。",
            detail: [
              "定义是 \\[X(e^{j\\omega})=\\sum_{n=-\\infty}^{\\infty}x[n]e^{-j\\omega n},\\]反变换是 \\[x[n]=\\frac1{2\\pi}\\int_{-\\pi}^{\\pi}X(e^{j\\omega})e^{j\\omega n}\\,d\\omega.\\]因为 \\(e^{-j\\omega n}\\) 对 ω 以 \\(2\\pi\\) 为周期，\\(X(e^{j\\omega})\\) 必然以 \\(2\\pi\\) 为周期；而时域离散使求和而不是积分，所以只要序列绝对可和（\\(\\sum_n|x[n]|<\\infty\\)）级数就收敛。这两个特征与连续 FT 正好对偶：时域离散 ⇒ 频域周期，时域周期 ⇒ 频域离散。",
              "它与第1章的关系可以一句话概括：对连续信号采样，其频谱被周期化，\\[X_s(j\\Omega)=\\frac1T\\sum_k X_c\\!\\left(j(\\Omega-k\\Omega_s)\\right);\\]把频率按 \\(\\omega =\\Omega T\\) 归一化到数字角频率，取其中一个周期，就得到 DTFT。因此 \\(\\omega =\\pi\\) 对应的物理频率是 \\(f_s/2\\)（折叠频率），\\(\\omega =\\)\\(2\\pi\\) 对应 \\(f_s\\)。换算时记住 ω 是无量纲的 rad，物理频率要乘 \\(f_s/\\)(\\(2\\pi\\))。"
            ],
            points: [
              "DTFT 定义 \\(X(e^{j\\omega})=\\sum_n x[n]e^{-j\\omega n}\\)；绝对可和是它绝对收敛的充分条件，不是必要条件",
              "频谱以 \\(2\\pi\\) 为周期、关于 ω 连续；周期来自时域离散",
              "时域离散 ⇒ 频域周期；时域周期 ⇒ 频域离散（与连续情形对偶）",
              "\\(\\omega =\\pi\\) 对应折叠频率 \\(f_s/2\\)；物理频率 \\(= \\omega\\)\\(\\cdot f_s/\\)(\\(2\\pi\\))"
            ],
            pitfalls: [
              "把 ω 当成物理角频率：DTFT 的 ω 是归一化数字角频率，换算成赫兹要乘 \\(f_s/\\)(\\(2\\pi\\))",
              "只在 0–\\(2\\pi\\) 之外讨论频谱：DTFT 以 \\(2\\pi\\) 为周期，区间之外只是重复"
            ],
            links: [
              { to: "signals-ch1-frequency", why: "DTFT 就是连续频谱在一个周期内的归一化版本" },
              { to: "signals-ch2-dft", why: "把 DTFT 在一个周期内等间隔取样，就得到 DFT" }
            ],
            formula: "\\begin{aligned}X(e^{j\\omega})&=\\sum_{n=-\\infty}^{\\infty}x[n]e^{-j\\omega n}\\\\[6pt]x[n]&=\\frac1{2\\pi}\\int_{-\\pi}^{\\pi}X(e^{j\\omega})e^{j\\omega n}\\,d\\omega\\end{aligned}",
            variables: [
              "ω：数字角频率（rad），以 \\(2\\pi\\) 为周期",
              "\\(X(e^{j\\omega})\\)：DTFT，连续且周期",
              "n：整数序号",
              "积分区间 \\(2\\pi\\)：任意一个完整周期即可"
            ]
          },
          {
            id: "signals-ch2-dft",
            ...
            detail: [
              "定义是 \\(X[k]=\\sum_{n=0}^{N-1}x[n]e^{-j2\\pi kn/N}\\)，\\(k=0\\)…,N−1；反变换 \\[x[n]=\\frac1N\\sum_{k=0}^{N-1}X[k]e^{j2\\pi kn/N}.\\]\\(X[k]\\) 就是 \\(X(e^{j\\omega})\\) 在 \\(\\omega=2\\pi k/N\\) 上的样点，因此频域网格间距是 \\(\\Delta f=f_s/N_{\\mathrm{fft}}\\)（\\(N_{\\mathrm{fft}}\\) 为变换点数），能观察到的最高频率是 \\(f_s/2\\)（对应 \\(k=\\)\\(N_{\\mathrm{fft}}/2\\)）。",
              "网格间距不等于分辨能力——能否分开两个靠近的频率，由记录时长 \\(T_{\\mathrm{rec}}=L/f_s\\)（L 为实际数据点数）与窗函数主瓣宽度决定，补零只把网格加密。",
              "DFT 隐含周期性：正反变换的指数都以 N 为周期，等价于把序列和频谱都当作周期为 N 的循环序列。这带来两个必须知道的后果。其一是循环卷积：时域循环卷积对应频域相乘，要得到线性卷积的结果必须补零到长度 ≥ \\(N_1+N_2-1\\)。其二是截断效应：对长信号取 N 点相当于乘矩形窗，频谱被窗函数的谱“涂抹”，形成泄漏（旁瓣）与栅栏效应（只看得到离散频点，真实峰值落在频点之间时读数偏低）。",
              "增加 N 提高分辨率，换窗函数只改变泄漏与主瓣宽度的折中，两者不能互相替代。"
            ],
            points: [
              "DFT 是 DTFT 在一个周期内的 N 个等间隔样点，\\(X[k]=\\sum_n x[n]e^{-j2\\pi kn/N}\\)",
              "频域网格间距 \\(\\Delta f=f_s/N_{\\mathrm{fft}}\\)；可观察的最高频率是 \\(f_s/2\\)（\\(k=\\)\\(N_{\\mathrm{fft}}/2\\)）",
              "DFT 隐含 N 周期：时域循环卷积 ↔ 频域相乘，线性卷积需补零到 ≥\\(N_1+N_2-1\\)",
            ...
            variables: [
              "N：变换长度（数据点数）",
              "k：频率序号，对应频率 \\(f_k=kf_s/N\\)",
              "n：时间序号",
              "\\(\\Delta f=f_s/N_{\\mathrm{fft}}\\)：频域网格间距；分辨能力另由记录时长 \\(T_{\\mathrm{rec}}=L/f_s\\) 决定"
            ]
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "DFT 的定义是 N 次乘法求和，算全部 N 个频点就是 \\(O(N^2)\\) 次运算。\\(N=1024\\) 时约一百万次，\\(N=10^6\\) 时是一万亿次——直接算频谱分析根本没法用。FFT 不是新的变换，而是利用旋转因子的对称性和周期性，把同一个 DFT 算得更快。这一节只讲思路，下一节给算法。",
            detail: [
              "记旋转因子 \\(W_N=e^{-j2\\pi/N}\\)，则 \\(X[k]=\\sum_n x[n]W_N^{kn}\\)。\\(W_N\\) 有三条性质可用：周期性 \\(W_N^{k+N}=W_N^k\\)、对称性 \\(W_N^{k+N/2}=-W_N^k\\)、可约性 \\(W_N^{kn}=W_{N/m}^{kn/m}\\)。周期性说明只有 N 个不同的旋转因子，对称性说明一半的值只是另一半取负，可约性说明大问题能拆成小问题——这三条就是全部加速空间的来源。",
              "具体做法是分治：把 N 点 DFT 按序号奇偶拆成两个 N/2 点 DFT，再合并。这样每层把运算量减半、共 log₂N 层，总复杂度从 \\(O(N^2)\\) 降到 \\(O(N \\log _2N)\\)。\\(N=1024\\) 时约 10⁴ 量级，比直接计算快两个数量级。代价是基本算法要求 N 为 2 的幂（基2算法），实际使用中通常把数据补零到最近的 2 的幂——注意补零不增加信息量，它只是让算法可用并让频点更密。"
            ],
            points: [
              "DFT 直接计算是 \\(O(N^2)\\)；FFT 用分治把它降到 \\(O(N \\log _2N)\\)",
          ...
          },
          {
            ...
            detail: [
              ...
              "把同样的拆分递归下去，共 log₂N 层，每层 N/2 个蝶形，总运算量约 (N/2)log₂N 次复数乘法，即 \\(O(N \\log _2N)\\)。因为每层都按奇偶重新分组，输入序列的实际访问顺序是位倒序（bit-reversal），就地计算时通常先把输入按位倒序重排，再逐层蝶形。如果 N 不是 2 的幂，可以用混合基或补零。",
              "补零后算的是补零序列的 DFT，它只在与原网格重合的那些频点上等于原值，其余频点是插值：例如数据长度 \\(L=300\\)、补到 \\(N_{\\mathrm{fft}}\\)\\(=1024\\) 时，只有 \\(k=0\\)、75、150、225 四处与 300 点网格重合；只有 \\(N_{\\mathrm{fft}}\\) 是 L 的整数倍（如 256 补到 1024）时，原来的全部频点才都落在新网格上。"
            ],
            points: [
              "按时间抽取：奇偶分组后 \\(E[k]\\)、\\(O[k]\\) 由蝶形合并成 \\(X[k]\\) 与 \\(X[k+N/2]\\)",
              "蝶形核心：\\(X[k]=E[k]+W_N^k O[k]\\)，\\(X[k+N/2]=E[k]-W_N^k O[k]\\)",
              "共 log₂N 层，每层 N/2 个蝶形，总复杂度 \\(O(N \\log _2N)\\)",
          ...
          },
          {
            ...
            detail: [
              "快速卷积：两个长度分别为 N₁、N₂ 的序列，线性卷积长度是 \\(N_1+N_2-1\\)。把两者都补零到 \\(N \\ge\\) \\(N_1+N_2-1\\)，做 N 点 FFT、频域相乘、再 IFFT，就得到线性卷积结果。这样做的代价从 \\(O(N_1N_2)\\) 降到 \\(O(N \\log _2N)\\)，序列越长越划算。若数据是分块到来的（实时处理），用重叠相加法或重叠保留法把长序列切成块，块间用 FFT 卷积再拼起来，避免只做循环卷积导致尾部绕回。",
              "频谱分析：先定采样率 \\(f_s\\)（按奈奎斯特率留余量），再由需要的分辨能力反推记录时长 \\(T_{\\mathrm{rec}}\\approx1/\\Delta f\\) 与数据点数 \\(L=f_sT_{\\mathrm{rec}}\\)，最后把点数补到 2 的幂做 FFT。读数时记住三条：谱线间隔是 Δf，可观察上限是 \\(f_s/2\\)，加窗会同时加宽主瓣并压低旁瓣——矩形窗主瓣最窄但旁瓣最高，汉宁窗旁瓣低但主瓣宽，选窗是折中而不是“越好越好”。若要比较不同长度记录的幅度，还要按窗函数的相干增益做归一化。"
            ],
            points: [
              "快速卷积：补零到 \\(N\\ge\\)\\(N_1+N_2-1\\) 再做 N 点 FFT 相乘，得到的是线性卷积",
              "实时分块用重叠相加或重叠保留，避免循环卷积的尾部绕回",
              "频谱分析先定 \\(f_s\\)，再由 Δf 反推记录时长 \\(T_{\\mathrm{rec}}\\approx1/\\Delta f\\) 与数据点数 \\(L=f_sT_{\\mathrm{rec}}\\)，最后补零到 2 的幂",
            ...
            variables: [
              "\\(\\Delta f=f_s/N_{\\mathrm{fft}}\\)：频域网格间距（Hz）",
              "\\(T_{\\mathrm{rec}}=L/f_s\\)：记录时长（s）；分辨能力由它与窗主瓣宽度决定",
              "L：实际数据点数；\\(N_{\\mathrm{fft}}\\)：FFT 点数（补零后常取 2 的幂）",
              "N₁、N₂：参与卷积的两个序列长度"
            ]
          },
          {
            ...
            detail: [
              "定义是 \\(X(z)=\\sum_{n=-\\infty}^{\\infty}x[n]z^{-n}\\)，其中 z 为复变量。级数收敛的 z 取值集合称为收敛域（ROC）。常用变换对要连同收敛域一起记：\\(\\delta[n]\\)↔1（全平面）、\\(a^n u[n]\\)↔\\(\\frac{1}{1-az^{-1}}\\)（\\(|z|>|a|\\)）、\\(-a^n u[-n-1]\\)↔\\(\\frac{1}{1-az^{-1}}\\)（\\(|z|<|a|\\)）。最后两对说明关键的一点：同一个代数式配上不同收敛域，对应完全不同的序列。",
            ...
            points: [
              "Z 变换 \\(X(z)=\\sum_n x[n]z^{-n}\\)，是离散域的拉普拉斯变换",
              "收敛域 ROC 是变换的组成部分，必须与代数式一起给出",
              "同一代数式配不同 ROC 对应不同序列：如 \\(\\frac{1}{1-az^{-1}}\\) 在 \\(|z|>|a|\\) 与 \\(|z|<|a|\\) 下分别是右边与左边序列",
            ...
            variables: [
              "z：复变量，\\(z=re^{j\\omega}\\)；ω 为数字角频率（rad）",
              "ROC：使级数收敛的 z 平面区域",
              "\\(a^n u[n]\\) ↔ \\(\\frac{1}{1-az^{-1}}\\)，ROC 为 \\(|z|>|a|\\)",
          ...
          },
          {
            ...
            detail: [
              "Z 变换与拉普拉斯：\\(z=e^{sT}\\) 联系的是“采样冲激串的拉普拉斯变换”与“序列的 Z 变换”——把 \\(x_s(t)=\\sum_n x[n]\\delta(t-nT)\\) 做拉普拉斯变换得 \\(\\sum_n x[n]e^{-snT}\\)，令 \\(z=e^{sT}\\) 正是 \\(X(z)\\)。",
              "它不是把原连续信号 \\(x_c(t)\\) 的拉普拉斯表达式直接换变量：\\(x_c(t)=e^{-t}u(t)\\) 的变换是 \\(1/(s+1)\\)，采样后序列的 Z 变换是 \\(\\frac1{1-e^{-T}z^{-1}}\\)，用 \\(s=\\ln(z)/T\\) 代入 \\(1/(s+1)\\) 并不等于它。",
              "映射性质：s 左半平面（\\(\\sigma<0\\)）映到单位圆内（\\(|z|<1\\)），虚轴 \\(\\sigma=0\\) 映到单位圆；每条宽 \\(2\\pi /T\\) 的横带都多对一地映到 z 平面去掉原点的部分，\\(z=0\\) 不能由任何有限的 s 到达。判断稳定性不需要“选哪条横带”，判据只看极点是否落在单位圆内。",
              "Z 变换与 DTFT：把 z 取在单位圆上，\\(z=e^{j\\omega}\\)，Z 变换退化为 DTFT。收敛域包含单位圆可以保证 DTFT 绝对收敛，但这是充分条件，不是“一切意义下 DTFT 存在”的充要条件——周期序列的 ROC 不含单位圆，却仍有广义（冲激串）谱。注意区分两个频率记号：ω 是数字角频率（rad，以 \\(2\\pi\\) 为周期），连续角频率 \\(\\Omega=\\omega/T\\) 才是 rad/s。",
              "DFT 再进一步：它是 DTFT 在一个周期内等间隔取的 N 个点，即单位圆上的 N 等分点 \\(z=e^{j2\\pi k/N}\\)。四者关系可排成一条链：拉普拉斯 \\(\\to (\\)\\(z=e^{sT}\\)\\() \\to Z\\) 变换 \\(\\to (\\)单位圆\\() \\to DTFT \\to (N\\) 点等角采样\\() \\to DFT\\)。",
              "稳定性判据只适用于因果、有理的 LTI 输入输出系统：极点全在单位圆内就 BIBO 稳定；一般情形仍要看 ROC，零极点图本身不能替代 ROC。"
            ],
            points: [
              ...
              "数字角频率 ω 与连续角频率 Ω 相差因子 T：\\(\\Omega=\\omega/T\\)",
            ...
            variables: [
              "\\(s=\\sigma+j\\Omega\\)：连续复频率，Ω 为连续角频率（rad/s）；T：采样间隔",
              "\\(z=e^{sT}\\)：采样冲激串的拉普拉斯变换到 Z 变换的映射（多对一，不取 z=0）",
              "\\(|z|=1\\)：单位圆，对应 \\(\\sigma=0\\) 的虚轴",
              "ω：数字角频率（rad），与 Ω 的关系为 \\(\\Omega=\\omega/T\\)",
              "\\(z_k=e^{j2\\pi k/N}\\)：DFT 用的 N 等分点"
            ]
          },
          {
            ...
            detail: [
              "命令要点：用 `n=0:N-1` 显式建立序号向量（不要依赖数组下标从 1 开始的默认约定），用 `stem(n,x)` 画离散序列（plot 会连成折线，看起来像连续信号，是常见误用）；单位样值可以直接构造 `x=[1 zeros(1,N-1)]`，阶跃用 `x=(n>=0)`，复指数用 `exp(1j*w0*n)`。画周期序列前先算 \\(\\omega_0/(2\\pi)\\) 是否为有理数。",
              "验证点：取 \\(\\omega_0=2\\)（即 \\(\\cos (2n)\\)），用 `n=0:200` 画图会看到波形并不重复，与解析判据“\\(\\omega_0/(2\\pi)\\)\\(=1/\\pi\\) 为无理数、不是周期序列”一致；再把 \\(\\omega_0\\) 换成 π/8（π/8/(\\(2\\pi\\)\\()=1/16\\) 为有理数，周期 \\(N=16\\)），图上每 16 点重复一次，可以直接数出来。这一步把“离散正弦不一定周期”从公式变成可看见的事实。"
            ],
            points: [
              "序号向量必须显式建立：`stem(n,x)` 而不是 `stem(x)`",
              "画离散序列用 stem，用 plot 会连成折线造成误读",
              "复指数写成 `exp(1j*w0*n)`：i、j 虽是内置虚数单位，但常被同名循环变量覆盖，用 1j 最稳",
              "验证点：\\(\\omega_0=2\\) 不周期，\\(\\omega_0=\\pi/8\\) 的周期为 16"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            detail: [
              "命令要点：`y=conv(x,h)` 返回的是长度为 \\(N_1+N_2-1\\) 的线性卷积，但只有数值、不带时间信息——序号范围是两者的起点之和到终点之和。如果 x、h 的起点分别是 n1、n2，则 y 的起点是 n1+n2，画图时要用 `ny=(n1+n2):(n1+n2+length(y)-1)` 重新对齐，否则整条曲线会平移错位。要算循环卷积则用 `ifft(fft(x).*fft(h))`，两者在补零长度足够时结果一致。",
              "验证点：取 \\(x=[1 1 1 1]\\)（宽度 4 的矩形）与 \\(h=[1 1]\\)（宽度 2），conv 得到 \\([1 2 2 2 1]\\)，长度 \\(4+2-1=5\\)，峰值 \\(2 = \\mathrm{min} (4,2)\\)，总和 \\(8 = 4\\times 2\\)——与第一节“峰值取较短者、总面积等于两者面积之积”的结论逐条对上。再用频域路径核对：`yf=ifft(fft(x,8).*fft(h,8))`，前 5 点应与 conv 结果一致（容差 1e-10），后 3 点应近零。",
              "注意两条路径的数组长度必须相同——`fft(x,8)` 与 `fft(h,8)` 都补到 8 点才能逐个相乘；写成 `fft([x zeros(1,3)])` 只有 7 点，与另一个 7 点数组虽然也能相乘，但那是 7 点循环卷积，与 8 点补零的含义不同。"
            ],
            points: [
              "conv 给线性卷积，长度 \\(N_1+N_2-1\\)，但不含序号信息，必须自己对齐起点",
              "补零后用 `ifft(fft(x,8).*fft(h,8))` 得到同样的线性卷积：前 5 点等于 conv 结果，后 3 点近零",
              "验证点：\\(x=[1 1 1 1]\\)、\\(h=[1 1] \\to [1 2 2 2 1]\\)，峰值 \\(2=\\mathrm{min} (4,2)\\)，总和 \\(8=4\\times 2\\)",
          ...
          },
          {
            ...
            detail: [
              "命令要点：`X=fft(x)` 之后必须自己构造频率轴 `f=(0:N-1)*(fs/N)`；只看前一半 `f(1:N/2+1)`（对应 0 到 \\(f_s/2\\)），取 `abs(X)` 得幅度谱；若要看单边幅度谱，只有实序列才能这样构造：先除以 N，再把非直流分量乘 2；偶数 N 时直流分量与 \\(k=N/2\\)（奈奎斯特）都不翻倍，奇数 N 没有奈奎斯特端点。",
              "加窗后要按所用窗的相干增益归一化。窗口用 `x=x.*hann(N)'`，注意加窗后幅度与分辨率都会变，比较不同窗的结果前先统一归一化。",
              "验证点：取 \\(fs=1000\\) Hz、\\(x=\\cos (\\)\\(2\\pi \\cdot 100\\cdot t\\))，\\(N=1000\\) 点，fft 后峰值应出现在 \\(k=100\\) 处，换算成 \\(f=100\\) Hz；把 N 改成 100（记录时间 0.1 s），分辨率降到 10 Hz，峰仍然读得到 100 Hz，但相邻频点间距从 1 Hz 变成 10 Hz——直接演示 \\(\\Delta f=f_s/N\\)。",
              "再把信号频率改成 105 Hz，在 \\(N=100\\) 时峰值会摊到 100 与 110 两点上，这就是栅栏效应。"
            ],
            points: [
              "频率轴必须自己构造：`f=(0:N-1)*(fs/N)`，只看前一半",
              "单边幅度谱只对实序列成立：除 N 后非直流乘 2；偶数 N 时直流与 \\(k=N/2\\) 都不翻倍",
              "验证点：\\(fs=1000\\)、\\(N=1000\\)、\\(100 Hz \\to\\) 峰值在 \\(k=100\\)；\\(N=100\\) 时 Δf 变为 10 Hz",
          ...
          },
          {
            ...
            detail: [
              "命令要点：`fft(x)` 与 `fft(x, N)` 的区别在第二个参数——后者把序列补零或截断到 N 点。`nfft=2^nextpow2(length(x))` 是常用写法，先把点数补到 2 的幂再变换。要比较直接 DFT 与 FFT 的耗时，可用 tic/toc 包住 `fft(x)` 与手写的双重循环 DFT，取 \\(N=512\\) 与 \\(N=4096\\) 两组看数量级差别。",
              "注意两点：fft 的实现不要求 N 是 2 的幂（库会按长度选择分解方式，素数长度也能算），而且实测耗时比不等于运算量比——缓存、向量化与库实现都会影响结果，所以只作量级验证，不要用具体倍数下结论。",
              "验证点：其一，补零不增加信息——数据长度 \\(L=256\\)、补零到 \\(N_{\\mathrm{fft}}\\)\\(=1024\\) 时，1024 是 256 的整数倍，原有 256 个频点全部落在新网格上，用容差（如 1e-10）比较应当一致；多出来的频点是插值（图形更光滑但峰宽不变）。若 L 与 \\(N_{\\mathrm{fft}}\\) 不是整数倍关系，就只有部分频点重合——例如 \\(L=300\\) 补到 1024 时只有 \\(k=0\\)、75、150、225 四处重合。",
              "其二，分辨率由有效记录长度决定——把同一个信号分别用 0.05 s 与 0.5 s 的记录长度做 FFT（都补零到相同点数），后者能把相隔更近的两个频率分开，前者不能。这两点合起来说明：补零改善的是显示密度，不是分辨能力。"
            ],
            points: [
              "`fft(x, N)` 的第二个参数是补零或截断到 N 点；常用 `nfft=2^nextpow2(length(x))`",
              "验证点一：\\(L=256\\) 补到 1024（整数倍）时原频点全重合，可用容差比较；L 与 \\(N_{\\mathrm{fft}}\\) 非整数倍时只有部分重合",
              "验证点二：记录长度从 0.05 s 增到 0.5 s 才能分开更近的两个频率",
              "fft 不要求 N 为 2 的幂；实测耗时比不等于运算量比，只作 \\(O(N\\log N)\\) 对 \\(O(N^2)\\) 的量级验证"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            detail: [
              "命令要点：把 \\(X(z)\\)\\(=B(z)/A(z)\\) 写成 \\(z^{-1}\\) 的降幂系数向量 b、a，然后 `zplane(b,a)` 画零极点图、tf2zp 求零点极点增益、`residuez(b,a)` 做部分分式展开（返回留数、极点与直接项）。差分方程用 `filter(b,a,x)` 求响应、impz 求单位样值响应、freqz 求频率响应。",
              "符号计算可用 Symbolic Math Toolbox 的 ztrans/iztrans（单边定义），双边问题仍要自己用部分分式加收敛域判断，再用上面的数值函数核对。",
              "验证点：取 \\(b=[1]\\)、\\(a=[1 -0.5]\\)，即 \\(H(z)=\\frac1{1-0.5z^{-1}}\\)。zplane 显示单极点 \\(z=0.5\\) 在单位圆内，据上一节的判据该系统因果且稳定；impz 给出 \\(0.5^n\\)，与解析式一致；`freqz(1,[1 -0.5],[0 pi])` 显式指定两个端点频点，幅频响应在 \\(\\omega =0\\) 处为 \\(1/(1-0.5)=2\\)，在 \\(\\omega =\\pi\\) 处为 \\(1/(1+0.5)\\approx 0.667\\)，与把 \\(z=±1\\) 代入 \\(H(z)\\) 一致。",
              "这三条把“极点在单位圆内 ⇒ 稳定”“单位圆上取值 ⇒ 频率响应”从口诀变成可读的数。"
            ],
            points: [
              "Z 变换的工具围绕有理式：zplane 看零极点、residuez 做部分分式、freqz 求频响",
              "差分方程用 filter / impz 求响应，与解析推得的 \\(h[n]\\) 对照",
              "验证点：\\(H(z)=\\frac1{1-0.5z^{-1}}\\) → 极点 0.5 在单位圆内，稳定；\\(h[n]=0.5^n\\)",
              "`freqz(1,[1 -0.5],[0 pi])` 显式取端点：读数应等于把 \\(z=±1\\) 代入 \\(H(z)\\)"
            ],
            pitfalls: [
              "系数向量顺序写错：freqz/zplane/residuez 的 b、a 按 [b0,b1,…] 对应 \\(z^0\\)、\\(z^{-1}\\)、… 排列，而 tf2zp 用的是 z 的降幂——同一个 \\(H(z)\\)\\(=1-2\\)\\(z^{-1}\\) 在 zplane 里写 \\([1 -2]\\)、在 tf2zp 里要写成 \\(([1 -2],[1 0])\\)。顺序写错时 tf2zp 会直接报“传递函数不当”",
        ...
        examples: [
          ...
          },
          {
            ...
            steps: [
              "离散角频率只在模 \\(2\\pi\\) 意义下区分，频率可相差整数倍 fs。",
          ...
          {
            title: "判断离散正弦是否周期",
            prompt: "判断 \\(x[n]\\)\\(=\\cos (2n)\\) 与 \\(y[n]\\)\\(=\\cos (\\pi n/8)\\) 是否为周期序列，是则求周期。",
            steps: [
              "判据是 \\(\\omega_0/(2\\pi)\\) 是否为有理数。",
              "对 \\(x[n]\\)：\\(\\omega_0=2\\)，2/(\\(2\\pi\\)\\()=1/\\pi\\) 是无理数，故非周期。",
              "对 \\(y[n]\\)：\\(\\omega_0=\\pi/8\\)，(π/8)/(\\(2\\pi\\)\\()=1/16\\) 是有理数；\\(N=16\\) 时 \\(\\omega_0\\)\\(N=\\)\\(2\\pi\\)，故周期为 16。"
            ],
            answer: "\\(\\cos (2n)\\) 不是周期序列；\\(\\cos (\\pi n/8)\\) 是周期序列，周期 \\(N=16\\)。"
          },
          {
            title: "抽取前的限带要求",
          ...
          },
          {
            ...
            steps: [
              "\\[a_k=\\frac1N\\sum_{n=0}^{N-1}x[n]e^{-j2\\pi kn/4},\\]\\(N=4\\)。",
              "\\(a_0=\\frac14(1+1+0+0)=\\frac12\\)。",
              "\\(\\begin{aligned}a_2&=\\frac14(1+e^{-j\\pi})\\\\&=\\frac14(1-1)=0\\end{aligned}\\)。"
            ],
            answer: "\\(a_0\\)\\(=1/2\\)（直流分量），\\(a_2\\)\\(=0\\)；系数以 \\(N=4\\) 为周期重复。"
          },
          {
            title: "由采样率换算 DTFT 的频率轴",
            prompt: "某系统采样率 \\(fs=8\\) kHz。DTFT 中 \\(\\omega =0.5\\pi\\) 对应的物理频率是多少？\\(\\omega =\\pi\\) 呢？",
            steps: [
              "换算关系是 \\(f=\\omega \\cdot fs/\\)(\\(2\\pi\\))。",
              "\\(\\omega =0.5\\pi\\) 时 \\(f=0.5\\pi \\times 8000/\\)(\\(2\\pi\\)\\()=2000\\) Hz。",
              "\\(\\omega =\\pi\\) 时 \\(f=\\pi \\times 8000/\\)(\\(2\\pi\\)\\()=4000 Hz=fs/2\\)，正是折叠频率。"
            ],
            answer: "\\(\\omega =0.5\\pi\\) 对应 2 kHz；\\(\\omega =\\pi\\) 对应 4 kHz（折叠频率）。"
          },
          {
            title: "DFT 的分辨率与最高频率",
            prompt: "以 \\(fs=8000\\) Hz 采样，取 \\(N=1024\\) 点作 DFT。频率分辨率与可观察的最高频率各是多少？",
            steps: [
              "分辨率 \\(\\Delta f=fs/N=8000/1024\\approx 7.8\\) Hz。",
              "\\(k=0\\) 对应直流，\\(k=N/2=512\\) 对应 \\(fs/2=4000\\) Hz。",
              "k>N/2 的部分对应负频率，不是更高的正频率。"
            ],
            answer: "\\(\\Delta f\\approx 7.8\\) Hz；可观察最高频率 4 kHz。"
          },
          {
            title: "FFT 与直接 DFT 的计算量对比",
            prompt: "\\(N=1024\\) 时，直接计算 DFT 与基2 FFT 的复数乘法量级各是多少？",
            steps: [
              "直接 DFT 约 \\(N^2=1024^2\\approx 1.05\\times 10\\)⁶ 次。",
              "基2 FFT 约 \\((N/2)\\log _2N=512\\)\\(\\times 10=5.1\\times 10\\)³ 次。",
          ...
          },
          {
            ...
            steps: [
              ...
              "只有两条网格重合的频点才等于原值：\\(L=300\\) 与 \\(N_{\\mathrm{fft}}\\)\\(=1024\\) 的重合条件是 k·1024 能被 300 整除。",
          ...
          },
          {
            ...
            steps: [
              "蝶形关系：\\(\\begin{aligned}X[k]&=E[k]+W_8^k O[k]\\\\X[k+4]&=E[k]-W_8^k O[k]\\end{aligned}\\)。",
          ...
          {
            title: "同一代数式的两种收敛域",
            prompt: "\\(X(z)\\)\\(=1/(1-0.5\\)\\(z^{-1}\\))。分别取 ROC 为 |z|>0.5 与 |z|<0.5，写出对应的序列。",
            steps: [
              "查表：\\(a^n u[n]\\)↔\\(\\frac{1}{1-az^{-1}}\\)，ROC 为 \\(|z|>|a|\\)。",
              "取 |z|>0.5 得右边序列 \\(x[n]=0.5^n u[n]\\)。",
              "取 |z|<0.5 得左边序列 \\(x[n]=−0.5^n\\) \\(u[-n-1]\\)。"
            ],
            answer: "\\(|z|>0.5 \\to\\) \\(0.5^n u[n]\\)；\\(|z|<0.5 \\to -\\)\\(0.5^n\\) \\(u[-n-1]\\)。同一代数式、不同 ROC、不同信号。"
          },
          {
            title: "判断离散系统的因果性与稳定性",
            prompt: "\\(H(z)\\)\\(=1/(1-0.8\\)\\(z^{-1}\\))，ROC 取 |z|>0.8。判断因果性与稳定性。",
            steps: [
          ...
          },
          {
            ...
            prompt: "画 `n=0:200` 上的 \\(\\cos (2n)\\) 与 \\(\\cos (\\pi n/8)\\)，如何从图上确认周期性判据？",
            steps: [
              "序号向量 `n=0:200`，序列 \\(x=\\cos (2*n)\\)、\\(y=\\cos (pi/8*n)\\)。",
              "`stem(n,x)` 看不出重复；\\(stem(n,y)\\) 每隔 16 点重复一次。",
              "与判据对照：2/(\\(2\\pi\\)\\()=1/\\pi\\) 无理数不周期；(π/8)/(\\(2\\pi\\)\\()=1/16\\) 有理数、周期 16。"
            ],
            answer: "\\(\\cos (2n)\\) 的图不重复、\\(\\cos (\\pi n/8)\\) 每 16 点重复，与解析判据一致。"
          },
          {
            ...
            answer: "\\(y=[1 2 2 2 1]\\)，定义在 \\(n=0\\)…4；峰值 \\(2=\\mathrm{min} (4,2)\\)，总和 \\(8=4\\times 2\\)。"
          },
          {
            title: "读单边幅度谱",
          ...
          {
            title: "freqz 与解析频响对照",
            prompt: "\\(H(z)=\\frac1{1-0.5z^{-1}}\\)。用 freqz 读 \\(\\omega =0\\) 与 \\(\\omega =\\pi\\) 的幅度，并与解析值比较。",
            steps: [
              "\\(\\omega =0\\) 对应 \\(z=1\\)：\\(|H|=1/|1-0.5|=2\\)。",
              "\\(\\omega =\\pi\\) 对应 \\(z=-1\\)：\\(|H|=1/|1+0.5|\\approx 0.667\\)。",
              "\\(freqz([1],[1 -0.5])\\) 在这两点给出同样的数；极点 0.5 在单位圆内，响应有界。"
            ],
            answer: "\\(\\omega =0\\) 处 2、\\(\\omega =\\pi\\) 处约 0.667，与把 \\(z=±1\\) 代入 \\(H(z)\\) 完全一致。"
          }
        ],
        experiments: [
          {
            id: "signals-ch2-aliasing",
            title: "时频双域采样、混叠与模拟重建",
            workbench: "notebook",
            goal: "看清时域采样在频域引起的周期延拓，理解同一组样点为何对应多个连续信号，并区分显示连线与有条件的信号重建。",
            steps: [
              "先预测：把 \\(f_s\\) 从 20 Hz 降到 10 Hz，样点会怎样变化？观测频率会变成多少？写下来再动手。",
              "取 \\(f_{in}=7\\) Hz、\\(f_s=20\\) Hz，观察 sinc 重建与原曲线的贴合程度，并记录 RMSE 读数。",
              "反例对照一：把 \\(f_s\\) 降到 10 Hz，核对指标卡的观测频率 3 Hz，并验证 7 Hz 与 3 Hz 的余弦在同一组样点上取值相同。",
              "反例对照二：分别选零阶保持与线性插值，比较重建波形的台阶/折线形状与 RMSE，说明“点连得更平滑”为什么不能消除混叠。",
              "用自己的话解释：为什么重建需要带限这类先验条件，而不是仅靠把样点画得更平滑？"
            ],
            expected: "7/20：sinc 重建 RMSE < 0.01、ZOH > 0.5；7/10：指标卡显示观测频率 3 Hz 且状态为混叠；7/14：状态显示“临界，依赖相位，不保证一般恢复”。",
            limitation: "sinc 重建只取 \\(n=-32\\)…32 的有限截断，不是完整 DAC 模型；未包含量化、孔径与前端滤波误差。"
          },
          {
            id: "signals-ch2-spectral-leakage",
            title: "观察时间、窗与补零分别改变了什么",
            workbench: "notebook",
            goal: "把「频谱看不清」拆成三个互不相同的原因：观察时间决定主瓣宽度、窗形决定旁瓣高低、补零只改变读数密度而不增加信息。",
            steps: [
              "先预测：把观察长度 L 从 16 增到 64（\\(f_s\\) 固定 64 Hz），10 Hz 与 12 Hz 两个音在谱上会怎样变化？只补零能不能让它们分开？写下来再动手。",
              "取默认 \\(L=32\\)、矩形窗、补零 \\(M=0\\)，记录指标卡的 \\(L/f_s\\)、\\(f_s/L\\) 与补零后网格 \\(f_s/N_{fft}\\) 三个量，并数一数 \\([9,13]\\) Hz 内有几个局部峰。",
              "反例对照一：把 L 降到 16，两音合并成一个约 11.08 Hz 的峰；再把 L 增到 64，分成约 9.89 Hz 与 12.14 Hz 两个峰。说明「增大观察时间 ⇒ 主瓣变窄」而不是「补零让谱变清晰」。",
              "反例对照二：固定 \\(L=64\\)，只把补零 M 从 0 增到 224。谱线变密、网格 \\(f_s/N_{fft}\\) 变细，但同一频率上的谱值完全不变——补零增加的是读数密度，不是信息。",
              "用自己的话解释：换用 Hann 窗后第一旁瓣从约 −13 dB 降到约 −31 dB，主瓣却更宽了。这个代价换来的是什么？为什么低旁瓣值得付这个代价？"
            ],
            expected: "\\(L=16\\) 时 \\([9,13]\\) Hz 内只有 1 个约 11.08 Hz 的局部峰；\\(L=64\\) 时分成约 9.89 Hz 与 12.14 Hz 两个峰；补零前后同一频率的谱值不变；\\(L=64\\) 时矩形窗第一旁瓣约 −13.3 dB、Hann 约 −31.5 dB，且 Hann 主瓣（约 2.03 Hz）比矩形（约 1.00 Hz）更宽。",
            limitation: "只对照矩形窗与对称 Hann 窗、只画 \\([0,32]\\) Hz；\\(f_s\\) 固定 64 Hz，不含窗增益校正、重叠相加等工程细节，也不把 \\(f_s/L\\) 当成不依赖窗的精确双峰分辨率。"
          },
          {
            id: "signals-ch2-circular-convolution",
            title: "循环卷积：尾部如何绕回头部",
            workbench: "notebook",
            goal: "看清循环卷积就是把线性卷积的结果按 n mod N 折回叠加，并分清「折回样本数」与「受影响桶数」这两个不同的量。",
            steps: [
              "先预测：长度 4 的矩形序列与自身做 4 点循环卷积，结果还是 \\([1,2,3,4]\\) 吗？写下来再动手。",
              "取 \\(N_1=N_2=4\\)、\\(N=4\\)，对照左图线性结果 \\([1,2,3,4,3,2,1]\\) 与右图循环结果 \\([4,4,4,4]\\)，指出被折回的是哪三个样本。",
              "反例对照一：把 N 增到 7（\\(= N_1+N_2-1\\)），循环结果与线性完全一致、首 N 点误差 \\(E=0\\)；再看 \\(N=10\\)，尾部出现 3 个零。说明 N 够大就没有折回，而不是「循环卷积总是不准」。",
              "反例对照二：取 \\(N_1=N_2=8\\)、\\(N=2\\)，13 个折回样本被压进 2 个桶（桶数 \\(= \\mathrm{min} (N, L-N)\\)），每桶累加成 32；再对照 DFT 核对路线与直接路线。",
              "用自己的话解释：为什么 \\(N \\ge N_1+N_2-1\\) 就不会发生尾部折回？频域采样与时域周期求和是怎么对应起来的？"
            ],
            expected: "(4,4,N4)：线性 \\([1,2,3,4,3,2,1]\\)、循环 \\([4,4,4,4]\\)、首 N 点误差 \\(E=6\\)；\\(N=7\\) 与线性一致；\\(N=10\\) 尾部 3 个零；(8,8,N2)：\\([32,32]\\)、折回 13 个样本、受影响 2 个桶；DFT 核对路线与直接路线一致。",
            limitation: "输入限定为从 0 开始的单位矩形序列，折回样本数与受影响桶数的公式只对当前正值矩形成立；不给出一般序列的循环卷积误差估计。"
          }
        ],
        check: [
          {
            ...
            answer: 1,
            explanation: "时域乘冲激串 ⇒ 频域周期延拓：\\(X_s(j\\Omega )=(1/T)\\Sigma X_c(j(\\Omega -k\\Omega _s))\\)，副本间隔为 \\(\\Omega _s\\)。"
          },
          {
          ...
          },
          {
            ...
            answer: 2,
            explanation: "离散复指数周期 ⟺ ω₀/(2π) 为有理数；\\(2/(2\\pi )=1/\\pi\\) 为无理数，故不周期。"
          },
          {
            id: "signals-ch2-check-8",
            prompt: "单位样值序列 \\(\\delta [n]\\) 的性质是：",
            options: ["面积为 1 的冲激", "\\(\\delta [0]=1\\) 的普通序列，并有筛选性质", "恒为 0", "只在 \\(n=\\infty\\) 处非零"],
            answer: 1,
            explanation: "离散的 \\(\\delta [n]\\) 有确定取值（\\(n=0\\) 时为 1），不是极限意义上的冲激；筛选性质 \\(\\Sigma x[n]\\delta [n-k]=x[k]\\) 是卷积和的出发点。"
          },
          {
          ...
          },
          {
            ...
            answer: 2,
            explanation: "基函数 \\(e^{j2\\pi kn/N}\\) 对 k 以 N 为周期，k 与 k+N 是同一个谐波；系数也以 N 为周期。"
          },
          {
          ...
          },
          {
            ...
            prompt: "采样率 \\(fs=8\\) kHz 时，DTFT 中 \\(\\omega =\\pi\\) 对应的物理频率是：",
            options: ["8 kHz", "4 kHz", "2 kHz", "π kHz"],
            answer: 1,
            explanation: "\\(f=\\omega \\cdot fs/\\)(2π)，\\(\\omega =\\pi\\) 时 \\(f=fs/2=4\\) kHz，即折叠频率。"
          },
          {
            ...
            answer: 1,
            explanation: "\\(X[k]\\) 就是 \\(X(e^{j\\omega })\\) 在 \\(\\omega =2\\pi k/N\\) 上的取值；频域网格间距为 \\(fs/N_{fft}\\)，而分辨能力由记录时长决定。"
          },
          {
            id: "signals-ch2-check-15",
            prompt: "用 FFT 计算两个序列的线性卷积时，必须：",
            options: ["直接频域相乘", "补零到 \\(N\\ge N_1+N_2-1\\) 再相乘", "只保留前 N 点", "先做反折"],
            answer: 1,
          ...
          },
          {
            ...
            answer: 1,
            explanation: "FFT 不改变 DFT 的定义，只把计算量从 \\(O(N^2)\\) 降到 \\(O(N \\log _2N)\\)。"
          },
          {
            id: "signals-ch2-check-17",
            prompt: "基2 FFT 蝶形运算的两条输出是：",
            options: ["\\(E[k]+O[k]\\) 与 \\(E[k]-O[k]\\)", "\\(E[k]+W_N^k O[k]\\) 与 \\(E[k]-W_N^k O[k]\\)", "\\(E[k]\\)\\(\\cdot O[k]\\) 与 \\(E[k]/O[k]\\)", "\\(E[k]\\) 与 \\(O[k]\\)"],
            answer: 1,
          ...
          },
          {
            ...
            options: ["补零点数", "窗函数类型", "记录时长 \\(T_{rec}=L/fs\\)", "FFT 算法"],
            answer: 2,
            explanation: "分辨能力由记录时长 \\(T_{rec}=L/fs\\)（L 为数据点数）与窗主瓣宽度共同决定：补零只加密网格，换窗改变主瓣与旁瓣的折中，都不能突破记录时长的物理下限。"
          },
          {
        ...
        summary: [
          "采样把连续频谱以 fs 为周期整份复制；副本不重叠的条件 \\(f_s\\ge 2f_{max}\\) 就是时域采样定理。",
          "频域采样定理是它的对偶：N 个频点足以表示长度 N 的序列，这也正是 DFT 的定义来源。",
          "DFS 用 N 条谐波表示周期序列，DTFT 用连续周期谱表示非周期序列，DFT 是 DTFT 的 N 点采样。",
          "FFT 不改变 DFT 的定义，只把计算量降到 \\(O(N \\log _2N)\\)；分辨率的物理下限由记录时间决定。",
      ...
      },
      {
        ...
        sections: [
          {
            ...
            detail: [
              "方程是第一条路。连续系统写成常系数线性微分方程，离散系统写成差分方程 \\(y[n]=-\\Sigma a_k y[n-k]+\\Sigma b_k x[n-k]\\)。方程阶数决定了需要几个初始条件才能定解：一阶差分方程要 \\(y[-1]\\)，二阶要 \\(y[-1]\\)、\\(y[-2]\\)。方程右边只含输入的项给出零状态响应的来源，左边带输出的项决定系统自身的动态。",
          ...
          },
          {
            ...
            detail: [
              "两个反例最能说明“逐项判断”的必要。其一 \\(y[n]=n\\)\\(\\cdot x[n]\\)：它满足线性和无记忆，也是因果的，但含显式时间系数所以时变；更关键的是取 \\(x[n]\\)≡1（有界），输出 \\(y[n]=n\\) 在 n 遍历全部整数时无界，因此不是 BIBO 稳定。其二 \\(y[n]=(x[n])^2\\)：它不是线性（\\(T{2x}=4y\\ne 2T{x}\\)），但时不变、因果、无记忆，且 \\(|x|\\le 1\\) 时 \\(|y|\\le 1\\)，是 BIBO 稳定的。可见“线性”“稳定”“因果”“可逆”彼此不能互推。",
              "对 LTI 系统，几条判据可以化简：因果 ⟺ \\(h[n]=0\\)（n<0）；BIBO 稳定 ⟺ \\(\\Sigma |h[n]|<\\infty\\)（连续情形为 \\(\\int |h(t)|dt<\\infty\\)）；可逆则要求存在 \\(h_{inv}\\) 使 \\(h*h_{inv}=\\delta\\)。稳定与可逆是两件事：稳定系统可以有零点在关键频率上，使逆系统不可实现或不可稳定实现——这正是下一节解卷积要处理的问题。"
            ],
            points: [
              "线性要求同时满足齐次性与可加性，只满足一条不算线性",
              "\\(y[n]=n\\)\\(\\cdot x[n]\\)：线性、无记忆、因果，但时变且非 BIBO 稳定（取 x≡1 得 \\(y=n\\) 无界）",
              "\\(y[n]=x[n]^2\\)：非线性，但时不变、因果、无记忆且 BIBO 稳定",
              "LTI 判据：因果 ⟺ n<0 时 \\(h[n]=0\\)；BIBO 稳定 ⟺ \\(\\Sigma |h[n]|<\\infty\\)"
            ],
            pitfalls: [
              "由“线性”推出“稳定”，或由“因果”推出“无记忆”：这四条性质彼此独立，必须逐条验证",
              "检查 BIBO 稳定时只在有限范围内取 n：\\(y[n]=n\\)\\(\\cdot x[n]\\) 要看出无界，必须让 n 遍历无界整数集"
            ],
            links: [
          ...
          },
          {
            ...
            detail: [
              ...
              "只要初值非零，输出就多出一项。取 \\(y[-1]=1\\)、输入仍为单位阶跃，则零输入分量为 \\(0.5^(n+1)\\)（\\(n\\ge 0\\)），与零状态分量 \\(2(1-0.5^(n+1))\\) 叠加得全响应 \\(2-0.5^(n+1)\\)。这组数字说明：同一系统同一输入，初值不同结果就不同，而差值正是零输入分量。时域法的优点是能看清每个分量的来源；缺点是卷积要按区间分段，计算量大。"
            ],
            points: [
              "全响应 = 零输入响应 + 零状态响应，两者必须分开算再相加",
              "零状态响应 \\(= x*h\\)；零输入响应由特征根与初值决定",
              "样例 \\(y[n]=x[n]+0.5y[n-1]\\)：\\(h[n]=0.5^n u[n]\\)，阶跃响应 \\(2(1-0.5^(n+1))\\to 2\\)",
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "时域卷积难算，频域把它变成乘法。对 LTI 系统，输入复指数 \\(e^{j\\omega n}\\) 时输出仍是同频复指数，只乘一个与时间无关的系数 \\(H(e^{j\\omega })\\)——这就是频率响应。它决定各频率分量被放大、衰减还是移相，也解释了为什么滤波器可以“按频率挑选信号”。",
            detail: [
              "正弦稳态是频率响应最直观的用法：输入 \\(A \\cos (\\omega _0n+\\varphi )\\) 时，稳态输出为 \\(A|H(e^{j\\omega _0})|\\cos (\\omega _0n+\\varphi +\\)∠\\(H(e^{j\\omega _0}))\\)——幅度乘幅频特性、相位加相频特性。注意这里说的是稳态：瞬态分量由系统极点决定，会随时间衰减（极点须在单位圆内），与输入无关。",
            ...
            points: [
              "复指数是 LTI 系统的特征函数：输入 \\(e^{j\\omega n}\\) 时输出为 \\(H(e^{j\\omega })e^{j\\omega n}\\)",
          ...
          },
          {
            ...
            detail: [
              ...
              "其次是噪声与病态。以 Y/X 估计 H 时，分母在 X 小的频点上会把噪声放大：哪怕测量噪声很小，在 \\(X\\to 0\\) 处估计值也会发散。常用对策是改在相关域做估计（\\(H\\approx S_{xy}/S_{xx}\\)，噪声与输入不相关时偏差更小），或在分母上加正则项抑制小信噪比频点。辨识的目标是“在关心的频段内给出可用的模型”，不是逐点复现测量。"
            ],
            points: [
              "系统辨识 = 由已知输入与测得输出估计 H，基本关系 \\(H=Y/X\\)（零状态）",
              "输入必须在关心频段内持续激励，否则部分频段的 H 无法观测",
              "小分母放大噪声：X 接近零的频点估计会发散",
              "改用相关域估计（\\(S_{xy}/S_{xx}\\)）或加正则项，可显著改善小信噪比频点"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            detail: [
              "取 \\(H(z)=1-2z^{-1}\\)。它是有限长冲激响应（\\(h[0]=1\\)、\\(h[1]=-2\\)、其余为零），显然 BIBO 稳定。它的因果逆是 \\(H_{inv}(z)=1/(1-2z^{-1})\\)，极点在 \\(z=2\\)——单位圆外，因此这个因果逆不稳定；输入为单位阶跃时递推 \\(y[n]=x[n]+2y[n-1]\\) 按 \\(2^(n+1)\\) 增长（\\(n=39\\) 时约 1.1×10¹²）；输入为冲激时 \\(h[n]=2^n\\)，\\(n=39\\) 时约 5.5×10¹¹。两种输入都说明因果逆不满足 BIBO 稳定。也就是说，虽然 \\(H(z)\\) 处处非零、逐点相除在代数上可行，但恢复出的输入会被噪声以同样速率放大，实际不可用。",
          ...
          },
          {
            ...
            points: [
              "链路：模拟前端 → 抗混叠滤波 → 采样保持 → 量化编码 → 数字运算 \\(\\to DAC\\) 与重建滤波",
          ...
          },
          {
            ...
            detail: [
              "舍入量化模型：把步长 Δ 的均匀量化写成 \\(Q(x)=\\Delta\\)\\(\\cdot round(x/\\Delta )\\)。只要不发生溢出，误差满足 \\(|Q(x)-x|\\le \\Delta /2\\)——这是确定性的界，对每个样本都成立，不需要任何统计假设。位数每增加 1 位，Δ 减半，误差上界也随之减半（约 6 dB）。",
              "常被引用为“量化噪声功率 Δ²/12”的结论则需要额外前提：误差在 \\([-\\Delta /2,\\Delta /2]\\) 上均匀分布、与信号不相关、各样本互不相关，且不溢出。只有这些条件大致满足时，才能把它当作白噪声来估算信噪比；对正弦等规则信号、或误差与信号强相关时，量化误差是确定性的失真（谐波），用 Δ²/12 会给出错误的乐观估计。系数量化则要单独看：它直接移动极点，窄带或高阶滤波器的极点可能被推到单位圆外，因此实现前应检查量化后的极点半径。"
            ],
            points: [
              "舍入量化 \\(Q(x)=\\Delta\\)\\(\\cdot round(x/\\Delta )\\)：无溢出时 \\(|Q(x)-x|\\le \\Delta /2\\)（确定性界，不含统计假设）",
          ...
          },
          {
            ...
            points: [
              ...
              "验证点：阶跃前 20 点 \\(\\to 1\\)、1.5、1.75、…，与 \\(2(1-0.5^(n+1))\\) 一致（容差 1e-12）",
          ...
          },
          {
            ...
            detail: [
              ...
              "验证点：对 \\(H(z)=1/(1-0.5z^{-1})\\)，freqz 在 \\(\\omega =0\\) 给出 2、在 \\(\\omega =\\pi\\) 给出 \\(1/1.5\\approx 0.6667\\)，与把 \\(z=1\\)、\\(z=-1\\) 代入 \\(H(z)\\) 完全一致；这与时域稳态值 2 也对应（直流增益 = 阶跃响应的稳态值）。反过来说，如果时域稳态值不等于 \\(H(1)\\)，说明要么递推还没到稳态，要么零输入分量仍在——这是最省事的自检。"
            ],
            points: [
              "\\(freqz(b,a,[0 pi])\\) 显式取端点，读数可与 \\(z=1\\)、\\(z=-1\\) 代入 \\(H(z)\\) 对照",
              "样例题：\\(H(1)=2\\)、\\(H(-1)=1/1.5\\approx 0.6667\\)",
          ...
          },
          {
            ...
            detail: [
              "命令要点：用 randn 产生白噪声作为持续激励；用 filter 由已知 b、a 生成输出；估计可走频域（\\(S_{xy}./S_{xx}\\)，配合 cpsd 与 pwelch 或直接 fft 相除后平滑）或时域最小二乘。系统辨识工具箱提供 tfest/n4sid 等函数，但纯基础函数即可完成教学规模的辨识。逆滤波一侧，直接相除与加正则项（分母加一个小的 ε）两种做法要并排比较。",
            ...
            points: [
              ...
              "频域估计用 \\(S_{xy}/S_{xx}\\)，时域可用最小二乘；工具箱的函数只是封装",
        ...
        examples: [
          ...
          {
            title: "逐项检验两个系统",
            prompt: "判断 \\(y[n]=n\\)\\(\\cdot x[n]\\) 与 \\(y[n]=(x[n])^2\\) 的线性、时不变、因果、无记忆与 BIBO 稳定性。",
            steps: [
              "\\(y=n\\cdot x\\)：满足齐次与可加，故线性；系数含 n，故时变；输出只依赖当前输入，故无记忆且因果。",
              "取有界输入 \\(x[n]\\)≡1，输出 \\(y[n]=n\\) 在 n 遍历全部整数时无界，故非 BIBO 稳定。",
              "\\(y=x^2\\)：\\(T{2x}=4y\\ne 2T{x}\\)，故非线性；时不变、因果、无记忆。",
              "\\(|x|\\le 1\\) 时 \\(|y|\\le 1\\)，任一界 M 的输入给出界 M²，故 BIBO 稳定。"
            ],
            answer: "\\(y=n\\cdot x\\)：线性、时变、无记忆、因果、非稳定；\\(y=x^2\\)：非线性、时不变、因果、无记忆、稳定。"
          },
          {
            title: "一阶离散系统的全响应",
            ...
            steps: [
              ...
              "零输入响应由 \\(y[-1]=1\\) 递推：\\(y_{zi}[n]=0.5^(n+1)\\)（\\(n\\ge 0\\)）。",
              "全响应 \\(= 2(1-0.5^(n+1))+0.5^(n+1) = 2-0.5^(n+1)\\)。"
            ],
            answer: "零状态 \\(2(1-0.5^(n+1))\\)，零输入 \\(0.5^(n+1)\\)，全响应 \\(2-0.5^(n+1)\\)（\\(n\\ge 0\\)）。"
          },
          {
            title: "由频率响应读稳态输出",
            prompt: "系统 \\(H(z)=1/(1-0.5z^{-1})\\)，输入 \\(x[n]=\\cos (0.5\\pi n)\\)。求稳态输出。",
            steps: [
              "代入 \\(z^{-1}=e^{-j0.5\\pi }=-j\\)：\\(H=1/(1+0.5j)=0.8-0.4j\\)。",
              "幅值 \\(|H|=1/\\sqrt (1+0.25)=0.894\\)，相位 ∠\\(H=-arctan(0.5)=-0.4636\\) rad。",
              "稳态输出为 \\(0.894\\cos (0.5\\pi n-0.4636)\\)。"
            ],
            answer: "\\(0.894\\cos (0.5\\pi n-0.4636)\\)（幅度乘 |H|、相位加 ∠H；瞬态分量已衰减）。用递推拟合可核对：\\(A=0.8944\\)、\\(\\varphi =-0.4636\\)。"
          },
          {
            title: "复频域解含初值的差分方程",
          ...
          {
            title: "辨识的激励条件",
            prompt: "用 \\(x[n]=\\cos (0.1\\pi n)\\) 去辨识一个未知系统。能辨识出什么？缺什么？",
            steps: [
              "单一正弦只在 \\(\\omega =±0.1\\pi\\) 有能量。",
              "输出只在这两个频点携带系统信息，H 在其余频点完全未知。",
              "需要改用宽带激励（白噪声、扫频或多频叠加），使关心频段内都有能量。"
            ],
            answer: "只能得到 \\(\\omega =0.1\\pi\\) 附近的 H 值；其余频段不可辨识，必须换成持续激励输入。"
          },
          {
            title: "判断逆系统能否稳定实现",
            ...
            steps: [
              ...
              "因果逆 \\(H_{inv}=1/(1-2z^{-1})\\) 的极点在 \\(z=2\\)，位于单位圆外。",
          ...
          {
            title: "量化误差界的成立条件",
            prompt: "\\(\\Delta =0.1\\) V，输入是幅度 5 V 的正弦。能否直接估计量化信噪比？误差上界是多少？",
            steps: [
              "无溢出时确定性界 \\(|Q(x)-x|\\le \\Delta /2=0.05\\) V，对每个样本都成立。",
          ...
          },
          {
            ...
            steps: [
              "解析式：\\(y[0]=2(1-0.5)=1\\)；\\(y[3]=2(1-0.5^4)=1.875\\)。",
          ...
          {
            title: "直流增益与稳态值互相校验",
            prompt: "某系统阶跃响应的稳态值是 2.5，而 freqz 在 \\(\\omega =0\\) 读出 2.0。问题出在哪？",
            steps: [
          ...
          },
          {
            ...
            answer: "极点 \\(0.5 \\to\\) 稳定；极点 \\(2 \\to\\) 不稳定。残差项都是 1，差别只在极点。"
          },
          {
            title: "辨识与逆滤波的对照实验",
            prompt: "已知 \\(H(z)=1-2z^{-1}\\)，用白噪声做实验。辨识和逆滤波各会得到什么结果？",
            steps: [
              "辨识：白噪声在全频段有能量，\\(S_{xy}/S_{xx}\\) 能复现 |H|（\\(1-2z^{-1}\\) 的幅频）。",
        ...
        check: [
          {
            ...
            prompt: "\\(y[n]=n\\)\\(\\cdot x[n]\\) 是否为 BIBO 稳定系统？",
          ...
          },
          {
            ...
            prompt: "\\(y[n]=(x[n])^2\\) 的性质是？",
            options: ["线性且稳定", "非线性但时不变、因果且 BIBO 稳定", "线性但不稳定", "非线性且不稳定"],
            answer: 1,
            explanation: "\\(T{2x}=4y\\ne 2T{x}\\)，故非线性；其余性质成立，且 \\(|x|\\le M\\) 时 \\(|y|\\le M^2\\)，BIBO 稳定。"
          },
          {
            id: "signals-ch3-check-3",
            prompt: "因果离散 LTI 系统的冲激响应必须满足什么？",
            options: ["\\(h[n]=0\\)，n<0", "\\(h[n]=0\\)，n>0", "\\(\\Sigma h[n]=0\\)", "\\(h[n]\\) 必须周期"],
            answer: 0,
          ...
          },
          {
            ...
            options: ["0.5", "1", "2", "发散"],
            answer: 2,
            explanation: "阶跃响应为 \\(2(1-0.5^(n+1))\\)，\\(n\\to \\infty\\) 时趋于 2；这也等于直流增益 \\(H(1)=1/(1-0.5)=2\\)。"
          },
          {
          ...
          },
          {
            ...
            prompt: "LTI 系统对输入 \\(A \\cos (\\omega _0n+\\varphi )\\) 的稳态输出是？",
            options: ["\\(A \\cos (\\omega _0n+\\varphi )\\)", "\\(A|H|\\cos (\\omega _0n+\\varphi +\\)∠H)", "\\(|H|\\cos (\\omega _0n)\\)", "\\(A/H\\cdot \\cos (\\omega _0n)\\)"],
            answer: 1,
            explanation: "复指数是特征函数：幅度乘 \\(|H(e^{j\\omega _0})|\\)、相位加 ∠\\(H(e^{j\\omega _0})\\)；瞬态分量已衰减。"
          },
          {
          ...
          },
          {
            ...
            prompt: "舍入量化 \\(Q(x)=\\Delta\\)\\(\\cdot round(x/\\Delta )\\) 在无溢出时的误差满足？",
            options: ["\\(|Q(x)-x|\\le \\Delta\\)", "\\(|Q(x)-x|\\le \\Delta /2\\)", "误差恒为 Δ²/12", "误差与 Δ 无关"],
            answer: 1,
        ...
        summary: [
          "系统性质逐项检验：线性、时不变、因果、无记忆、BIBO 稳定、可逆彼此不可互推（\\(y=n\\cdot x\\) 与 \\(y=x^2\\) 是两个反例）。",
          "全响应 = 零输入 + 零状态；卷积只给零状态，\\(Y=HX\\) 也只在零状态下无条件成立。",
          "时域、频域、复频域是同一系统的三种等价描述：卷积↔相乘↔极零点与 ROC。",
          "系统辨识要求持续激励；逆滤波要求 H 的零点位置合适，否则噪声按 1/|H| 放大。",
          "数字实现新增采样与量化两道不可逆关口；量化误差界 \\(|Q-x|\\le \\Delta /2\\) 是确定性的，Δ²/12 另有假设。"
        ],
      ...
      },
      {
        ...
        sections: [
          {
            ...
            detail: [
              "从频域看，滤波就是给频谱乘一个窗函数形状的传输函数：\\(Y(j\\Omega )=H(j\\Omega )X(j\\Omega )\\)。理想低通在 |Ω|<Ωc 内让 \\(H=1\\)、之外为 0，但它的冲激响应是 sinc，非因果且无限长——物理上无法实现，只能逼近。因此实际滤波器必须接受三件事：通带不绝对平坦、阻带不绝对为零、通带到阻带之间有过渡带。这三件事构成了下一节全部指标的来源。",
            ...
            points: [
              "滤波 = 频域乘传输函数：\\(Y(j\\Omega )=H(j\\Omega )X(j\\Omega )\\)",
          ...
          },
          {
            ...
            detail: [
              "衰减用分贝表示：\\(A(\\Omega )=-20lg|H(j\\Omega )|\\)。通带指标写“\\(|\\Omega |\\le \\Omega p\\) 时 \\(A\\le Ap\\)”，阻带写“\\(|\\Omega |\\ge \\Omega\\) s 时 \\(A\\ge As\\)”，Ωp 与 Ωs 之间是过渡带，不作要求。这样写的好处是每一项都能在幅频曲线上量出来：Ap 决定通带能有多“平”，As 决定阻带能压多低，Ωs/Ωp 决定过渡带有多陡——三者共同决定阶数，缺一项就设计不出来。",
            ...
            points: [
              "衰减定义 \\(A(\\Omega )=-20lg|H(j\\Omega )|\\)；通带写 \\(A\\le Ap\\)，阻带写 \\(A\\ge As\\)，之间是过渡带",
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "巴特沃思低通的幅频特性是 \\(|H(j\\Omega )|^2=1/(1+(\\Omega /\\Omega c)^{2N})\\)。它有三个便于手算的性质：通带内最大平坦（前 2N−1 阶导数在 \\(\\Omega =0\\) 处为零）、幅频随 Ω 单调下降、无论阶数多少在 \\(\\Omega =\\Omega c\\) 处恒为 −3.0103 dB。设计流程就是由 Ap、As 与 Ωs/Ωp 解出所需阶数 N。",
            detail: [
              "阶数公式由两个边界条件联立得到：在 Ωp 处要求衰减不超过 Ap，在 Ωs 处要求衰减不少于 As，两式相除消去 Ωc，得 \\(N \\ge lg[(10^{As/10}-1)/(10^{Ap/10}-1)] / (2 lg(\\Omega s/\\Omega p))\\)，取不小于该值的最小整数。算例：\\(Ap=1\\) dB、\\(As=40\\) dB、\\(\\Omega s/\\Omega p=2\\) 时 \\(N \\ge 7.6185\\)，取 \\(N=8\\)；代回校验，\\(N=8\\) 在 Ωs 处的衰减为 42.297 dB，满足 \\(\\ge 40\\) dB 的要求。",
            ...
            points: [
              "巴特沃思幅频：\\(|H(j\\Omega )|^2=1/(1+(\\Omega /\\Omega c)^{2N})\\)，通带最大平坦、随 Ω 单调下降",
              "任意阶数在 \\(\\Omega =\\Omega c\\) 处恒为 −3.0103 dB（\\(|H|=0.7071\\)）",
              "阶数公式 \\(N \\ge lg[(10^{As/10}-1)/(10^{Ap/10}-1)]/(2 lg(\\Omega s/\\Omega p))\\)，取最小整数",
              "算例：\\(Ap=1\\) dB、\\(As=40\\) dB、\\(\\Omega s/\\Omega p=2 \\to N\\ge 7.6185\\) 取 \\(N=8\\)，校验 Ωs 处 42.297 dB（Ωc 由 Ωp 处恰为 Ap 确定，不是直接取 \\(\\Omega c=\\Omega p\\)）"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "切比雪夫低通的幅频特性是 \\(|H(j\\Omega )|^2=1/(1+\\varepsilon ^2T_N^2(\\Omega /\\Omega c))\\)，其中 \\(T_N\\) 是 N 阶切比雪夫多项式。它把误差均匀摊在通带内，形成等波纹，换来比同阶巴特沃思更陡的过渡带——代价是通带不再平坦，且相位非线性更明显。",
            detail: [
              "切比雪夫多项式在 \\(|x|\\le 1\\) 时满足 \\(T_N(x)=\\cos (N\\cdot arcco\\)s x)，在 \\([-1,1]\\) 内等幅振荡于 ±1；在 |x|>1 时 \\(T_N(x)=cosh(N\\cdot arcos\\)h x) 快速增长。这两段行为正好对应滤波器的两个频段：通带内 |H| 在 1 与 1/√(1+ε²) 之间等波纹起伏，阻带内则快速下降。纹波量由 ε 唯一决定：\\(Ap=10lg(1+\\varepsilon ^2)\\)，即 \\(\\varepsilon =\\sqrt (10^{Ap/10}-1)\\)。\\(Ap=1\\) dB 时 \\(\\varepsilon =0.5088\\)，通带内 |H| 在 0.8913 与 1.0000 之间起伏，在 \\(\\Omega =\\Omega c\\) 处恰好为 −1.0000 dB。",
              "阶数公式与巴特沃思不同形：切比雪夫要用反双曲余弦，\\(N \\ge arcosh\\sqrt [(10^{As/10}-1)/(10^{Ap/10}-1)] / arcosh(\\Omega s/\\Omega p)\\)，取不小于该值的最小整数。注意不能照搬巴特沃思的 lg/(2lg) 形式——那会得到明显偏高的阶数。算例：\\(Ap=1\\) dB、\\(As=40\\) dB、\\(\\Omega s/\\Omega p=2\\) 时分子为 \\(arcosh(\\sqrt 38619)=5.9737\\)、分母为 \\(arcosh(2)=1.31696\\)，得 \\(N\\ge 4.536\\) 取 \\(N=5\\)，而同一指标下巴特沃思需要 8 阶。选择哪一种取决于任务：如果通带内不允许起伏（例如需要精确的幅度测量），必须用巴特沃思；如果可以接受通带内的等波纹（例如只要信噪比达标），切比雪夫用更少阶数就能达到同样的阻带衰减。还有第三种选择——椭圆函数滤波器，它在通带与阻带都允许纹波，过渡带最陡，本节只作为对照提及。"
            ],
            points: [
              "切比雪夫幅频：\\(|H(j\\Omega )|^2=1/(1+\\varepsilon ^2T_N^2(\\Omega /\\Omega c))\\)，N 阶切比雪夫多项式",
              "\\(|x|\\le 1\\) 时 \\(T_N=\\cos (N arccos x)\\) 等幅振荡 → 通带等波纹；|x|>1 时按 cosh 快速增长 → 阻带陡降",
              "纹波由 ε 决定：\\(Ap=10lg(1+\\varepsilon ^2)\\)，即 \\(\\varepsilon =\\sqrt (10^{Ap/10}-1)\\)；\\(Ap=1\\) dB 时 \\(\\varepsilon =0.5088\\)",
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "归一化低通原型只有一套设计数据，其余频带全部靠频率变换得到。变换的做法是把原型中的复频率 s 用一个 s 的有理函数替换：低通到高通用 \\(s\\to \\Omega p/s\\)，低通到带通用把 s 换成含中心频率与带宽的二次式。变换后原来满足指标的原型，仍严格对应新的指标。",
            detail: [
              "低通到高通是最简单的一例：把 s 换成 Ωp/s，则原型在 Ω 处的响应被搬到 Ωp/Ω 处。原型的通带 \\(0\\le \\Omega \\le \\Omega p\\) 映射为 \\(\\Omega p/\\Omega \\ge \\Omega p\\)……即高频段保留，低频段被抑制，得到高通。同时原型的 \\(\\Omega =0\\)（直流最大平坦点）被映到 \\(\\Omega =\\infty\\)，因此高通在无穷远仍保持平坦。这个“倒频率轴”的观点可以直接用来核对变换后的指标：原型阻带边界 Ωs 应映到高通的通带边界。",
              "低通到带通与带阻用二次变换，把一个频率点映成一对频率点，因此需要两个参数：中心频率 Ω0 与带宽 B。变换后的滤波器阶数是原型的两倍（每个原型极点生成一对共轭极点）。工程上还有一步“归一化”：先把指标折算成 \\(\\Omega p=1\\) rad/s 的原型指标（这一步只做频率缩放），查表得到原型，再做频率变换与去归一化——这样一套表格能覆盖全部四种频带，也是教材把频率变换单独列节的原因。"
            ],
            points: [
              "四种频带都从归一化低通原型经频率变换得到：低通原型是唯一的“设计数据源”",
              "低通→高通：\\(s\\to \\Omega p/s\\)（倒频率轴，直流平坦点映到无穷远）",
          ...
          },
          {
            ...
            detail: [
              ...
              "一阶 RC 的截止频率是 \\(fc=1/(2\\pi RC)\\)，在 fc 处 \\(|H|=1/\\sqrt 2\\)。有源结构则通过反馈把 Q 值做得比无源 RC 高得多，从而在同样元件值下获得更陡的过渡带与更接近理论的响应。设计时的顺序通常是：指标 → 阶数与极点 → 分解为二阶节 → 分配 Q 与 \\(\\omega 0 \\to\\) 选拓扑与元件值 → 用蒙特卡洛或敏感度分析核对容差影响。"
            ],
            points: [
              "RC 有源结构避免电感：一阶 RC + 运放缓冲，二阶节用 Sallen-Key 或多重反馈",
              "高阶滤波器 = 一阶/二阶节级联，每节对应分母里的一对共轭极点",
              "一阶 RC：\\(fc=1/(2\\pi RC)\\)，截止处 \\(|H|=1/\\sqrt 2\\)",
          ...
          },
          {
            ...
            detail: [
              "数字域多出两件模拟域没有的事。第一是周期性与折叠：\\(H(e^{j\\omega })\\) 以 2π 为周期，只在一个周期内有意义，因此指标必须写在 \\(0\\le \\omega \\le \\pi\\) 内，且设计前要确定采样率——模拟指标 Ωp、Ωs 要按 \\(\\omega =\\Omega T\\) 折算成数字指标 ωp、ωs。第二是字长：系数与运算都在有限精度下进行，IIR 的反馈会把舍入误差累积，FIR 则没有这个问题。",
            ...
            points: [
              ...
              "\\(H(e^{j\\omega })\\) 以 2π 为周期，指标写在 \\(0\\le \\omega \\le \\pi\\)，设计前需由 \\(\\omega =\\Omega T\\) 折算",
          ...
          },
          {
            ...
            detail: [
              "冲激响应不变法：令 \\(h[n]=T\\)\\(\\cdot h_a(nT)\\)，则数字极点 \\(z=e^{s_{kT}}\\) 由模拟极点映射而来。映射把左半平面映到单位圆内，因此原型稳定则数字滤波器稳定。问题是时域采样对应频域周期延拓，若模拟响应在折叠频率以上衰减不够，混叠就会破坏阻带指标——所以它不适合高通与带阻，只适合带限的低通/带通。",
              "双线性变换法：\\(s=(2/T)(1-z^{-1})/(1+z^{-1})\\)。它把整条模拟虚轴一一映到单位圆上（Ω 从 −∞ 到 +∞ 对应 ω 从 −π 到 π），因此完全没有混叠，适用任何频带；代价是频率映射非线性：\\(\\Omega =(2/T)\\tan (\\omega /2)\\)，高端频率被压缩。补偿办法是预畸变——把每一个关键数字频率按 \\(\\omega =2arctan(\\Omega T/2)\\) 折算成模拟频率去设计原型，设计完再变换回来，这样关键点（通带边界、阻带边界）就能精确落位。本节算例：\\(T=1\\) ms、\\(\\omega p=0.4\\pi\\) 时预畸变得 \\(\\Omega p=1453.085\\) rad/s，回代精确得到 0.4π（往返误差 <1e−12）。"
            ],
            points: [
              "IIR：\\(H(z)=B(z)/A(z)\\)，带反馈，阶数低但相位一般非线性，必须核对极点",
              "冲激响应不变法：\\(z=e^{s_{kT}}\\)，稳定原型给出稳定数字滤波器，但有频谱混叠，不适合高通/带阻",
              "双线性变换：\\(s=(2/T)(1-z^{-1})/(1+z^{-1})\\)，虚轴一一映到单位圆，无混叠但有频率扭曲",
              "预畸变：\\(\\Omega =(2/T)\\tan (\\omega /2)\\) 及其反变换 \\(\\omega =2arctan(\\Omega T/2)\\)，用于让关键频率精确落位"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "FIR 数字滤波器是有限长冲激响应，没有反馈：\\(y[n]=\\Sigma b_k x[n-k]\\)。它天然 BIBO 稳定，且在系数对称（或反对称）时可获得严格线性相位，因此在对波形保真要求高的场合是首选。代价是达到同样过渡带陡度通常需要更高阶数，运算量更大。",
            detail: [
          ...
          },
          {
            ...
            detail: [
              "命令要点：\\(buttord(Wp,Ws,Rp,Rs,'s')\\) 由指标求巴特沃思最小阶数与截止频率（'s' 表示模拟域，省掉会被当成数字设计并因 Wp/Ws 越界报错）；cheb1ord 对应切比雪夫；butter/cheby1 带 's' 时按模拟设计、给出零极点增益 \\([z,p,k]\\)；zp2tf 转成多项式系数；\\(freqs(b,a,w)\\) 在指定模拟频率点上求频率响应。注意 freqs 的 w 必须是向量——传标量会被当成“点数 n”而在错误的频率上取值。求出的 \\(H(s)\\) 可以用 freqs 扫频，再与指标线比较——这是最直接的验收方式。",
              "验证点：以 \\(Ap=1\\) dB、\\(As=40\\) dB、\\(\\Omega s/\\Omega p=2\\) 为例，buttord 给出的阶数应为 8（手算 \\(N\\ge 7.6185\\)）；用 freqs 在 Ωp 处读出衰减应 \\(\\le 1\\) dB、在 Ωs 处应 \\(\\ge 40\\) dB（\\(N=8\\) 时理论值 42.297 dB）。若把 As 放松到 30 dB，阶数会降到 6——这个对照说明指标与阶数的直接关系。注意所有频率量都要用 rad/s 且与指标同一量纲，混用 Hz 与 rad/s 是最常见的错误。"
            ],
            points: [
              "buttord/cheb1ord 由指标算阶数；butter/cheby1 给零极点增益；freqs 扫频验收",
              "验证点：\\(Ap=1\\)、\\(As=40\\)、\\(\\Omega s/\\Omega p=2 \\to\\) 阶数 8；freqs 在 Ωp 处 \\(\\le 1\\) dB、Ωs 处 \\(\\ge 40\\) dB",
          ...
          },
          {
            ...
            detail: [
              ...
              "FIR 路径：fir1 用窗函数法（指定阶数、截止频率与窗），firpm 用等波纹逼近（指定阶数与各带的目标幅值与权重）；freqz 给出频率响应，grpdelay 给出群延迟。验证点：对称系数的 FIR 用 grpdelay 读出的群延迟应恒为 M/2；用 5 点移动平均（\\(b=ones(1,5)/5\\)）时群延迟应为 2 个样点，且在 \\(\\omega =\\pi /4\\) 处幅度为 0.4828（与理论式 \\(|\\sin (5\\omega /2)/(5\\sin (\\omega /2))|\\) 一致），直流增益为 1。"
            ],
            points: [
              "IIR：butter/cheby1 + bilinear/impinvar；双线性必须配合预畸变",
              "FIR：fir1（窗函数法）与 firpm（等波纹）；freqz 看幅频、grpdelay 看群延迟",
              "验证点：对称 FIR 的群延迟恒为 M/2；5 点移动平均群延迟 2 样点、\\(|H(e^{j\\pi /4})|=0.4828\\)、DC 增益 1",
        ...
        examples: [
          {
            ...
            steps: [
              "对矩形幅频做反变换，得到冲激响应 \\(h(t)=\\sin (\\Omega c t)/(\\pi t)\\)——一个无限长的 sinc。",
          ...
          {
            title: "由指标算巴特沃思阶数",
            prompt: "设计要求 \\(Ap=1\\) dB、\\(As=40\\) dB、\\(\\Omega s/\\Omega p=2\\)。求最小阶数与截止角频率位置。",
            steps: [
              "代入 \\(N \\ge lg[(10^{40/10}-1)/(10^{1/10}-1)]/(2 lg 2) = lg(9999/0.2589)/0.6021 = 7.6185\\)。",
              "取不小于该值的最小整数：\\(N=8\\)。",
              "校验：\\(N=8\\) 时在 Ωs 处的衰减为 \\(10lg[1+2^{16}(10^{0.1}-1)] = 42.297 dB \\ge 40\\) dB，满足要求。"
            ],
            answer: "\\(N=8\\)；Ωc 由“使 Ωp 处衰减恰为 Ap”确定（\\(Ap=1\\) dB 时 Ωc 略大于 Ωp），此时阻带在 Ωs 处的衰减为 42.297 dB，满足 \\(\\ge 40\\) dB。"
          },
          {
            title: "切比雪夫纹波与 ε",
            prompt: "切比雪夫滤波器允许 0.5 dB 通带纹波。求 ε，并说明通带内 |H| 的取值范围。",
            steps: [
              "由 \\(Ap=10lg(1+\\varepsilon ^2)\\) 反解：\\(\\varepsilon =\\sqrt (10^{0.05}-1)\\)。",
              "\\(10^{0.05}=1.1220\\)，故 \\(\\varepsilon =\\sqrt 0.1220=0.3493\\)。",
              "通带内 |H| 在 1 与 \\(1/\\sqrt (1+\\varepsilon ^2)=1/\\sqrt 1.1220=0.9441\\) 之间等波纹起伏。"
            ],
            answer: "\\(\\varepsilon \\approx 0.3493\\)；通带内 \\(|H|∈[0.9441, 1]\\)，对应 −0.5 dB 到 0 dB 的等波纹。"
          },
          {
            title: "由低通原型造高通",
            prompt: "归一化低通原型 \\(\\Omega p=1\\) rad/s、阻带边界 \\(\\Omega s=2\\) rad/s。用 \\(s\\to \\Omega p/s\\) 变换后，高通的通带与阻带边界是什么？",
            steps: [
              "变换把频率 Ω 映到 Ωp/Ω：原型的通带 \\(0\\le \\Omega \\le 1\\) 映为 \\(\\Omega p/\\Omega \\ge 1\\)，即高通通带 \\(|\\Omega |\\ge 1\\) rad/s。",
              "原型的阻带 \\(\\Omega \\ge 2\\) 映为 \\(\\Omega p/\\Omega \\le 0.5\\)，即高通阻带 \\(|\\Omega |\\le 0.5\\) rad/s。",
              "过渡带由原来的 1–2 rad/s 变为 0.5–1 rad/s，宽度与位置都随之改变。"
            ],
            answer: "高通通带 \\(|\\Omega |\\ge 1\\) rad/s、阻带 \\(|\\Omega |\\le 0.5\\) rad/s；原型的最大平坦点 \\(\\Omega =0\\) 映到 \\(\\Omega =\\infty\\)，所以高通在无穷远仍平坦。"
          },
          {
            title: "一阶 RC 与二阶节的 Q",
            prompt: "一阶 RC 取 \\(R=10 k\\Omega\\)、\\(C=10 nF\\)。求截止频率与该点的幅值比；若把它做成二阶节，Q 起什么作用？",
            steps: [
              "时间常数 \\(RC=10^4\\)\\(\\times 10^-8=10^-4\\) s。",
              "\\(fc=1/(2\\pi RC)=1/(2\\pi\\)\\(\\times 10^-4)\\approx 1591.5\\) Hz，该点 \\(|H|=1/\\sqrt 2\\)。",
              "二阶节的 Q 决定极点离虚轴的距离：Q 越大峰越尖、过渡带越陡，但对元件容差也越敏感。"
            ],
            answer: "\\(fc\\approx 1.59\\) kHz，\\(|H|=0.707\\)；Q 决定二阶节的陡度与敏感度，高阶滤波器里高 Q 节最需要做容差分析。"
          },
          {
            title: "模拟指标折算成数字指标",
            prompt: "采样率 \\(fs=1\\) kHz，设计要求通带到 100 Hz、阻带从 200 Hz 起。写出数字角频率指标。",
            steps: [
              "数字角频率与物理频率的关系是 \\(\\omega =2\\pi f/fs\\)。",
              "\\(\\omega p=2\\pi\\)\\(\\times 100/1000=0.2\\pi\\) rad；\\(\\omega s=2\\pi\\)\\(\\times 200/1000=0.4\\pi\\) rad。",
              "指标区间为 \\(0\\le \\omega \\le \\pi\\)（对应 0 到 \\(fs/2=500\\) Hz），因此本例未越界。"
            ],
            answer: "\\(\\omega p=0.2\\pi\\)、\\(\\omega s=0.4\\pi\\)；设计前必须做这一步折算，否则频率位置会整体偏移。"
          },
          {
            title: "双线性变换的预畸变",
            prompt: "\\(T=1\\) ms，要求数字通带边界 \\(\\omega p=0.4\\pi\\)。用双线性变换设计时，模拟原型应在哪个频率上满足通带边界？",
            steps: [
              "预畸变公式 \\(\\Omega =(2/T)\\tan (\\omega /2)\\)。",
              "代入 \\(\\omega =0.4\\pi\\)：\\(\\tan (0.2\\pi )=0.7265\\)，\\(\\Omega p=(2/10^-3)\\)\\(\\times 0.7265=1453.085\\) rad/s。",
              "校验往返：\\(\\omega =2arctan(\\Omega p T/2)=2arctan(0.7265)=0.4\\pi\\)，精确回到原值。"
            ],
            answer: "模拟原型应在 \\(\\Omega p=1453.085\\) rad/s 处满足通带指标；不做预畸变则实际通带边界会偏移。"
          },
          {
            title: "线性相位 FIR 的群延迟",
          ...
          },
          {
            ...
            prompt: "用 MATLAB 设计巴特沃思低通：\\(Ap=1\\) dB、\\(As=40\\) dB、\\(\\Omega s/\\Omega p=2\\)。写出流程，并说明如何验收。",
            steps: [
              "\\(wp=1\\); \\(ws=2\\); \\([N,wc]=buttord(wp,ws,1,40,'s')\\) 返回 \\(N=8\\)（与手算 7.6185 取整一致）。末尾的 's' 不能省：不加它 MATLAB 会按数字滤波器解释 w，而数字设计的 Wp、Ws 必须落在 (0,1)，本例的 \\(ws=2\\) 会直接报“截止频率必须在 (0,1) 区间内”。butter/cheby1/freqs 同样是模拟专用，调用时统一带 's'。",
              "\\([z,p,k]=butter(N,wc)\\) 得到零极点增益，zp2tf 转成 b、a。",
              "用 \\(freqs(b,a,[wp ws])\\) 读两点衰减（w 必须是向量）。验收标准是 wp 处 \\(\\le 1\\) dB、ws 处 \\(\\ge 40\\) dB；实测：buttord 返回的 \\(wc=1.1247\\) 让阻带恰好取到 40.000 dB、通带留 0.617 dB 余量，而正文手算的 \\(\\Omega c=1.088119\\)（让通带恰为 1 dB）在 ws 处给出 42.297 dB——两者都合格，差别只在把余量留在哪一端。"
            ],
            answer: "阶数 8；验收方式是在 Ωp、Ωs 两点读衰减并与指标比较，而不是只看曲线形状。"
          },
          {
            title: "5 点移动平均的频率响应与群延迟",
            prompt: "\\(b=[1 1 1 1 1]/5\\) 的 FIR。用 freqz 与 grpdelay 核对：群延迟多少？\\(\\omega =\\pi /4\\) 处幅度多少？",
            steps: [
              "长度 5 的对称 FIR，群延迟\\(=(5-1)/2=2\\) 个样点。",
              "幅频为 \\(|\\sin (5\\omega /2)/(5\\sin (\\omega /2))|\\)；代入 \\(\\omega =\\pi /4\\) 得 0.4828。",
              "DC（\\(\\omega =0\\)）处极限为 1，即直流增益为 1；\\(freqz(b,1,[0 pi/4])\\) 可直接读出这两点。"
            ],
            answer: "群延迟 2 样点；\\(|H(e^{j\\pi /4})|=0.4828\\)；DC 增益 1。"
          }
        ],
        experiments: [
          {
        ...
        check: [
          ...
          },
          {
            ...
            answer: 1,
            explanation: "低通原型是唯一的设计数据源；高通用 \\(s\\to \\Omega p/s\\)，带通/带阻用二次变换，阶数变为两倍。"
          },
          {
          ...
          },
          {
            ...
            prompt: "衰减的定义是？",
            options: ["\\(A=20lg|H|\\)", "\\(A=-20lg|H|\\)", "\\(A=|H|^2\\)", "\\(A=1-|H|\\)"],
            answer: 1,
            explanation: "衰减取负的分贝值，因此 |H| 越小 A 越大；通带要求 \\(A\\le Ap\\)，阻带要求 \\(A\\ge As\\)。"
          },
          {
            id: "signals-ch4-check-5",
            prompt: "巴特沃思滤波器在 \\(\\Omega =\\Omega c\\) 处的衰减是？",
            options: ["与阶数有关", "恒为 −3.0103 dB", "恒为 −1 dB", "恒为 0 dB"],
            answer: 1,
            explanation: "代入 \\(|H|^2=1/(1+1)=1/2\\)，与 N 无关；Ωc 是 −3 dB 点，不是阻带起点。"
          },
          {
            id: "signals-ch4-check-6",
            prompt: "巴特沃思阶数公式 \\(N \\ge lg[(10^{As/10}-1)/(10^{Ap/10}-1)]/(2lg(\\Omega s/\\Omega p))\\) 中，哪个量增大一定会使 N 增大？",
          ...
          },
          {
            ...
            options: ["\\(\\varepsilon =Ap\\)", "\\(\\varepsilon =\\sqrt (10^{Ap/10}-1)\\)", "\\(\\varepsilon =10^{Ap/20}\\)", "两者无关"],
            answer: 1,
            explanation: "由 \\(Ap=10lg(1+\\varepsilon ^2)\\) 反解得到；\\(Ap=1\\) dB 时 \\(\\varepsilon =0.5088\\)，通带 |H| 在 0.8913–1.0000 之间起伏。"
          },
          {
            id: "signals-ch4-check-9",
            prompt: "低通原型到高通的频率变换是？",
            options: ["\\(s\\to s+\\Omega p\\)", "\\(s\\to \\Omega p/s\\)", "\\(s\\to s^2\\)", "\\(s\\to s/\\Omega p\\)"],
            answer: 1,
            explanation: "\\(s\\to \\Omega p/s\\) 把频率轴取倒数：原型直流点映到无穷远，因此高通在无穷远保持平坦。"
          },
          {
          ...
          },
          {
            ...
            prompt: "\\(R=10 k\\Omega\\)、\\(C=10 nF\\) 的一阶 RC 低通，截止频率约为？",
            options: ["159 Hz", "1.59 kHz", "15.9 kHz", "159 kHz"],
            answer: 1,
            explanation: "\\(fc=1/(2\\pi RC)=1/(2\\pi\\)\\(\\times 10^-4)\\approx 1591.5\\) Hz；该点 \\(|H|=1/\\sqrt 2\\)。"
          },
          {
            ...
            answer: 1,
            explanation: "\\(\\omega =2\\pi f/fs=2\\pi\\)\\(\\times 100/1000=0.2\\pi\\) rad；数字指标必须落在 \\(0\\le \\omega \\le \\pi\\)。"
          },
          {
          ...
          },
          {
            ...
            answer: 1,
            explanation: "\\(\\Omega =(2/T)\\tan (\\omega /2)\\)：用该式把数字关键频率折算成模拟频率去设计原型，变换回来即可精确落在目标位置。"
          },
          {
        ...
        summary: [
          ...
          "巴特沃思最大平坦、Ωc 处恒 −3.0103 dB；切比雪夫以通带等波纹换更低阶数（\\(\\varepsilon =\\sqrt (10^{Ap/10}-1)\\)）。",
      ...
      },
      {
        ...
        sections: [
          {
            ...
            detail: [
              "概率结构由一族有限维分布函数描述：取定时刻 t₁,…\\(t_n\\)，它们的联合分布 \\(F(x_1\\)…\\(x_n\\); t₁,…\\(t_n)\\) 就是随机过程在这组时刻上的“快照”。严格（狭义）平稳要求这族分布对时间平移不变——任意平移后联合分布完全相同，这是很强的要求。宽（广义）平稳只要求两条：均值为常数、自相关函数只依赖时间差 \\(R_x(t_1,t_2)=R_x(\\tau )\\)，\\(\\tau =t_1-t_2\\)。工程上绝大部分结论建立在宽平稳上，因为功率谱与相关函数的全部关系只需要二阶矩。",
            ...
            points: [
              ...
              "宽平稳只需两条：均值为常数、\\(R_x(t_1,t_2)\\) 只依赖 \\(\\tau =t_1-t_2\\)",
          ...
          },
          {
            ...
            detail: [
              "定义与用途：均值 \\(\\mu _x=E{x(t)}\\)；方差 \\(\\sigma _x^2=E{[x(t)-\\mu _x]^2}=R_x(0)-\\mu _x^2\\)，即“总平均功率减去直流功率”；自相关函数 \\(R_x(\\tau )=E{x(t)x*(t-\\tau )}\\)，其 \\(\\tau =0\\) 处取值 \\(R_x(0)=E{|x(t)|^2}\\) 就是平均功率。互相关 \\(R_{xy}(\\tau )=E{x(t)y*(t-\\tau )}\\) 描述两个过程的关联，且满足 \\(R_{yx}(\\tau )=R_{xy}*(-\\tau )\\)。这几条把“随机信号有多强、变化多快、两路信号像不像”都变成了数字。",
              "两条常用性质要记牢：实过程的自相关是偶函数且 \\(|R_x(\\tau )|\\le R_x(0)\\)（\\(\\tau =0\\) 处取最大）；若过程含周期分量，\\(R_x(\\tau )\\) 也含同周期分量，因此相关函数可以用来检测被噪声淹没的周期信号——这正是第1章相关分析在统计意义上的延续。协方差函数 \\(C_x(\\tau )=R_x(\\tau )-\\mu _x^2\\) 则去掉了直流的影响，描述纯起伏部分的相关性。估计时用有限长记录的时间平均替代集合平均，估计方差随记录长度下降但不会为零，所以“测出来的相关函数”本身也带统计误差。"
            ],
            points: [
              "均值 \\(\\mu _x\\) 描述直流；方差 \\(\\sigma _x^2=R_x(0)-\\mu _x^2\\) 描述起伏，即总功率减直流功率",
              "自相关 \\(R_x(\\tau )=E{x(t)x*(t-\\tau )}\\)，\\(R_x(0)\\) 就是平均功率；估计它需要各态历经性，而不只是平稳",
              "实过程：\\(R_x(\\tau )\\) 为偶函数且 \\(|R_x(\\tau )|\\le R_x(0)\\)；含周期分量时相关函数同周期",
              "互相关满足 \\(R_{yx}(\\tau )=R_{xy}*(-\\tau )\\)；有限记录的时间平均估计本身带统计误差"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "随机信号没有确定频谱（它的傅里叶变换不存在），但它的平均功率随频率的分布是确定的——这就是功率谱密度 \\(S_x(\\omega )\\)。维纳—辛钦定理把 \\(S_x(\\omega )\\) 与自相关函数 \\(R_x(\\tau )\\) 连成一对傅里叶变换，于是时域统计与频域功率可以互相换算。",
            detail: [
              "为什么是“功率谱”而不是“频谱”：随机信号的每条样本能量无限，频谱密度不存在；但平均功率有限，因此描述的对象是功率在频率上的密度（单位 Hz 或 rad/s 上的功率）。维纳—辛钦关系 \\(S_x(\\omega )=\\int R_x(\\tau )e^{-j\\omega \\tau }d\\tau\\) 与 \\(R_x(\\tau )=(1/2\\pi )\\int S_x(\\omega )e^{j\\omega \\tau }d\\omega\\) 是本章使用频率最高的一对式子。由 \\(\\tau =0\\) 得 \\(R_x(0)=(1/2\\pi )\\int S_x(\\omega )d\\omega\\)：相关函数原点的取值等于功率谱曲线下的总面积——这条恒等式是检验推导与数值结果是否自洽的最快方法。",
              "性质方面：实过程的 \\(S_x(\\omega )\\) 是实的非负偶函数；白噪声的功率谱为常数，其自相关是 δ 函数（任意两个不同时刻都不相关）。带通过程、窄带过程的谱形与相关函数的包络之间存在对应关系，是通信里“包络检波”“相干时间”等概念的基础。估计功率谱最直接的方法是用周期图（对有限记录做 FFT 后取幅度平方），但周期图是有限长度有偏、渐近无偏而方差恒定的估计——它的根本缺陷是加长记录并不让曲线变平滑，把记录分段平均（Welch 法）才是用偏差换方差的常用折中。"
            ],
            points: [
              "随机信号没有确定频谱，描述对象是功率谱密度 \\(S_x(\\omega )\\)（单位频率上的平均功率）",
              "维纳—辛钦：\\(S_x(\\omega )\\) 与 \\(R_x(\\tau )\\) 互为傅里叶变换对",
              "恒等式 \\(R_x(0)=(1/2\\pi )\\int S_x(\\omega )d\\omega\\)：相关函数原点 = 功率谱总面积（自洽性速检）",
              "实过程 \\(S_x(\\omega )\\) 为非负偶函数；白噪声谱平坦、自相关为 δ"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            detail: [
              "推导思路：输出 \\(y(t)=h(t)*x(t)\\)。对均值取期望，\\(E{y}=E{x}\\int h(t)dt=\\mu _x\\)\\(\\cdot H(0)\\)——即均值被直流增益缩放。对相关函数做二阶矩运算可得 \\(R_y(\\tau )=R_x(\\tau )*h(\\tau )*h(-\\tau )\\)，两边取傅里叶变换即得 \\(S_y(\\omega )=|H(j\\omega )|^2S_x(\\omega )\\)。输入输出之间的互谱为 \\(S_{xy}(\\omega )=H*(j\\omega )S_x(\\omega )\\)，互相关为 \\(R_{xy}(\\tau )=R_x(\\tau )*h*(-\\tau )\\)。这里出现共轭，是本站互相关定义 \\(R_{xy}(\\tau )=E{x(t)y*(t-\\tau )}\\) 的直接结果——若改用 \\(R_{xy}(\\tau )=E{x(t)y(t+\\tau )}\\) 的定义，共轭就会消失，换教材或换软件前先确认约定。注意 \\(S_y\\) 只与 |H| 有关而与相位无关——这一点很重要：功率谱无法反映系统的相位信息，因此也不能靠输出功率谱辨识一个非最小相位系统。",
              "两条工程推论。第一，白噪声通过系统后，输出功率谱的形状就是 \\(|H(j\\omega )|^2\\) 的形状——这正是用白噪声激励测量系统频率响应的原理（配合相关或谱平均抑制估计方差）。第二，输出平均功率 \\(E{|y|^2}=R_y(0)=(1/2\\pi )\\int |H(j\\omega )|^2S_x(\\omega )d\\omega\\)，它把“输入功率谱有多少落在系统通带内”算成一个积分：窄带系统只放行落在通带内的那部分功率，这是信噪比计算的标准做法。"
            ],
            points: [
              "输出仍宽平稳；均值 \\(E{y}=\\mu _x\\)\\(\\cdot H(0)\\)（乘直流增益）",
              "自相关 \\(R_y(\\tau )=R_x(\\tau )*h(\\tau )*h(-\\tau )\\)；功率谱 \\(S_y(\\omega )=|H(j\\omega )|^2S_x(\\omega )\\)",
              "互谱 \\(S_{xy}(\\omega )=H*(j\\omega )S_x(\\omega )\\)、互相关 \\(R_{xy}(\\tau )=R_x(\\tau )*h*(-\\tau )\\)（按本站 \\(R_{xy}=E{x(t)y*(t-\\tau )}\\) 的定义；换定义则共轭消失）",
              "输出平均功率 \\(R_y(0)=(1/2\\pi )\\int |H|^2S_x d\\omega\\)；白噪声激励下输出谱形状 \\(= |H|^2\\)"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            importance: "core",
            sourceStatus: "verified_local",
            content: "离散情形与连续情形逐条对应：输出均值乘直流增益 \\(H(1)\\)、功率谱乘 \\(|H(e^{j\\omega })|^2\\)、相关函数与冲激响应做卷积。差别主要在记号：离散谱以 2π 为周期、通常只画 \\(0\\le \\omega \\le \\pi\\)（利用偶对称），但求功率的积分仍要覆盖一个 2π 周期；另外“白噪声”指样本间不相关（各样本独立同分布时更强）。这一节同时给出手算输出方差的简便公式。",
            detail: [
              "核心结论：\\(S_y(e^{j\\omega })=|H(e^{j\\omega })|^2S_x(e^{j\\omega })\\)；若输入是方差 \\(\\sigma _x^2\\) 的白噪声，则 \\(S_x\\) 为常数 \\(\\sigma _x^2\\)，于是输出方差 \\(R_y(0)=(\\sigma _x^2/2\\pi )\\int |H(e^{j\\omega })|^2d\\omega =\\sigma _x^2\\Sigma _k|h[k]|^2\\)。最后这个式子最好用：白噪声通过任何 FIR 滤波器，输出方差等于输入方差乘以系数平方和。算例：方差 9 的白噪声通过三点移动平均 \\(b=[1/3,1/3,1/3]\\)，\\(\\Sigma b^2=3\\)\\(\\times (1/3)^2=1/3\\)，输出方差 \\(9\\times 1/3=3\\)（数值验证 3.0053）；通过 5 点移动平均则 \\(\\Sigma b^2=1/5\\)，输出方差降到 1.8。",
              "离散情形还有一条实用性质：白噪声经线性系统后，输出的自相关函数等于 h 与其自身的（确定性）自相关卷积再乘 \\(\\sigma _x^2\\)。因此只要测出输出的相关函数，就能反推系统的冲激响应——这是相关法辨识系统的基础，也解释了为什么伪随机序列（PRBS）激励在工程辨识中这么常见：它接近白噪声，但可以精确复现。"
            ],
            points: [
              "\\(S_y(e^{j\\omega })=|H(e^{j\\omega })|^2S_x(e^{j\\omega })\\)；输出均值乘 \\(H(1)\\)",
              "白噪声输入时输出方差 \\(= \\sigma _x^2\\)\\(\\cdot \\Sigma |h[k]|^2\\)（用系数平方和直接算）",
              "算例：\\(\\sigma ^2=9\\) 经 3 点移动平均 → 方差 3；经 5 点移动平均 → 方差 1.8",
              "输出相关函数 \\(= \\sigma _x^2\\)\\(\\cdot (h*h_{rev})\\)，可用于由相关函数反推冲激响应（相关法辨识）"
            ],
            pitfalls: [
              "把“白噪声”当成“高斯噪声”：白噪声只要求样本间不相关，不必服从高斯分布",
              "用连续公式算离散方差：离散要用 \\(\\Sigma |h[k]|^2\\)（系数平方和），不是积分"
            ],
            links: [
          ...
          },
          {
            ...
            detail: [
              "为什么会有过渡：随机信号的激励从某一时刻开始，输出是输入与 h 的卷积，而 h 的“记忆”需要时间才展开。以离散情形为例，若输入白噪声在 \\(n\\ge 0\\) 才加入，则 \\(y[n]=\\Sigma _{k=0}^{n}h[k]x[n-k]\\)，求和上限随时间增长；输出方差 \\(\\sigma _y^2[n]=\\sigma _x^2\\Sigma _{k=0}^{n}h^2[k]\\) 单调增加到平稳值 \\(\\sigma _x^2\\Sigma _{k=0}^{\\infty }h^2[k]\\)。也就是说，过渡过程的长短由 h 的能量累积速度决定。",
            ...
            points: [
              ...
              "离散白噪声输入：\\(\\sigma _y^2[n]=\\sigma _x^2\\Sigma _{k=0}^{n}h^2[k]\\)，单调升到平稳值",
            ...
            links: [
              { to: "signals-ch5-lti-discrete", why: "稳态结论是本节 \\(n\\to \\infty\\) 的极限" },
          ...
          },
          {
            ...
            detail: [
              "问题设定：观测 \\(x(t)=s(t)+n(t)\\)，希望用 x 的线性滤波 ŝ\\((t)=h(t)*x(t)\\) 去逼近 \\(s(t)\\)，准则是最小均方误差 E{(s−ŝ)²}。正交性原理（误差与观测正交）给出维纳—霍夫方程 \\(R_x(\\tau )*h(\\tau )=R_{xs}(-\\tau )\\)，变换到频域即 \\(H_{opt}(j\\omega )=S_{xs}*(j\\omega )/S_x(\\omega )\\)——共轭同样来自本站的互相关定义 \\(R_{xs}(\\tau )=E{x(t)s*(t-\\tau )}\\)；若把互谱定义成 \\(E{s(t)x*(t-\\tau )}\\) 的变换，共轭就消失。若信号与噪声不相关，则 \\(S_{xs}=R_s\\) 的变换（实数）、\\(S_x=S_s+S_n\\)，于是 \\(H_{opt}=S_s/(S_s+S_n)\\)——这就是最常引用的形式：在信号功率占优的频段给高增益，在噪声占优的频段压低增益，形状由两者的功率比自动决定。",
              "边界必须写清楚。第一，它要求信号与噪声都是宽平稳的，并且已知（或能估计出）各自的功率谱——实际中统计量本身要估计，估计误差会直接进入 \\(H_{opt}\\)。第二，它是“线性”最优，非线性估计可能更好。第三，它给出的是稳态解，不处理启动阶段；而且若 \\(H_{opt}\\) 非因果（要求用到未来数据），实际实现需要引入延迟或改用因果维纳解。这些限制正是卡尔曼滤波与自适应滤波出现的动因。"
            ],
            points: [
              "准则：最小均方误差 E{(s−ŝ)²}，解由正交性原理（维纳—霍夫方程）给出",
              "频域解 \\(H_{opt}(j\\omega )=S_{xs}*(j\\omega )/S_x(\\omega )\\)（按本站互相关定义带共轭）；信号噪声不相关时退化为实数形式 \\(H_{opt}=S_s/(S_s+S_n)\\)",
              "直观含义：按各频段的信噪功率比分配增益",
              "前提：宽平稳 + 已知二阶统计量 + 线性最优 + 稳态解（非因果时需延迟或改因果解）"
            ],
            pitfalls: [
              "把 \\(H_{opt}=S_s/(S_s+S_n)\\) 当成通用公式：它要求信号与噪声不相关，否则要用互谱 \\(S_{xs}\\)",
          ...
          },
          {
            ...
            detail: [
              "递推结构（离散线性模型 \\(x_k=Ax_{k-1}+w_{k-1}\\)，\\(z_k=Hx_k+v_k\\)）：预测步 x̂\\(_k^-=Ax\\)̂\\(_{k-1}\\)、\\(P_k^-=AP_{k-1}A\\)ᵀ+Q；校正步 \\(K_k=P_k^-H\\)ᵀ\\((HP_k^-H\\)ᵀ+R)⁻¹、x̂\\(_k=x\\)̂\\(_k^-+K_k(z_k-Hx\\)̂\\(_k^-)\\)、\\(P_k=(I-K_{kH})P_k^-\\)。关键在于增益 K 由协方差自动算出：观测噪声 R 大则 K 小（更信模型），模型噪声 Q 大则 K 大（更信观测）。这也是它相对固定系数的维纳滤波最本质的差别——增益随时间自适应。",
            ...
            points: [
              ...
              "递推两步：预测（x̂\\(^-=Ax\\)̂、\\(P^-=APA\\)ᵀ+Q）→ 校正（\\(K=P^-H\\)ᵀ(HP⁻Hᵀ+R)⁻¹，x̂\\(=x\\)̂\\(^-+K(z-Hx\\)̂⁻)）",
          ...
          },
          {
            ...
            detail: [
              "LMS（最小均方）算法：\\(w[n+1]=w[n]+\\mu\\)\\(\\cdot e[n]\\)\\(\\cdot x[n]\\)，其中 \\(e[n]=d[n]-w\\)ᵀ\\([n]x[n]\\) 是瞬时误差，μ 是步长。它的做法是用瞬时误差平方的梯度替代真实均方误差的梯度（后者需要统计期望）。收敛的充分条件是 0<μ<2/(\\(N\\cdot P_x)\\)（\\(P_x\\) 为输入功率，N 为抽头数）：μ 太小收敛慢，太大则不收敛或稳态失调变大——步长是自适应滤波里最关键的参数。数值验证：两抽头、输入方差 9、目标系数 \\([0.8, -0.5]\\)、\\(\\mu =0.01\\) 时，运行 20 万点后权向量收敛到 \\([0.796, -0.489]\\)，与目标一致。",
              "与前面两种滤波的关系可以这样排：维纳滤波是“统计量已知时的最优解”，LMS 是“用数据逐步逼近这个解”，卡尔曼滤波则是“有了动态模型时的最优递推解”。三种方法的选择取决于你手上有多少先验信息：有完整统计量用维纳；有动态模型用卡尔曼；都没有就用自适应，并接受收敛时间与稳态失调的折中。RLS（递归最小二乘）用矩阵求逆引理加速收敛，代价是运算量从 \\(O(M)\\) 升到 \\(O(M^2)\\)。"
            ],
            points: [
              "LMS 更新式：\\(w[n+1]=w[n]+\\mu\\)\\(\\cdot e[n]\\)\\(\\cdot x[n]\\)，\\(e[n]=d[n]-w\\)ᵀ\\([n]x[n]\\)",
              "步长约束 \\(0<\\mu <2/((M+1)P_x)\\)：太小收敛慢，太大稳态失调大",
              "验证：两抽头、目标 \\([0.8,-0.5]\\)、\\(\\mu =0.01\\)、期望信号含方差 0.1 的噪声 → 收敛到 \\([0.796,-0.489]\\)",
          ...
          },
          {
            ...
            detail: [
              "STFT 的定义是 \\(X(t,\\omega )=\\int x(\\tau )w(\\tau -t)e^{-j\\omega \\tau }d\\tau\\)：用窗函数 w 在 t 附近截取一段做傅里叶变换，滑动窗口得到二维分布。它的分辨率受不确定性原理约束：窗越短时间分辨率越高、频率分辨率越低，反之亦然；两者乘积有下界，因此不存在“同时在时间和频率上都任意精确”的分布。工程上取窗长等于希望分辨的最短事件长度，或者让窗长跟随信号变化（自适应窗）。",
            ...
            points: [
              "STFT：\\(X(t,\\omega )=\\int x(\\tau )w(\\tau -t)e^{-j\\omega \\tau }d\\tau\\)，把信号分段加窗后逐段看频谱",
              "分辨率受不确定性原理约束：时间分辨率与频率分辨率不能同时任意高",
              "谱图 \\(= |STFT|^2\\)；维格纳—维莱分布聚集性好但有交叉项；科恩类用核函数折中",
          ...
          },
          {
            ...
            detail: [
              "连续小波变换的定义是 \\(W(a,b)=(1/\\sqrt |a|)\\int x(t)\\psi *((t-b)/a)dt\\)，其中 ψ 是母小波，a 是尺度（与频率成反比），b 是平移。与 STFT 的区别在于：STFT 的基函数频率不同但时宽相同（等带宽），小波的基函数带宽与中心频率成正比（等 Q）——这正是它对非平稳信号更合适的原因。常用母小波有 Morlet、Daubechies、Meyer 等，选择依据是信号的形态与所需的正则性。",
              "离散小波变换（DWT）在 \\(a=2^j\\)、\\(b=k\\)\\(\\cdot 2^j\\) 的网格上取样，可用滤波器组高效实现：每层把信号分成低通近似与高通细节，再对近似下采样继续分解，形成多分辨率分析。它的优点是变换后系数几乎不相关、能量集中，因而广泛用于去噪（对小系数做阈值收缩）与压缩。边界要写清楚：小波不是“更高级的傅里叶”，它是另一种基——对持续存在的平稳正弦，傅里叶更紧凑；对瞬态与突变，小波更紧凑。"
            ],
            points: [
              "CWT：\\(W(a,b)=(1/\\sqrt |a|)\\int x(t)\\psi *((t-b)/a)dt\\)，尺度 a 与频率成反比",
          ...
          },
          {
            ...
            detail: [
              ...
              "验证点：\\(\\sigma =3\\)、\\(N=2\\times 10\\)⁵ 时，样本均值应落在 0 附近、样本方差应落在 9 附近，但都不会正好等于理论值——这是有限样本的统计波动，不是代码错误。量级可以预先算出来：均值估计的标准差约 \\(\\sigma /\\sqrt N\\approx 0.0067\\)，因此均值通常落在 ±0.02（约 3σ）内；方差估计的相对标准差约 \\(\\sqrt (2/N)\\approx 0.32%\\)，对应绝对偏差约 ±0.09。若实测偏差明显超过这个范围，先怀疑随机数发生器的质量（简单线性同余发生器在 2×10⁵ 样本上就可能让方差偏高百分之一点几（本次复算实测偏高 1.7%）），而不是先怀疑公式。教学结论：报统计量时必须同时给出样本量与置信范围。"
            ],
            points: [
              "rng 固定种子保证可复现；randn 生成正态样本，乘 σ 设方差",
              "var 默认无偏（除以 N−1）；xcorr 的 'biased'/'unbiased' 决定是否除以 N",
              "验证点：\\(\\sigma =3\\)、\\(N=2\\)\\(\\times 10^5 \\to\\) 均值标准差 \\(\\approx \\sigma /\\sqrt N\\approx 0.0067\\)、方差相对标准差 \\(\\approx \\sqrt (2/N)\\approx 0.32%\\)，实测偏差应落在这个量级内",
              "有限样本必然有波动：均值标准差 \\(\\approx \\sigma /\\sqrt N\\)，方差相对标准差 \\(\\approx \\sqrt (2/N)\\)，报数要带样本量"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            detail: [
              ...
              "验证点：对白噪声，单段周期图的曲线会在真值附近剧烈起伏（起伏幅度不随记录长度减小），而分段平均后的曲线明显平滑并收敛到平坦的 σ²；把信号换成白噪声通过 3 点移动平均滤波器的输出，估计出的谱形状应等于 \\(|\\sin (3\\omega /2)/(3\\sin (\\omega /2))|^2\\times \\sigma\\)²，在 \\(\\omega =0.5/1.0/1.5/2.0\\) 处实测比值 0.846/0.488/0.158/0.003 与理论 0.843/0.481/0.145/0.003 逐点吻合。"
            ],
            points: [
              "pwelch 分段平均；periodogram 单段周期图；cpsd/mscohere 用于互谱与相干",
              "单段周期图方差大且不随记录长度减小，分段平均用偏差换方差",
              "验证点：白噪声经 3 点移动平均 → 输出谱形状 \\(= |H|^2\\cdot \\sigma\\)²",
          ...
          },
          {
            ...
            detail: [
              "命令要点：\\(filter(b,1,x)\\) 得到输出；pwelch 分别估计输入输出谱，比较 \\(S_y./S_x\\) 与 \\(freqz(b,1,w)\\) 的 |H|²；用 xcorr 估计输出自相关并与理论式对照；画 \\(\\sigma _y^2[n]=\\sigma _x^2\\Sigma _{k\\le n}h^2[k]\\) 的累积曲线观察过渡过程何时结束。",
              "验证点（三条互相对照）：其一，输出方差用系数平方和直接算得 \\(\\sigma _x^2\\Sigma b^2\\)——\\(\\sigma ^2=9\\) 经 3 点移动平均得 3（仿真 3.0053），经 5 点移动平均得 1.8；其二，输出谱与输入谱之比应等于 |H|²，实测在四个频点上与理论吻合（见上一小节数据）；其三，输出均值应等于输入均值乘 \\(H(1)=1\\)（移动平均的直流增益为 1），因此均值不变。三条一致才说明“谱相乘”这套结论在你的数据和参数下真的成立。"
            ],
            points: [
              "路径一：filter 时域仿真得输出方差；路径二：pwelch 估谱后与 |H|² 比较",
              "白噪声输出方差 \\(= \\sigma _x^2\\Sigma |h|^2\\)：3 点平均 \\(\\to 3\\)（仿真 3.0053），5 点平均 \\(\\to 1.8\\)",
              "移动平均直流增益 \\(H(1)=1\\)，因此输出均值不变",
              "过渡过程用 \\(\\sigma _y^2[n]=\\sigma _x^2\\Sigma _{k\\le n}h^2[k]\\) 的累积曲线观察"
            ],
            pitfalls: [
          ...
          },
          {
            ...
            detail: [
              "命令要点：核心就是五行——预测 x̂\\(^-=A*x\\)̂、\\(P^-=A*P*A'+Q\\)；算增益 \\(K=P^-*H'/(H*P^-*H'+R)\\)；校正 x̂\\(=x\\)̂⁻+K*(z−H*x̂⁻)、\\(P=(I-K*H)*P^-\\)。用 randn 生成过程噪声与观测噪声（按 \\(sqrt(Q)\\)、\\(sqrt(R)\\) 缩放），把估计误差与真值一起画出来。",
              "验证点：取一维常值状态（\\(A=1\\)、\\(H=1\\)）、\\(Q=1e-4\\)、\\(R=1\\)，观测噪声标准差为 1 时，卡尔曼输出的方差应显著小于原始观测的方差（理论上稳态增益 \\(K\\approx 0.0099\\)，输出方差约为输入的 1%），且随 R 减小而更贴近观测、随 Q 增大而更贴近模型。把 Q 设得比真实过程噪声小一个数量级，会观察到滤波器跟不上真实变化（滞后）——这正是“Q 设小了迟钝、设大了抖动”的实证。"
            ],
            points: [
              "自写递推五行：预测（x̂⁻、P⁻）→ 增益 \\(K \\to\\) 校正（x̂、P）",
              "randn 按 \\(sqrt(Q)\\)、\\(sqrt(R)\\) 缩放生成噪声",
              "验证点：\\(A=H=1\\)、\\(Q=1e-4\\)、\\(R=1 \\to\\) 输出方差约为观测方差的 1%",
          ...
          },
          {
            ...
            detail: [
              "命令要点：MATLAB 的 wvd 提供维格纳—维莱分布与平滑伪 WVD（MathWorks 帮助文档中该函数分别列在小波工具箱与信号处理工具箱的参考页下，版本不同位置可能不同，用前先确认本机 toolbox 路径）；也可按定义自行实现——先取解析信号（hilbert），再对每个时刻计算 \\(x(t+\\tau /2)x*(t-\\tau /2)\\) 关于 τ 的傅里叶变换，这样能看清它的二次型结构与交叉项从何而来。单分量时取线性调频（chirp）信号，双分量时把两个不同起止时刻的 chirp 相加。",
        ...
        examples: [
          {
            title: "判断宽平稳并核对自洽性",
            prompt: "某过程均值恒为 0.5，自相关 \\(R_x(t_1,t_2)=e^{-3|t_1-t_2|}\\)。它是宽平稳的吗？平均功率与方差各是多少？",
            steps: [
              "宽平稳要求均值为常数——本例满足。",
              "自相关只依赖时间差 \\(\\tau =t_1-t_2\\)——本例满足，故是宽平稳的。",
              "平均功率 \\(R_x(0)=e^0=1\\)；方差 \\(\\sigma ^2=R_x(0)-\\mu ^2=1-0.25=0.75\\)（非负，自洽）。"
            ],
            answer: "宽平稳；平均功率 1、方差 0.75。顺手用 \\(R_x(0)\\ge \\mu ^2\\) 检查一组统计量是否自洽，是最快的一致性判据。"
          },
          {
            title: "由相关函数求方差",
            prompt: "零均值实过程的自相关 \\(R_x(\\tau )=9e^{-2|\\tau |}\\cos (10\\tau )\\)。求平均功率与方差。",
            steps: [
              "平均功率即 \\(R_x(0)=9\\)\\(\\times 1\\times 1=9\\)。",
              "零均值时方差等于平均功率，故 \\(\\sigma _x^2=9\\)。",
              "\\(R_x\\) 中的 \\(\\cos (10\\tau )\\) 说明过程含角频率 10 rad/s 的周期分量。"
            ],
            answer: "平均功率与方差都是 9；相关函数里的 cos 项对应过程的周期成分，可用它检测被噪声淹没的周期信号。"
          },
          {
            title: "由自相关求功率谱",
            prompt: "\\(R_x(\\tau )=e^{-a|\\tau |}\\)（a>0）。求 \\(S_x(\\omega )\\)，并用 \\(R_x(0)\\) 核对总功率。",
            steps: [
              "对 \\(e^{-a|\\tau |}\\) 做傅里叶变换得 \\(S_x(\\omega )=2a/(a^2+\\omega ^2)\\)（洛伦兹型）。",
              "核对：\\(R_x(0)=1\\)；\\((1/2\\pi )\\int 2a/(a^2+\\omega ^2)d\\omega =(1/2\\pi )\\)\\(\\times 2\\pi =1\\)，一致。",
              "频谱在 \\(\\omega =0\\) 处最大（2/a），随 ω 增大按 1/ω² 衰减——相关函数衰减越快（a 越大），谱越宽。"
            ],
            answer: "\\(S_x(\\omega )=2a/(a^2+\\omega ^2)\\)，且 \\((1/2\\pi )\\int S_x d\\omega =R_x(0)=1\\)；相关越快衰减，功率谱越宽。"
          },
          {
            title: "白噪声通过一阶 RC 低通的输出功率",
            prompt: "白噪声功率谱密度为 N₀/2，通过截止角频率 Ωc 的一阶 RC 低通 \\(|H|^2=1/(1+(\\Omega /\\Omega c)^2)\\)。求输出平均功率。",
            steps: [
              "输出功率 \\(= (1/2\\pi )\\int |H|^2S_x d\\Omega = (N_0/2)\\cdot\\)(1/2π)∫dΩ/(1+(Ω/Ωc)²)。",
              "积分 \\(\\int _{-\\infty }^{\\infty }d\\Omega /(1+(\\Omega /\\Omega c)^2)=\\pi \\Omega c\\)。",
              "结果 \\(= (N_0/2)\\)\\(\\cdot (\\pi \\Omega c/2\\pi )=N_0\\Omega c/4\\)。"
            ],
          ...
          },
          {
            ...
            steps: [
              "输出方差 \\(= \\sigma _x^2\\Sigma |h[k]|^2\\)。",
              "3 点：\\(\\Sigma h^2=3\\)\\(\\times (1/3)^2=1/3\\)，输出方差 \\(9\\times 1/3=3\\)。",
              "5 点：\\(\\Sigma h^2=5\\)\\(\\times (1/5)^2=1/5\\)，输出方差 \\(9\\times 1/5=1.8\\)。"
            ],
            answer: "3 点 \\(\\to 3\\)（仿真 3.0053）；5 点 \\(\\to 1.8\\)。平均点数越多，噪声压制越强，代价是带宽变窄、延迟变大。"
          },
          {
            title: "过渡过程有多长",
            prompt: "离散一阶系统 \\(h[n]=0.8^n u[n]\\)，输入白噪声方差为 1。求稳态输出方差，并估算需要的预热长度。",
            steps: [
              "稳态方差 \\(= \\sigma _x^2\\Sigma _{k=0}^{\\infty }h^2[k]=\\Sigma (0.64)^k=1/(1-0.64)=2.778\\)。",
              "到 n 点为止的累积方差 \\(= (1-0.64^{n+1})/(1-0.64)\\)。",
              "要求累积达到稳态的 99%：\\(1-0.64^{n+1}=0.99\\) ⇒ \\(0.64^{n+1}=0.01\\) ⇒ \\(n+1\\approx ln0.01/ln0.64\\approx 10.3\\)，故 \\(n\\approx 10\\)。"
            ],
          ...
          },
          {
            ...
            steps: [
              "不相关时 \\(H_{opt}=S_s/(S_s+S_n)\\)。",
              "代入得 \\(H_{opt}=8/10=0.8\\)。",
              "该频点信噪功率比为 \\(8/2=4\\)（约 6 dB），增益 0.8 是信号占比的直接体现。"
            ],
            answer: "\\(H_{opt}=0.8\\)；信号占比越高增益越接近 1，噪声占优的频段增益趋近 0。"
          },
          {
            title: "卡尔曼增益的稳态值",
            prompt: "一维常值状态（\\(A=1\\)、\\(H=1\\)），\\(Q=10^-4\\)、\\(R=1\\)。求稳态卡尔曼增益与输出方差近似值。",
            steps: [
              "稳态时各量都是常数（P⁻ 为先验/预测协方差、P 为后验/滤波后协方差）：\\(P^-=P+Q\\)（\\(A=1\\)）、\\(P=P^-(1-K)\\)、\\(K=P^-/(P^-+R)\\)。",
              "消去 P、P⁻ 得稳态标量 Riccati 方程 \\(K^2R+QK-Q=0\\)（等价 \\(P^2+PQ-RQ=0\\)）；代入 \\(Q=10^-4\\)、\\(R=1\\) 解得 \\(K=(-10^-4+\\sqrt (10^-8+4\\)\\(\\times 10^-4))/2\\approx 0.00995\\)。",
              "输出方差约为观测方差 \\(R=1\\) 的 \\(K/(1)\\approx 1%\\) 量级。"
            ],
            answer: "稳态增益 \\(K\\approx 0.0099\\)；R 远大于 Q 时滤波器几乎不信观测，输出方差压到观测的约 1%。"
          },
          {
            title: "LMS 收敛到维纳解",
            prompt: "两抽头、输入为方差 9 的白噪声、目标系数 \\([0.8, -0.5]\\)、步长 \\(\\mu =0.01\\)。运行足够长后权向量收敛到哪？",
            steps: [
              "LMS 的均值收敛到维纳解 \\(w*=R^-1p\\)，本例即目标系数 \\([0.8, -0.5]\\)。",
              "步长需满足充分条件 0<μ<2/(\\(N\\cdot P_x)\\)，N 为抽头数：两抽头时 0<μ<2/(\\(2\\times 9)\\approx 0.111\\)，0.01 在范围内。注意这是充分条件而不是精确的发散阈值。",
              "数值仿真（两抽头、期望信号 \\(d[n]=w*\\)ᵀ\\(x[n]+v[n]\\)，v 为零均值噪声，方差 0.1，20 万点）得 \\(w=[0.796, -0.489]\\)：噪声使权向量在目标附近随机游走，若去掉噪声则会精确收敛到 \\([0.8, -0.5]\\)。"
            ],
            answer: "带噪声时收敛到约 \\([0.8, -0.5]\\) 附近（实测 \\([0.796, -0.489]\\)）；无噪声时精确收敛到目标值。步长的充分条件是 μ<2/(\\(N\\cdot P_x)=0.111\\)，超过该值才可能发散。"
          },
          {
            title: "STFT 的分辨率折中",
            ...
            steps: [
              ...
              "取窗长\\(\\approx 5\\) ms（50 个样点）：频率分辨率约 200 Hz，足以分辨 1 kHz 附近的粗结构。",
          ...
          {
            title: "小波尺度与频率的换算",
            prompt: "用 Morlet 母小波（中心频率约 0.8125 Hz 归一化）分析 \\(\\omega =125.66\\) rad/s（20 Hz）的成分，尺度应取多少？",
            steps: [
              "换算关系为 f=a_中心/ (a·Δ)，其中 a_中心 为母小波中心频率，Δ 为采样间隔。",
              "取采样率 100 Hz（\\(\\Delta =0.01\\) s）、目标 \\(f=20\\) Hz：\\(a=0.8125/(20\\)\\(\\times 0.01)=4.06\\)。",
          ...
          {
            title: "样本统计量与理论值的差距",
            prompt: "\\(\\sigma =3\\)、\\(N=200000\\) 的白噪声样本，样本均值与方差相对理论值 0 与 9 应有多大偏差才算正常？",
            steps: [
              "均值估计的标准差约为 \\(\\sigma /\\sqrt N=3/447\\approx 0.0067\\)，故 3σ 范围约 ±0.02。",
              "方差估计的相对标准差约为 \\(\\sqrt (2/N)\\approx 0.0032\\)，对应绝对偏差约 ±0.09（3σ）。",
          ...
          },
          {
            ...
            steps: [
              "路径一（系数平方和）：\\(\\sigma _x^2\\Sigma b^2=9\\)\\(\\times (1/3)=3\\)。",
        ...
        check: [
          ...
          },
          {
            ...
            answer: 1,
            explanation: "\\(R_x(0)=E{|x|^2}\\) 是总平均功率；方差 \\(\\sigma ^2=R_x(0)-\\mu ^2\\) 是去掉直流后的起伏功率，零均值时二者才相等。"
          },
          {
            id: "signals-ch5-check-3",
            prompt: "实过程的自相关函数满足？",
            options: ["奇函数且无界", "偶函数且 \\(|R_x(\\tau )|\\le R_x(0)\\)", "恒为常数", "只在 \\(\\tau =0\\) 有定义"],
            answer: 1,
            explanation: "由定义可直接验证；含周期分量时 \\(R_x(\\tau )\\) 也含同周期分量，可用于检测被噪声淹没的周期信号。"
          },
          {
            id: "signals-ch5-check-4",
            prompt: "互相关函数满足的关系是？",
            options: ["\\(R_{yx}(\\tau )=R_{xy}(\\tau )\\)", "\\(R_{yx}(\\tau )=R_{xy}*(-\\tau )\\)", "\\(R_{yx}(\\tau )=-R_{xy}(\\tau )\\)", "两者无关"],
            answer: 1,
            explanation: "换下标要连共轭和符号一起换；实信号时才写作 \\(R_{xy}(-\\tau )\\)。"
          },
          {
            ...
            answer: 1,
            explanation: "\\(S_x(\\omega )=\\int R_x(\\tau )e^{-j\\omega \\tau }d\\tau\\)；由 \\(\\tau =0\\) 得 \\(R_x(0)=(1/2\\pi )\\int S_x d\\omega\\)，可用于自洽性速检。"
          },
          {
          ...
          },
          {
            ...
            answer: 0,
            explanation: "\\(S_y=|H|^2S_x\\)；注意它只含幅频信息，不含相位，因此无法据此区分非最小相位系统。"
          },
          {
            id: "signals-ch5-check-8",
            prompt: "输出均值与输入均值的关系是？",
            options: ["相等", "输出均值 = 输入均值 × \\(H(0)\\)", "输出均值恒为 0", "输出均值 = 输入均值 × \\(|H(1)|^2\\)"],
            answer: 1,
            explanation: "对卷积取期望即得 \\(\\mu _y=\\mu _x\\int h dt=\\mu _x\\)\\(\\cdot H(0)\\)，即被直流增益缩放。"
          },
          {
            ...
            options: ["9", "3", "1", "1.8"],
            answer: 1,
            explanation: "\\(\\sigma _y^2=\\sigma _x^2\\Sigma h^2=9\\times\\)(\\(3\\times (1/3)^2)=9/3=3\\)；5 点移动平均则为 1.8。"
          },
          {
            id: "signals-ch5-check-10",
            prompt: "白噪声通过线性系统后，输出自相关函数与什么有关？",
            options: ["只与输入均值有关", "等于 \\(\\sigma _x^2\\) 乘以 h 与自身的相关（可用于相关法辨识）", "恒为常数", "与 h 无关"],
            answer: 1,
            explanation: "\\(R_y=\\sigma _x^2(h*h_{rev})\\)，因此可由输出相关函数反推冲激响应——这就是相关法辨识与 PRBS 激励的依据。"
          },
          {
            id: "signals-ch5-check-11",
            prompt: "过渡期内输出方差随时间变化的原因是？",
            options: ["系统不稳定", "求和的冲激响应项数随时间增加：\\(\\sigma _y^2[n]=\\sigma _x^2\\Sigma _{k\\le n}h^2[k]\\)", "噪声非白", "采样率变化"],
            answer: 1,
          ...
          },
          {
            ...
            prompt: "维纳滤波的频域最优解是？",
            options: ["\\(H_{opt}=S_x/S_{xs}\\)", "\\(H_{opt}=S_{xs}*/S_x\\)（信号与噪声不相关时退化为 \\(S_s/(S_s+S_n)\\)）", "\\(H_{opt}=|H|^2\\)", "\\(H_{opt}=1\\)"],
            answer: 1,
            explanation: "由正交性原理（维纳—霍夫方程）导出；共轭来自本站 \\(R_{xs}(\\tau )=E{x(t)s*(t-\\tau )}\\) 的定义。含义是按各频段的信噪功率比分配增益，不相关时互谱为实、退化为 \\(S_s/(S_s+S_n)\\)。"
          },
          {
          ...
          },
          {
            ...
            options: ["\\(w[n+1]=w[n]+\\mu e[n]x[n]\\)，\\(0<\\mu <2/((M+1)P_x)\\)", "\\(w[n+1]=w[n]-\\mu x[n]\\)，μ 任意", "\\(w[n+1]=w[n]+e[n]\\)，与 μ 无关", "\\(w[n+1]=\\mu w[n]\\)"],
            answer: 0,
        ...
        summary: [
          ...
          "均值、方差、相关函数是时域数字特征；\\(R_x(0)\\) 是平均功率，\\(\\sigma ^2=R_x(0)-\\mu ^2\\) 是起伏功率。",
          "维纳—辛钦定理把自相关与功率谱连成一对变换，\\(R_x(0)=(1/2\\pi )\\int S_x d\\omega\\) 可作自洽性速检。",
          "随机信号通过稳定 LTI 系统：均值乘 \\(H(0)\\)、功率谱乘 |H|²、互谱乘 H；白噪声输入时输出方差 \\(= \\sigma ^2\\Sigma |h|^2\\)。",
