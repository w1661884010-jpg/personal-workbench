      ...
      },
      {
        ...
        sections: [
          ...
          },
          {
            ...
            detail: [
              "命令要点：维格纳—维莱分布在 MATLAB 里由小波工具箱的 wvd 提供（含平滑伪 WVD 版本），不在 Signal Processing Toolbox 中；也可按定义自行实现——先取解析信号（hilbert），再对每个时刻计算 x(t+τ/2)x*(t−τ/2) 关于 τ 的傅里叶变换，这样能看清它的二次型结构与交叉项从何而来。单分量时取线性调频（chirp）信号，双分量时把两个不同起止时刻的 chirp 相加。",
