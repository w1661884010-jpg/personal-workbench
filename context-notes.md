...
- 用户反复复测暂未再出现残影，按要求标记已修复；浏览器状态更新延迟未得到证实，不继续添加残影补丁。
- 393px 实测 SVG 369×360，viewBox 1200×720，等比绘图仅 221.4px 高，上下各 69.3px 空带。采用 ≤900px 的等比容器覆盖，保留有效绘图区大小与坐标，不拉伸图形、不改桌面或 3000。
- 本轮仅本地修改，不提交或推送。滚动状态问题不混入本轮。
- 核验：新增 5 项数字/模拟边界和缩放测试通过；完整测试 95/95（零跳过），Pages 构建与 diff 检查通过。393px 截图 mobile-canvas-fit-after.png 确认上下空带消失；未进行微信真机测试。
# 2026-09-07 自适应画布与元件查找
- 发布授权更新：用户现已要求提交、推送并同步现有 GitHub Pages；此前“未提交/未推送”是开发阶段记录。仅发布 3010，3000 不变。
- 新需求优先：上一轮 5:3 外框虽去空带但手机画布偏小，改为较高外框和动态 viewBox；背景跟随视野扩展，不能用 preserveAspectRatio=none 拉伸元件。
- 元件按名称/类型过滤，不持久化搜索；移动网格纵向浏览替代单行横滚；编辑操作仅未选中时收起。保持桌面控制与存档/仿真逻辑。
- 发现共享 geometry 的避让函数夹紧到 1200×720；3010 新增 circuit-placement.ts，复用碰撞判断并沿用环形搜索，移除固定边界，载入归一化同样使用新逻辑，3000 未改。
- 实测 393×1050：画布高 682.5px、未选中工具栏高 80.7px；截图 workbench-space-overview.png。98/98 测试零跳过，bundle 和 Pages 构建、diff 检查通过；未进行微信真机测试。未提交/推送。
# 3000 依赖隔离
- 验证：两份完整套件均 135/135，零跳过；新增隔离测试初次缺 CSS 输出路径，修正后完整复跑通过。
- 回收异常：Microsoft.VisualBasic.FileIO.FileSystem.DeleteDirectory 使用 SendToRecycleBin 返回成功且源目录不存在，但 Shell 回收站与 C/D 回收站索引未找到对应条目。不要承诺原仓库历史可恢复。D:/backups/personal-workbench-sites-src-backup-2026-09-05.zip 是既有源码备份，不等同删除前完整 Git 仓库。
- 源目录不存在后两份 npm run build 通过。未提交、未推送。esbuild 0.28.0 有低风险开发服务器漏洞，本站不使用其服务器，本轮未升级。
- 3010 工作区初始干净；3020 的运行文件及测试与 3010 一致。
- 复制必要的电路模块、预设、课程类型和工作台 CSS，不迁入整个旧应用。
- 两份站点各自安装依赖，3020 不依赖 3010 的 node_modules；保留既有 sync.mjs 的运行文件同步边界。
- 原 3000 仓库包含未配置远端的历史，必须使用回收站，不永久删除。
# README 实拍说明
- 实拍 localhost:3010，Chrome，1440×1050 暗色；演练和工作台截取有效说明区域。三个入口切换、添加开关/LED、连线及启动均成功，pageerror 为零。
