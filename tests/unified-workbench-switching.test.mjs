...
test("mount/layout tasks live outside the switch cancellation scope", async () => {
  ...
  assert.match(entry, /scheduleBridge[\s\S]{0,420}scheduleLayout\(/, "the instruments bridge is scheduled as a layout task");
});
...
test("wide workbench controls spend space on controls instead of empty fixed columns", async () => {
  const styles = await read("styles.css");
  const wideStart = styles.lastIndexOf('"pad head strip"');
  ...
  assert.match(wideOverride, /"pad head strip"\s*"pad storage storage"\s*"main main main"/, "storage should have its own full-width row (with the switcher pad column) instead of forcing all three groups to share a height");
